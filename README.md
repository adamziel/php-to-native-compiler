...
- builtins for the documented scalar/array/object subset: `strlen`, `isset`,
  ...
  including loose and strict scalar search-value filtering, `array_reverse`
  with default reindexing and boolean preserve-key behavior, `array_merge`
  over zero or more array operands, `in_array` and `array_search` including
  strict scalar searches, `var_dump`, and `print_r`
