...
- `str_replace()` supports scalar byte replacement plus bounded array
  search/replacement/subject forms, preserves outer subject array keys, writes
  the optional `$count` argument by reference, and throws PHP-style TypeErrors
  for invalid resource/object operands.
