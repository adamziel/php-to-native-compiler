...
- `str_replace()` supports scalar byte replacement plus bounded array
  search/replacement/subject forms over current string-convertible values,
  preserves array subject keys, writes the optional `$count` argument by
  reference, and throws PHP-style TypeErrors for invalid top-level operands.
