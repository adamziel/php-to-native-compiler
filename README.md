...
- `str_replace()` supports scalar and ordered-array byte replacement, writes
  the optional `$count` argument by reference, preserves array-subject keys,
  and throws PHP-style TypeErrors for invalid resource/object operands; deeper
  object/reference element parity remains bounded.
