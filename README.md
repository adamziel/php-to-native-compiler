...
- `str_replace()` supports scalar byte replacement and bounded ordered-array
  search/replacement/subject forms with scalar-convertible entries, writes the
  optional `$count` argument by reference, and throws PHP-style TypeErrors for
  invalid resource/object operands.
