...
- narrow `require`, `require_once`, and `include` statement execution for local
  string paths, including constant/string-concatenated paths resolved relative
  to the current source file, included files executing in caller scope, and
...
the current narrow local `require`/`require_once`/`include` statement slice,
eval,
