...
- `str_pad()` supports byte-length padding with pad constants; `strrev()`
  ...
  suffix stripping; `dirname()` handles repeated levels; `pathinfo()` returns
  binary-safe dirname/basename/extension/filename components with `PATHINFO_*`
  flags; `crc32()` computes length-aware CRC-32 integers; trim-family
  internals use PHP default bytes plus bounded charlists.
