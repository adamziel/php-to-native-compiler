...
- Modeled metadata includes `phpversion()`, `php_sapi_name()`,
  ...
  `get_loaded_extensions()`, locale constants, bounded `setlocale()`
  current/C/POSIX queries and sets, and bounded class/property existence
  checks.
...
- `str_replace()` supports scalar plus ordered-array search/replacement/subject
  forms, writes the optional `$count` argument by reference, emits
  array-to-string warnings for nested array operands, and throws PHP-style
  TypeErrors for invalid resource/object top-level operands.
