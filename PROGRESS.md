...
Refresh: 2026-06-11T14:09Z
Measured: `ptn-fo4z` rebased after current `origin/master`; array
...
literals, and COW evidence are integrated. Exact `array_fill_basic.phpt` now
passes; heredoc interpolation remains unsupported.
...
| Native compiled PHP snippets | 404 | 404 | 0 |
...
expression-form `print`, `@`, file predicates, selected array/string internals
including `array_fill()`, `array_flip()`, `array_change_key_case()`,
`array_count_values()`, `str_repeat()`, and integer `range()`, lvalue-reference
calls, list/append assignment, scalar variable variables, method callables,
...
include/include_once behavior, broader file APIs, and `chr()` float-to-int
...
`array_change_key_case.phpt`, `alternative_offset_syntax_in_encaps_string.phpt`,
`chr_out_of_range.phpt`, `ord_basic.phpt`, declared-function continue/switch
rows, and `print_r_ints.phpt`; `cargo fmt --check`; `cargo build --bin phpc`;
