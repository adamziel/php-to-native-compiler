...
Latest accounted source capability: `8d5f3715` adds
...
metadata as known scoped callable-string signatures or runtime fallback. Previous
source commit `53bef000` adds `NativeSourceCallBindingOperands`, composing
method/static targets, pre-invocation cleanup, signature-driven by-reference
argument binding, and the exactly-once `NativeCallArgumentsHandle` emitter.

Why the headline bars did not jump: the new source hardens method/static
signature fallback and binding selection, but executable method/static
production still needs receiver ownership across lookup/preinvoke/invoke
outcomes, caller-scope policy, runtime/builtin/inherited/trait/interface
metadata, namespace/autoload and visibility/magic contracts, return summaries,
alias transfer, named/spread metadata, and diagnostic-result operand migration.
Executable return/throw/exit cleanup still needs terminal-kind contracts and
exact finally/destructor/shutdown ordering.
