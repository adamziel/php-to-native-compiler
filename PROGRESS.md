...
Refresh: 2026-06-11T21:29Z
Measured: `ptn-qm7v` rebased on `origin/master` at `8addafc6a`.
...
`ext/standard/tests/array/001.phpt`. `ptn-qm7v` adds bounded
`highlight_string()` return-output behavior, empty `ob_get_contents()`, and
`phpc -d error_reporting=N` ini wiring, clearing
...
| Native/compiler Rust suite | 473 | 473 | 0 |
...
mutation, modeled `pow()`, plain `array_merge()`, CLI `error_reporting` ini
wiring, bounded `highlight_string()`/empty output-buffer reads, and direct
array mutators including `array_pop`, `array_push`, `array_shift`,
`array_unshift`, `ksort`, and `shuffle`.
...
- `ptn-lrty.4`, `ptn-loyg`, and `ptn-qm7v`: 3 string/output rows remain. `004`
  now reaches array-element inc/dec after sort/shuffle support; `005` is
  covered; `006` and `strlen` diagnostic-order/source-path parity remain.
...
exact `ext/standard/tests/array/001.phpt` passes for `ptn-k95f`; exact
`ext/standard/tests/strings/005.phpt` passes for `ptn-qm7v`. `ptn-lrty.3`
...
`ptn-qm7v` bounded PHPT `manifest-20260611T202811Z.txt` is 186/200 on its
pre-merge base. COW PHPT `summary-20260611T202912Z.txt`,
`summary-20260611T203045Z.txt`, and `summary-20260611T203453Z.txt` are 29/29;
`ptn-k95f` COW PHPT `run-20260611T204003Z.log` and `ptn-qm7v` COW PHPT
`manifest-20260611T203729Z.txt` are 29/29; callback PHPT
`summary-20260611T203116Z.txt` is 2/2. Focused `ptn-qm7v` coverage includes
`compile_highlight_string_and_empty_output_buffer_to_native_binary` and
`phpc_error_reporting_ini_sets_initial_level`.
