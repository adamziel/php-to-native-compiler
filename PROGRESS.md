...
Refresh: 2026-06-10T15:27Z
Measured: `ptn-yjo` rebased on `origin/master@01da7022`; `addcslashes()` and
`stripcslashes()` support plus current `array_reduce()` and non-array
`foreach` diagnostic evidence.
...
binary strings, double-quoted octal/hex escapes, string offsets, scalar
...
`Class::method` callables, `addcslashes()`/`stripcslashes()`, generic
`array_reduce()` callback dispatch with accumulator debug refcounts, and `new
stdClass` boxed objects with public dynamic property reads/writes shared
through object aliases.
...
native/parser/lexer tests; exact `add-and-stripcslashes.phpt`.
