...
Compiler/backend consumers               [##########----------] 49%
Executable generalized PHP semantics     [####----------------] 20%
...
- Latest pushed primary commit observed before this update: `dea6899d native: route strlen through string conversion`.
- Latest integrated semantic compiler/runtime commit: `dea6899d native: route strlen through string conversion`.
...
- Latest integrated read: generated-C `strlen()` for lowerable null/bool/int/float/string operands now consumes the runtime native value-to-string conversion result and byte length, including embedded-NUL strings, instead of relying only on static string-length folding. Runtime arithmetic-number conversion sharing from `633da5fc` remains integrated.
