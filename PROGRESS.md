...
Measured latest: `ptn-w17z.28` collected Generator runtime row pack selected
6 rows, ran all 6, and passed 6/6.
Latest: `ptn-w17z.28` added generic `yield` parsing/lowering and bounded
collected `Generator` runtime objects for eager generator-body execution,
`foreach`, `Generator::current()`, metadata checks, by-reference yield
diagnostics, and call-site unpacking from generator values. Previous:
`ptn-w17z.16.3` completed the checked-in exception formatting row pack at
12/12 runnable.

Current hook: `ptn-w17z.28` focused row pack passed at
`.runtime/ptn-w17z28-generator-row-pack-final-head2/run-20260615T160837Z-manifest.log`.
The hook-start broad frontier baseline on
`tools/phpt-cow-broad-frontier-manifest.txt` was 46 selected, 34 runnable,
31 passed, 3 unrelated existing failures, and 12 generator/Fiber rows
classified. Final broad frontier rerun at
`.runtime/ptn-w17z28-broad-rebased/run-20260615T155020Z-manifest.log` selected
46 rows, ran 39, passed 37, retained 2 unrelated baseline failures, and
classified 7 remaining generator/Fiber rows for Fiber, `yield from`, true
suspension cleanup, and reference-timing edges. Previous hook:
`ptn-w17z.16.3` exception formatting row pack passed 12/12 runnable at
`.runtime/merge-ptn-w17z16-3-exception-format/summary-20260615T160318Z.txt`.

## 2026-06-15 ptn-w17z.28 Collected Generator Runtime Row Pack

Final manifest: `tools/phpt-ptn-w17z28-generator-runtime-row-pack.txt`.

Implemented behavior: `yield` tokens parse through AST/IR and mark yielding
functions as generator functions. Generator calls now create native
`Generator` objects backed by collected yielded values, with support for
`foreach`, `Generator::current()`, `class_exists()`/`method_exists()` and
`Generator`/`Iterator`/`Traversable` type checks. By-reference generator yields
preserve reference targets for supported lvalues, emit PHP notices for
non-reference sources, reject by-reference iteration of non-reference
generators, and allow call-site unpacking of generator values including the
bounded Traversable by-reference warning path.

Focused evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z28-generator-row-pack-final-head2 timeout 600 tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z28-generator-runtime-row-pack.txt`.
Artifact `.runtime/ptn-w17z28-generator-row-pack-final-head2/run-20260615T160837Z-manifest.log`
selected 6 rows, ran 6, and passed 6 with 0 failures or exclusions.

Broad frontier evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z28-broad-rebased timeout 900 tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-cow-broad-frontier-manifest.txt`.
Artifact `.runtime/ptn-w17z28-broad-rebased/run-20260615T155020Z-manifest.log`
selected 46 rows, ran 39, passed 37, failed 2 unrelated baseline rows, and
classified 7 remaining generator/Fiber rows.

Residual generator/Fiber blockers remain intentionally classified: true lazy
generator suspension, `yield from`, `send()`/`throw()`/`next()`/`getReturn()`
timing, Fiber runtime execution, generator body foreach cleanup/premature
close, and by-reference yielded assignment-expression timing.
