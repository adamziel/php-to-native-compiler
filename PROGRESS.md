...
Updated: 2026-05-22 12:56 CEST
Evaluation marker: `20260522T105253Z` plus post-evaluator semantic refresh through `f3c0f574`
Primary HEAD: current progress-only update on top of `f3c0f574`
Current primary semantic baseline: `f3c0f574 codegen: route array sort builtins through lvalue ABI`
...
Momentum is positive, but still uneven. The primary branch recently landed real request/symbol runtime storage, generated-C array/lvalue consumers, request missing-key diagnostics, runtime request/superglobal path mutation, a narrow generated-C shell-escape consumer over the shared string-result ABI, and comparator-free array sort-family routing over the array-lvalue ABI. Ordinary broad PHP program support still has major blockers in `$GLOBALS`, compiler-lowered superglobals, references/COW, calls, objects, control flow, exact diagnostics, and broad composition.
...
| Executable generalized PHP semantics | 57% | `[###########---------]` | Selected scalar, string, array, lvalue, symbol-runtime, and request-runtime behavior works. Broad PHP programs still hit structural blockers. |
| Arrays, references, COW, lvalues | 67% | `[#############-------]` | Selected generated-C array/lvalue execution works, including pointer/cursor builtins, direct value-offset writes, and comparator-free sort-family mutation over tracked owners. Arbitrary roots, full references/COW, by-reference foreach, and ArrayAccess/resource offsets remain open. |
...
| Broad integrated verification | 68% | `[##############------]` | Focused gates and linked tests are useful. Broad differential composition coverage remains thin. |
...
- `master` has semantic progress through `f3c0f574`; this dashboard update is progress-only.
- Array sort-family owner/path mutation ABI and generated-C consumer are integrated in `compiler/src/codegen.rs`, `compiler/tests/native_link.rs`, and `runtime/src/lib.rs`.
...
- `impl-native-integration-batch`: array sort-family owner/path mutation ABI and generated-C consumer has been sliced into primary; broader lane work remains excluded.
...
| --- | ---: | --- | --- |
| Array/lvalue execution | 67% | In progress | Prefer arbitrary-root writes, by-reference foreach, `??=`/RMW, owner/value/reference-slot materialization, LLVM parity, or deeper cleanup/diagnostic ordering over another sort/builtin breadth slice. |
...
The next best primary slices should turn landed runtime/storage surfaces into executable compiler behavior, especially superglobal reads/writes/`isset()`/`empty()`, `$GLOBALS` aliasing, request lifetime threading, or reference/COW materialization. Sort-family routing is counted as a real but narrow generated-C array-lvalue consumer, not a reason to keep prioritizing builtin breadth over deeper symbol/request/reference/call/control-flow work. Whole-lane merges, fixture-shaped production lowering, generated-source substring-only progress, formatter spillover, and docs-only churn outside this progress dashboard remain rejected.
