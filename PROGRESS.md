...
Updated: 2026-05-21 14:19 CEST  
Primary state: `master` clean and pushed at `537a084b native: route unsupported direct-call arguments`  
Progress estimate: **22% toward broad, generalized native PHP semantics**
...
The project is moving in the right direction: recent work is replacing one-off backend rejections with shared semantic-family contracts for calls, comparison results, symbol tables, diagnostics, arrays, object properties, references, and ownership cleanup. The most important recent primary win is that comparison branches now consume a real runtime ABI in executable native C.
...
```
Overall generalized PHP semantics       [####----------------] 22%
Runtime value/ABI foundation            [########------------] 42%
Compiler/backend ABI consumption        [#####---------------] 27%
Function calls and frames               [###-----------------] 17%
...
- [~] **Call semantics** `[#####---------------] 25%`
  - Done: shared call-operation blockers, call-result lvalue routing, argument-result cleanup preflight across several direct-call families.
