...
Refresh: 2026-06-10T16:02Z
Measured: `ptn-8wy` on `origin/master@8a659986f`; callback closures,
static/string callables, array_reduce COW, directory APIs, `stdClass`,
non-array `foreach` diagnostics, and
`array_count_values()` plus focused `bin2hex()` PHPT evidence.
...
`tools/phpt-cow-manifest.txt`: 28/29 passing; remaining COW PHPT failure:
`bug69068_2.phpt`.

## Already Ported

Lexer/parser, AST/IR/C backend, boxed values, variables/constants, ordered
arrays, `foreach`, source-spanned non-array `foreach` warnings, cursors,
numeric keys, payload refcounts, array/string COW, references, by-reference
params/foreach, array dimensions, temporaries, recursive/user functions,
anonymous function values for direct dynamic calls and internal callbacks,
magic constants, `func_*`, `print_r`, binary strings, string offsets, scalar
diagnostics, array literal references, array union `+`, scalar type hints,
by-reference return boundaries, `count()`, `??`, assignments, expression-level
`@`, file APIs including recursive `mkdir()` plus directory predicates,
...
entries skipped after modeled warnings, and double-quoted hex/octal byte
escapes that preserve binary bytes through `bin2hex()`.
...
fatal parity, Unicode string escapes, and broader file APIs.

## Verification

Commands: `cargo fmt --check`; `cargo build --bin phpc`; `cargo test`;
focused non-array `foreach` native tests; exact
`array_reduce_accumulator_refcount.phpt`; `bin2hex*.phpt` rows;
