...
Latest accounted source capability: `d67c7f14` adds reusable receiver-method and
static-method source-call target operands over access-context lookup-plus-invoke
helpers, with separate cleanup lanes for pre-invocation failure and
post-invocation auxiliary cleanup. Previous source commit `4d6f0f1e` validates
cleanup-frame enqueue producers and rejects terminal-source value ownership
before LLVM/generated C cleanup-frame emission.

Why the headline bars did not jump: the new source hardens method/static
source-call target ownership, but executable method/static production still
needs receiver evaluation order, delayed by-reference/named/spread argument
binding, visibility/magic/class-context policy, and result-consumer integration.
Constructors, closure invocation, by-reference alias transfer, spread
ownership, and cleanup operands from real `finally`, destructor, shutdown,
unwind, or terminal control-flow frames also still need generalized contracts.
...
| Calls, functions, frames | **85%** | `[#################---]` | Runtime callable table/value dispatch, call arguments/frame/result ABI, conditional handoff, generated-C direct/dynamic callable consumers, declared-method registration/wrapper frames, callable return facts, by-reference argument transport, descriptor closures, closure returns, request-state frame handoff, access-context lookup ABI, lookup-plus-invoke exactly-once argument ownership helpers, source-call result carrier selectors, selected production source-call carrier emission, direct generated user-function lookup-plus-invoke production, and method/static source-call target operands are integrated. Unknown runtime callables, executable method/static source-call production, builtin return summaries, executable by-reference alias transfer, full object/method parity, namespace fallback, autoload, magic calls, named/spread breadth, broader return references, constructors, cleanup/unwind, and backend parity remain open. |
...
| Control flow, cleanup, diagnostics | **65%** | `[#############-------]` | Selected branches, loops, transfers, finalizers, output buffers, diagnostic blockers, owned diagnostic-result list contracts, consumer contracts, backend family consumers, deferred-cleanup blockers, control-transfer cleanup result consumers, cleanup-frame producers/source metadata/report bridges, cleanup-frame stack aggregation, cleanup-frame enqueue validation, try-body call-boundary preflight, report sinks, continuation helpers, discarded statement-expression operands, and echo/print output operands exist. Broad unwind/finally/destructor/shutdown execution, cleanup result production from real control flow, executable reference binding, remaining semantic diagnostic-result producer migration, and source-ordered diagnostics remain open. |
...
| Callable access context and class metadata | **100%** `[####################]` | **100%** `[####################]` | **60%** `[############--------]` | Shared runtime access-context policy, lookup-plus-invoke argument ownership, source-call result carrier selectors, selected production source-call carrier emission, direct generated user-function lookup-plus-invoke production, method/static source-call target operands, and allocatable-class metadata are integrated for function/method/static/constructor lookup preflights. Executable method/static source-call production, constructor execution, closure argument-handle ownership, namespace fallback, autoload, magic, and full visibility parity remain open. |
...
| Cleanup/unwind execution | **30%** `[######--------------]` | **30%** `[######--------------]` | **30%** `[######--------------]` | Requirement/preflight boundaries, cleanup result consumers, cleanup report bridges, cleanup-frame producer queues, cleanup-frame source metadata, nested cleanup-frame stack aggregation, and cleanup-frame enqueue validation are integrated. Actual exception propagation, catch/finally/destructor/shutdown execution, production cleanup operand enqueueing from real control flow, terminal value transfer, and object lifetime cleanup are still not implemented. |
...
- Broader source-call production lowering over expression-owned
  ...
  over the new target operands, constructor allocation, closure
  argument-handle invocation ownership, by-reference alias transfer, and
  spread ownership.
- Remaining semantic diagnostic-result operand migration for terminal, cleanup
  ...
  and direct assembly.
- Pending diagnostic production from real control-flow cleanup, terminal value
  transfer through cleanup frames, and exact `finally`/destructor/shutdown
  sequencing.

## Latest Focused Verification

For `d67c7f14`, `4d6f0f1e`, `8ebeae19`, `964e3e2b`, `0430efcc`, `75f20f3f`,
`d26c64f7`, `50d19f99`, and `7891fcf3`:
