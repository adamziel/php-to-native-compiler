...
Measured: `ptn-0i2` on `origin/master@35eb930d`; focused
`tests/lang/array_shortcut_001.phpt` and `array_shortcut_002.phpt` pass, so
they expose no current short-array or `print_r()` output delta. Broader
dashboard remains from `ptn-wk3`.
...
Commands: `cargo build --bin phpc`; exact PHPT rows
`tests/lang/array_shortcut_001.phpt` and `array_shortcut_002.phpt`. Latest full
gate baseline remains `cargo fmt --check`; `cargo build --bin phpc`;
