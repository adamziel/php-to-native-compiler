...
`explode()`, `strncmp()`, `strpos()`, `strrchr()`, namespaces/imports, includes/once
...
integer diagnostics, `strncmp()`/`strpos()`/`strrchr()`, `basename()`,
...
| PHPT bounded manifest | 244 | 242 | 2 |
| PHPT Zend rows | 82 | 82 | 0 |
| PHPT ext/standard rows | 112 | 112 | 0 |
...
`str_replace()`, `strcasecmp()`, `strncmp()`, `strpos()`, `strrchr()`,
`pathinfo()`, `crc32()`, `basename()`, `chr()` diagnostics, `var_export()`,
array mutators, inc/dec, foreach destructuring, dynamic-variable writes/unsets, and
array/string-offset compound/null coalescing assignments.

## Remaining Bounded Failures

- None among the 242 runnable rows in the current 244-row bounded manifest.
  Two selected rows are classified out for unsupported ini requirements.

## Verification

Current slice verification after rebasing on `60d3576f3`: `git diff --check`;
`cargo fmt --check`; focused native `pathinfo()`/`strpos()` tests; focused PHPT
`pathinfo`/`strpos` rows; full `cargo test` 583/583 plus auxiliary COW/doc
tests; bounded PHPT 244 selected/242 runnable/2 excluded, 242/242 passed; PHPT
COW 29/29; and post-merge COW gate 17/17 oracle, 3/3 notice, 6/6 diagnostics.
