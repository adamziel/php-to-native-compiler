...
Updated: 2026-05-24 17:50 CEST
Evaluation marker: `20260524T1550Z`

Latest counted primary semantic/test baseline:
`8f5d8fb3 parser: admit static arrow descriptor closures`

Latest primary head before this progress update:
`8f5d8fb3 parser: admit static arrow descriptor closures`

Latest observed `origin/master` during this review:
`1780ca87 docs: update progress after static closures`
...
| Executable PHP semantics | **83%** | `[#################---]` | Many focused linked/runtime programs run, including supported static anonymous and static arrow descriptor closures, typed/default/variadic by-value descriptor closure parameters, supported root/reference and frame-local by-reference captures, by-reference descriptor closure parameters, arrow implicit capture execution, and callable-array public method-frame invocation, but execution remains selected islands rather than general PHP. |
...
| Calls, functions, frames | **80%** | `[################----]` | Bounded functions, descriptor-backed by-value/static anonymous/static arrow closures, supported by-reference closure captures including promoted frame locals, non-static arrow implicit captures, typed/default/variadic by-value closure parameters, untyped by-reference closure parameters, callable-array public method frames, and public object/method/constructor call paths work in selected generated-C cases. |
...
- Primary semantic work is committed at `8f5d8fb3`; this progress update is the separate documentation wrapper before push.
- Focused gates for the latest static arrow descriptor-closure slice passed using disk-backed `/tmp/phpc-primary-static-arrows`: `cargo check -q -p phpc -p php_runtime`, `cargo test -q -p phpc --test syntax_boundaries static_arrow`, `cargo test -q -p phpc --test native_link static_arrow`, `cargo test -q -p phpc --test native_link static_descriptor_closure`, `cargo test -q -p phpc --test native_link static_closure_this_binding`, `cargo test -q -p phpc --test native_link descriptor_closure`, `cargo test -q -p phpc --test native_function_call_boundary native_executable_c_source_routes_call_operation_blockers_across_call_families`, `cargo test -q -p phpc --test native_link callable_array`, `cargo test -q -p php_runtime --lib -- --test-threads=1` with 284 passed, scoped `rustfmt --edition 2021 --check compiler/src/parser.rs compiler/src/codegen.rs compiler/tests/native_link.rs compiler/tests/native_function_call_boundary.rs compiler/tests/syntax_boundaries.rs`, `git diff --check`, and `git diff --cached --check`.
