...
Updated: 2026-05-21 23:24 CEST
Evaluation marker: 20260521T212433Z
...
- Product HEAD at this review: `3207fc79 docs: correct progress timestamp`, synced with `origin/master`.
- Latest committed semantic baseline: `325c9c7d codegen: centralize native call diagnostics`.
- Live worktree status: dirty under active `primary-integrator` ownership in `runtime/src/lib.rs`, `compiler/src/codegen.rs`, `compiler/tests/native_cast_boundary.rs`, `compiler/tests/native_link.rs`, `compiler/tests/native_runtime_abi.rs`, and `compiler/tests/scalar_casts.rs`. This appears to be a generalized scalar-cast builtin/value-cast diagnostics slice and is **not counted as integrated** until gated, committed, and pushed.
...
- [x] Generated-C consumers for selected string builtins, array key/value operations, array append diagnostics, string-handle comparison operands/results/decisions, strict array/object identity in selected array-search builtins, array-handle comparisons, comparison status guards, cast/type-name echo value results, and centralized call diagnostics.
...
| --- | ---: | ---: | --- |
| String conversion, truthiness, byte buffers | 49% | 78% | Primary has shared value string-form semantics, numeric-string classification, selected generated-C string builtin consumers, cast/type-name echo value-result consumers, and comparison byte materialization. Current dirty primary work may add scalar-cast builtin consumers, but it is not integrated yet. |
...
| Comparison and conversion semantics | 59% | 72% | Primary has reusable comparison operation validation, branch/free/decision/status ABIs, generated-C status guards, string-handle comparison operands, shared numeric-string pair classification, materialized value comparison entry points, array-handle comparison consumers, compare/cast/type-name value-result consumers, and operand-side/value-family blockers. Loose array/object/resource/reference execution, warning order, arbitrary expression materialization, and broader LLVM/generated-C parity remain open. |
...
| Broad composition verification | 22% | 39% | Focused runtime/native-link gates cover comparison decision/status paths, value-result consumers, array-handle comparisons, array append diagnostics, call diagnostics, and selected full `native_link` batches. Broad differential PHP composition coverage remains thin, and broad `phpc --tests` still has known pre-existing gaps. |
...
- Active primary dirty candidate: scalar-cast builtins/value-cast diagnostics across runtime, codegen, native-link, ABI, and scalar-cast tests. Count only after primary-integrator commits and pushes.
...
1. Treat `325c9c7d` as the latest integrated semantic baseline until the active scalar-cast slice is gated, committed, and pushed.
2. Prefer small executable generated-C/LLVM consumers of existing ABI surfaces over more standalone vocabulary.
3. Strong next candidates: finish the active scalar-cast builtin/value-cast slice if it gates cleanly; isolate one narrow array-lvalue consumer from `impl-array-linked-exec`; add LLVM parity for comparison or array append diagnostic boundaries; or connect diagnostic/result producers to real generated-C reads/offsets.
