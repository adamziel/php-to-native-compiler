...
Refresh: 2026-06-10T15:09Z
Measured: `ptn-9pt` rebased on `origin/master@d2d51779`; `array_column()`
numeric-string key native/PHPT evidence plus current `array_reduce()`
accumulator ownership and non-array `foreach` diagnostics.
...
`array_column()` numeric-string column keys,
