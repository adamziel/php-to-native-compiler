...
Updated: 2026-05-22 00:18 CEST
Evaluation marker: 20260521T221248Z-plus-119c8d39-sem-b214ccc5
...
- Product HEAD at this review: `119c8d39 docs: update progress after string byte source`, synced with `origin/master` before this evaluator update.
- Latest committed semantic baseline: `b214ccc5 runtime: share PHP string byte source materialization`.
- Live worktree status before this evaluator update: clean and synced after the string-byte source progress commit.
- Resource note: `/dev/shm` is tight at about 6.8G free out of 22G, just above the 6G dispatcher floor. `/home` has about 249G free. The largest observed shared-memory target is `/dev/shm/phpc-target-native-exit-seed` at about 8.4G. Broad primary gates should remain isolated, single-job, and resource-aware.

## Grand Roadmap Position

The compiler is steadily replacing backend-local decisions and local byte/value handling with reusable runtime/ABI contracts and selected executable consumers. Recent primary progress is strongest in comparison routing, string-handle comparison operands, nested comparison decision rematerialization, numeric-string classifier sharing, value-result materialization, scalar value-cast ABI consumers, array-handle value operands, selected filesystem/cache ABI routing, call-boundary diagnostics, and shared runtime string-byte source materialization.
...
| --- | ---: | ---: | --- |
| String conversion, truthiness, byte buffers | 52% | 80% | Primary has shared value string-form semantics, numeric-string classification, selected generated-C string builtin consumers, cast/type-name echo value-result consumers, scalar value-cast generated-C consumers, comparison byte materialization, and now a shared runtime string-byte source/materialization boundary consumed by native strings, diagnostics, array keys, array-column keys, and PHP string values. |
...
| Broad composition verification | 27% | 40% | Focused runtime/native-link gates cover comparison decision/status paths, nested comparison decision operand composition, value-result consumers, scalar value-cast consumers, array-handle value operands/comparisons, array append diagnostics, realpath-cache filesystem blockers, call diagnostics, string-byte source materialization, and selected full `native_link` batches. Broad differential PHP composition coverage remains thin, and broad `phpc --tests` still has known pre-existing gaps. |
