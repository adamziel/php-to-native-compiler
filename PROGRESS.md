...
Measured addendum: `ptn-kfip` memory-limit/quantity row pack moved 11 broad tier-1000 rows from classified plus 2 broad tier-1000 failed rows into 13/13 passing, with broad tier-1000 classify-only 592 runnable / 408 classified -> 606 runnable / 394 classified.
...
## 2026-06-15 ptn-kfip Memory-Limit INI and Quantity Rows

Implemented behavior: `phpc -d` now propagates `memory_limit` and
`max_memory_limit`, runtime `ini_get()` reports both values, `ini_set()` mutates
`memory_limit` while keeping `max_memory_limit` readonly, and `memory_limit`
clamps to `max_memory_limit` at startup and runtime with PHP-compatible
warnings. The runtime also implements `ini_parse_quantity()` for decimal/hex
quantities and `k/m/g` multipliers, and `phpc -d error_reporting=...` now
accepts PHP error-reporting constant expressions such as `E_ALL ^ E_WARNING`.

The current deterministic tier-1000 broad manifest contains no
`ext/standard/tests/strings` rows, so the final pass evidence targets
broad-present core/basic memory-limit and quantity rows.

Broad tier-1000 classify-only evidence:

- Before:
  `.runtime/ptn-kfip-broad-before-progress/summary-20260615T024643Z.txt`
  selected 1,000 rows: 592 runnable, 408 classified.
- After:
  `.runtime/ptn-kfip-broad-final-classify-progress/summary-20260615T033327Z.txt`
  selected 1,000 rows: 606 runnable, 394 classified.

Focused before evidence:
`.runtime/ptn-kfip-core-config-before/summary-20260615T030745Z.txt`
selected 19 broad-present core/basic rows: 6 runnable, 13 classified, 2 passed,
and 4 failed. The 11 `gh17951_*` memory-limit rows were classified as
`unsupported-resource-limit-ini`; `ini_parse_quantity_basic.phpt` and
`ini_parse_quantity_warnings.phpt` were runnable failures.

Final focused command:
`PHPT_PROGRESS_DIR=.runtime/ptn-kfip-memory-quantity-final-rebased timeout 900 tools/run-bounded-phpt.sh --classify-harness-programs .runtime/ptn-kfip-memory-quantity-focused.txt`.
Artifact `.runtime/ptn-kfip-memory-quantity-final-rebased/summary-20260615T034806Z.txt`
selected 13 rows: 13 runnable, 13 passed, 0 failed, 0 classified.

Newly passing broad-present rows:
