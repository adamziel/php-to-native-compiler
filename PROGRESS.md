...
Measured latest: `ptn-w17z.25` serialize/unserialize reference identity row
pack moved the checked-in focused manifest from 6 selected / 6 runnable / 0
passed / 6 failed to 6/6 passing.
Latest: `ptn-w17z.25` added generic serialize reference numbering for value
slots, PHP references, and object identity, plus an `unserialize()` parser for
scalar, array, object, `R:`, and `r:` records that lazily materializes shared
reference slots. Previous: `ptn-w17z.12` completed the checked-in magic
methods/lifecycle row pack.

Current hook: `ptn-w17z.25` serialize/unserialize reference identity row pack
selected 6 full-corpus `ext/standard/tests/serialize` rows and passed all 6 at
`.runtime/ptn-w17z25-after-rebase/summary-20260615T145920Z.txt`.
Previous hook: `ptn-w17z.12` magic methods/lifecycle row pack passed 49/49 at
`.runtime/merge-ptn-w17z12-magic-lifecycle/summary-20260615T144358Z.txt`;
`ptn-w17z.10` kept 185/185 SKIPIF rows runnable at
...
Final manifest:
`tools/phpt-ptn-w17z25-serialize-reference-identity-row-pack.txt`.

Implemented behavior: `serialize()` now uses a generic value-ID state that
counts serialized value slots, emits `R:<id>` for repeated PHP reference
containers, emits `r:<id>` for repeated object identities, and permits
reference-mediated array recursion to expand once before repeated references
resolve through `R:`. `unserialize()` is registered as a modeled internal and
parses `N`, `b`, `i`, `d`, `s`, `a`, `O`, `R`, and `r` records, rebuilding
arrays and declared-object shells while lazily converting target slots into
shared `PtnReference` cells only when an `R:` record requires identity.

Focused evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z25-before timeout 1200s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z25-serialize-reference-identity-row-pack.txt`
selected 6 rows, all 6 runnable, and passed 0 with 6 failures.

Final focused evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z25-after-rebase timeout 1200s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z25-serialize-reference-identity-row-pack.txt`.
Artifact `.runtime/ptn-w17z25-after-rebase/summary-20260615T145920Z.txt`
selected 6 rows, all 6 runnable, and passed 6.
