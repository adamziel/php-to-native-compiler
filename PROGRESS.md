...
Refresh: 2026-06-13T10:37Z
Measured: `ptn-zfsw` rebased on current `origin/master` `527a6eb99`;
verification green with post-rebase focused rerun.
...
internals, `str_replace()` scalar/count and array operand slices,
...
exceptions, tests/lang operator rows, `crc32()`, `str_replace()` counts and
array operands, and integer validation for `chr()`, `intdiv()`, and file
offsets.
...
`explode()`, `str_replace()` scalar/count and ordered-array operand forms,
...
destructuring, dynamic-variable writes/unsets, catchable operator arithmetic
exceptions, alternate not-equal parsing, keyword boolean tails, and
...
Current branch verification for `ptn-zfsw`: post-rebase diff check,
post-rebase `cargo fmt --check`, post-rebase focused `str_replace` native
tests 2/2, `cargo test` native/compiler 589/589 plus ancillary suites, native
smoke matrix 6/6, bounded PHPT 268/268 with 2 classified exclusions, PHPT COW
...
embedded-NUL internals, object IDs, host-locale parity, remaining
`str_replace()` edge parity, and object/reference targets.
