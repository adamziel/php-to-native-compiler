...
Measured latest: `ptn-w17z.33` extract globals refs row pack selected 3 rows,
kept 3 runnable, and passed 3/3 in the focused run.
Latest: `ptn-w17z.33` completed the checked-in extract globals refs row pack.
Previous: `ptn-w17z.16` completed the checked-in exception/backtrace/lifecycle
row pack at 41/41.

Current hook: `ptn-w17z.33` extract globals refs row pack passed 3/3 at
`.runtime/ptn-w17z33-extract-globals-refs-rebased/summary-20260615T153503Z.txt`.
...
`tools/phpt-ptn-w17z33-extract-globals-refs.txt`.

Implemented behavior: `extract()` is modeled as a symbol-table mutating
internal with standard `EXTR_*` constants, common overwrite/skip/prefix/
if-exists modes, and `EXTR_REFS` reference extraction. Direct `extract($GLOBALS,
EXTR_REFS)` calls now bypass the ordinary `$GLOBALS` snapshot and read the real
global symbol table. Existing `global $name` bindings are preserved as real
global references, while ordinary local variables still receive extracted
values without turning later local writes into global writes. Extracting a
literal or copied array with `EXTR_REFS` converts source entries to references
after separating shared arrays, and attempts to extract the unprefixed
`GLOBALS` target remain count-only/no-op for safety.

Focused evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z33-extract-globals-refs-rebased timeout 900s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z33-extract-globals-refs.txt`.
Artifact
`.runtime/ptn-w17z33-extract-globals-refs-rebased/summary-20260615T153503Z.txt`
selected 3 rows, kept 3 runnable, and passed 3.

Verification:

- `cargo fmt --check`
- `cargo test compile_extract_refs_and_globals_to_native_binary --test compile_native -- --nocapture`
