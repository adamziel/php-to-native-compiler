...
`tools/phpt-ptn-w17z34-output-buffer-compact.txt`.

Implemented behavior: output writes from `echo`, `printf()`, `vprintf()`, and
string `exit()` route through the native output buffer stack. `ob_start()`
pushes optional callback handlers, `ob_get_contents()`, `ob_get_length()`,
`ob_get_level()`, and `ob_list_handlers()` inspect active buffers, and
`ob_end_clean()`, `ob_end_flush()`, `ob_get_clean()`, `ob_get_flush()`, and
shutdown flush close buffers through callback dispatch. Callback output is
suppressed while the handler runs, and handler exceptions are rethrown through
the existing `try`/`catch` runtime. Direct `compact()` collects current-scope
variables from string and recursive array arguments, while dynamic
`compact()` calls, including output-handler callbacks, throw `Error`.

Focused evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z34-output-buffer-compact-rebased timeout 600s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z34-output-buffer-compact.txt`.
Artifact `.runtime/ptn-w17z34-output-buffer-compact-rebased/summary-20260615T154737Z.txt`
selected 1 row, kept 1 runnable, and passed 1 with no failures or exclusions.

Additional verification:

- `cargo fmt --check`
- `cargo test output_buffer --test compile_native -- --nocapture`
- `cargo test compile_ob_start_compact_handler_rejects_dynamic_compact_to_native_binary --test compile_native -- --nocapture`
- `cargo test compile_scalar_echo_keeps_direct_output_path_to_native_binary --test compile_native -- --nocapture`
