...
Refresh: 2026-06-10T14:04Z
Measured: `ptn-y9k` rebased on `origin/master@be9cc35`; scalar offset lvalue
false-promotion and non-array fatal boundary evidence.
...
object/property compound lvalues, broader scalar offset-lvalue fatal parity,
and broader file APIs.

## Verification

Commands: focused scalar/string/`??=` native tests; `cargo fmt --check`;
`cargo build --bin phpc`; focused invalid-container PHPT row blocked by complex
interpolation; prior `cargo test`; `tools/run-native-smoke-matrix.sh`;
