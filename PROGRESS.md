...
| ptn-w17z.16 exception formatting row pack | 12 | 12 |
| ptn-kia6 by-ref/reference-boundary row pack | 107 | 75 |

Refresh: 2026-06-15T15:37Z.
...
Measured latest: `ptn-w17z.16.3` exception formatting row pack selected 13
rows and moved 0/10 runnable passed with 3 classified -> 12/12 runnable passed
with 1 ReflectionProperty classified under `--classify-harness-programs`.
Latest: `ptn-w17z.16.3` added binary-safe exception message/string formatting,
`getTraceAsString()` stack formatting, `zend.exception_string_param_max_len`,
and built-in `ErrorException` subclass constructor diagnostics. Previous:
`ptn-w17z.16` completed the checked-in exception/backtrace/lifecycle row pack.

Current hook: `ptn-w17z.16.3` exception formatting row pack selected 13 rows,
ran 12 after classification, and passed 12/12 with only
`Zend/tests/exceptions/bug63762.phpt` still classified for ReflectionProperty
metadata/mutation support at
`.runtime/ptn-w17z16-3-exception-format-after-rebase/summary-20260615T153535Z.txt`.
Previous hook: `ptn-w17z.16` exception/backtrace/lifecycle row pack passed
41/41 at
`.runtime/merge-ptn-w17z16-exception-lifecycle/summary-20260615T151015Z.txt`.
...
## 2026-06-15 ptn-w17z.16 Exception Formatting Row Pack

Final checked-in focused manifest:
`tools/phpt-ptn-w17z16-3-exception-format-row-pack.txt`.

Implemented behavior: exception messages now retain explicit byte lengths
through object construction, `getMessage()`, throw bridging, uncaught fatal
output, and exception string casting/echoing. `Exception::getTraceAsString()`
and exception `__toString()` now render stack frames with PHP-style function
names and argument summaries, including escaped non-printable/high-byte string
bytes, object argument names, and `zend.exception_string_param_max_len`
truncation. The modeled INI key is accepted through `phpc -d`, runtime
`ini_get()`/`ini_set()`/`ini_restore()`, and the PHPT classifier. Built-in
exception metadata now includes `ErrorException`, `RuntimeException`, and
`UnhandledMatchError` in parser/runtime inheritance checks, so declared
subclasses inherit the right constructor contract and diagnostics. Exception
`var_export()` now emits `\Exception::__set_state(array(...))` with the cached
string field when present. Internal function dispatch now uses robust lookup
over the existing internal table order, fixing nested exception-trace calls to
internals such as `basename()`.

Before evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z16-3-exception-format-before timeout 1500s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z16-3-exception-format-row-pack.txt`.
Artifact
`.runtime/ptn-w17z16-3-exception-format-before/summary-20260615T144900Z.txt`
selected 13 rows: 10 runnable, 0 passed, 10 failed, and 3 classified
(`bug61362.phpt`/`bug63762.phpt` diagnostics-runtime,
`exception_024.phpt` unsupported INI).

Final after evidence:
`PHPT_PROGRESS_DIR=.runtime/ptn-w17z16-3-exception-format-after-rebase timeout 1500s tools/run-bounded-phpt.sh --classify-harness-programs tools/phpt-ptn-w17z16-3-exception-format-row-pack.txt`.
Artifact
`.runtime/ptn-w17z16-3-exception-format-after-rebase/summary-20260615T153535Z.txt`
selected 13 rows: 12 runnable, 12 passed, 0 failed, and 1 classified.

Remaining exclusion:
`Zend/tests/exceptions/bug63762.phpt` remains classified as
`unsupported-internal-reflection-metadata` because it mutates
`Exception::$trace` through `ReflectionProperty`, which still needs
ReflectionProperty metadata/mutation support.

Passing rows:

- `Zend/tests/exceptions/bug60569.phpt`
- `Zend/tests/exceptions/bug61362.phpt`
- `Zend/tests/exceptions/bug73350.phpt`
- `Zend/tests/exceptions/exception_018.phpt`
- `Zend/tests/exceptions/exception_019.phpt`
- `Zend/tests/exceptions/exception_020.phpt`
- `Zend/tests/exceptions/exception_021.phpt`
- `Zend/tests/exceptions/exception_022.phpt`
- `Zend/tests/exceptions/exception_023.phpt`
- `Zend/tests/exceptions/exception_024.phpt`
- `Zend/tests/exceptions/exception_025.phpt`
- `Zend/tests/exceptions/gh10810.phpt`

Verification:

- `cargo fmt --check`
- `cargo test --test compile_native exception -- --nocapture` passed 6/6
- `cargo test --test phpt_classifier -- --nocapture` passed 61/61
- Final PHPT row pack passed 12/12 runnable rows at
  `.runtime/ptn-w17z16-3-exception-format-after-rebase/summary-20260615T153535Z.txt`
