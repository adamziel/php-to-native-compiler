...
Refresh: 2026-06-13T13:18Z
Measured: `ptn-4qsw` integration on current `origin/master` `14d1d49`;
focused array type-hint reducers and PHPT rows passed.
...
`str_replace()`, `str_split()`, `strncasecmp()`, `chr()` diagnostics, `crc32()`,
PHP metadata constants,
...
`str_split()`, `strncasecmp()`, array parameter/return hints, and integer
validation for `chr()`/`intdiv()`/file offsets.
...
| Native/compiler Rust suite | 625 | 625 | 0 |
...
metadata constants, `strncasecmp()`, scalar/array type hints, PHPT runner ini
values, statement-form `(void)`, array mutators, inc/dec, `global`, dynamic
variables, and offset compound/null coalescing.
...
Current branch verification for `ptn-4qsw`: parser/native array type-hint
reducers 3/3 and focused PHPT
`return_separation.phpt`, `array_slice_variation11.phpt`, `bug72622.phpt` 3/3.
