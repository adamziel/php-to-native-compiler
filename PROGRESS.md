...
Refresh: 2026-06-12T09:36Z
Measured: `ptn-b6i8` rebased after `origin/master` `86e0e60e`.
...
`array_keys()`, key-boundary helpers, `array_search()`, `array_is_list()`,
catchable `intdiv()`, ASCII case string internals, `strrev()`, and PHP-style
`ceil()`/`floor()` numeric-argument diagnostics.
...
`array_search()` key lookup, `array_is_list()` list-key checks, `intdiv()`
exceptions, ASCII case conversion, key-preserving natural
`natsort()`/`natcasesort()`, binary-safe `strrev()`, and `ceil()`/`floor()`
invalid numeric-string/unsupported-operand `TypeError` parity.
...
metadata intrinsics, `is_callable()`, assertions, interpolation, streams,
`pow()`, `ceil()`, `floor()`, `array_merge()`, `strrev()`, `array_search()`,
`array_is_list()`, `call_user_func_array()`, highlight output paths,
`var_export()`, direct array mutators including `natcasesort()`, set
...
Verification: `ptn-b6i8` passed fmt, focused `array_is_list()`
native/parser coverage, current bounded PHPT 204/204, COW PHPT 29/29, and
`cargo test` with native/compiler 533/533 plus COW/doc-tests.
