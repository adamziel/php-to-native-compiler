...
Measured latest: `ptn-w17z.30` foreach mutation visibility row pack selected
11 rows, kept 7 runnable, classified 4 eval dynamic-code rows, and passed
7/7 runnable rows in the focused run.
Latest: `ptn-w17z.30` completed the checked-in foreach mutation visibility row
pack. Previous: `ptn-w17z.35` completed the checked-in compact closure this row
pack at 2/2.

Current hook: `ptn-w17z.30` foreach mutation visibility row pack passed 7/7
runnable rows at
`.runtime/ptn-w17z30-after-rebase/summary-20260615T161455Z.txt`.
...
`tools/phpt-ptn-w17z30-foreach-mutation-row-pack.txt`.

Implemented behavior: by-value foreach array snapshots keep iterating the
original bucket set when the iterated array is referenced outside the loop, and
live by-reference foreach keeps correct position when the current element is
removed or numeric keys are reindexed. Object-property foreach now retains the
iterated object, skips public/dynamic properties shadowed by a scoped private
property with the same display name, and preserves PHP-style undefined-property
warning text for current-property removals. `get_defined_vars()` now snapshots
the runtime symbol table for foreach/unset rows such as `bug39036`.
...
The four classified rows are the `eval(...)` variants of the array mutation
loops and are recorded as `unsupported-dynamic-eval`, matching the native
runtime dynamic-code boundary.
