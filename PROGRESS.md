...
Refresh: 2026-06-10T16:28Z
Measured: `ptn-og2` on `origin/master@35eb930d`; focused
`ext/standard/tests/directory/directory_constants.phpt` passes, so it exposes
no current `DIRECTORY_SEPARATOR` or `PATH_SEPARATOR` delta. Broader dashboard
remains from `ptn-wk3`.
...
Commands: `cargo build --bin phpc`; exact PHPT row
`ext/standard/tests/directory/directory_constants.phpt`. Latest full gate
baseline remains `cargo fmt --check`; `cargo build --bin phpc`; `cargo test`;
