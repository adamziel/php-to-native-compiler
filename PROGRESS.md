...
Measured latest: `ptn-vqg1` temp stream wrappers row pack selected 3 rows,
kept 3 runnable, and passed 3/3 in the focused run.
Latest: `ptn-vqg1` completed the checked-in temp stream wrappers row pack.
Previous: `ptn-w17z.35` completed the checked-in compact closure this row pack
at 2/2.

Current hook: `ptn-vqg1` temp stream wrappers row pack passed 3/3 at
`.runtime/ptn-vqg1-temp-streams-rebased/run-20260615T162130Z-manifest.log`.
...
Final checked-in focused manifest: `tools/phpt-ptn-vqg1-temp-streams.txt`.

Implemented behavior: `fopen()` now models `php://memory` and `php://temp`
wrapper streams as reusable runtime stream resources with independent byte
buffers, read/write cursor state, append-mode writes, sparse zero-fill writes,
`ftruncate()` support, `fstat()` size metadata, read-only write failure, and
PHP wrapper metadata. `php://temp/maxmemory:{n}` and
`php://temp/maxmemory={n}` record spill thresholds and preserve seek/write state
when stream size crosses the threshold.
