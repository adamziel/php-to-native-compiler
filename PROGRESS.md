...
Primary is making real integrated progress in selected native PHP execution islands. The strongest recent momentum is the generated-C object path: declared class allocation, declared child allocation with ancestor metadata, runtime string-valued declared-class `new` for constructorless classes, constructorless argument evaluation, public property read/write/`isset`/`empty`/`unset` including inherited public slots, named `instanceof` across supported ancestor chains, public instance methods with `$this`, runtime string-valued public dynamic instance methods, supported public and inherited constructors, named public static methods including inherited lookup, and public object static-receiver method calls now share runtime/frame/property, object/class relation, dynamic-call name, or declared-method frame boundaries and have focused source/link proof.
...
The latest dynamic declared-class `new` slice extends generated-C object instantiation through a runtime class-name value path for registered constructorless declared classes. Dynamic variable class names are materialized as native values, matched case-insensitively against generated declared-class candidates through the shared dynamic-name helper, and then allocated through the same declared-class or ancestor-aware declared-class allocation helpers used by named `new`. Constructorless argument lists are now evaluated and cleaned up for both named and dynamic constructorless `new`, so argument side effects compose with object allocation without adding a fixture-shaped constructor recognizer. Dynamic class names that require constructor dispatch, non-public visibility execution, method/property overrides, interfaces/traits, magic methods, contextual `self`/`parent`/`static`, callable arrays/objects/closures, exact diagnostics, LLVM/direct assembly object lowering, and broad object/reference/COW semantics remain blocked.
...
- [x] Object slice: generated-C can allocate supported declared class objects and declared child objects with ancestor metadata, instantiate constructorless declared classes through runtime string-valued class names, preserve type/class identity across supported ancestor chains, execute public properties including inherited public slots and `unset`, evaluate named `instanceof`, call supported public and inherited instance methods with `$this` using static or runtime string method names, run supported public and inherited constructors, call supported named public static methods without `$this`, and call supported public static methods through object receivers.
...
- [x] Primary semantic work is committed as `e2d20f3`.
- [x] Latest counted semantic commit is `e2d20f3`.
- [x] Primary head before this progress update is `e2d20f3`.
...
| --- | ---: | --- | --- |
| Runtime and ABI foundations | **90%** | `[##################--]` | Strong shared surfaces exist, now consumed by output buffers and the declared-object/property/method family, including ancestor-aware declared object allocation, dynamic name matching, and dynamic method-name miss diagnostics; some surfaces remain scaffolding until broader consumers land. |
| Compiler/backend consumers | **92%** | `[##################--]` | Generated-C is broad in selected areas, now including supported declared inheritance allocation, runtime string-valued constructorless declared-class `new`, constructorless argument evaluation, inherited public properties, inherited public constructors, and named/runtime public instance/static method paths; LLVM and C assembly consume selected ABI families. Direct assembly and many nested/object consumers remain blocked. |
| Executable PHP semantics | **71%** | `[##############------]` | Many focused linked programs run, now including supported dynamic constructorless declared-class `new`, declared child objects, inherited public property/method/static/constructor execution, public object static-receiver calls, and runtime string-valued public dynamic instance methods, but semantics are still selected islands rather than general PHP. |
...
| Calls, functions, frames | **65%** | `[#############-------]` | Bounded functions, public and inherited public instance methods, runtime string-valued dynamic public instance methods, public and inherited constructors, named public static methods, public object static-receiver calls, and constructorless `new` argument evaluation work for selected generated-C paths; closures, callable arrays/objects, named/unpacked args, dynamic class names requiring constructor dispatch, non-public contexts, and by-reference returns remain open. |
| Objects, properties, methods | **42%** | `[########------------]` | Primary now has a useful declared-object subset with supported inheritance metadata, runtime string-valued constructorless declared-class `new`, inherited public properties, public instance/static method execution, public dynamic instance-method names, inherited public constructors, and public object static-receiver dispatch; non-public methods, overrides, interfaces/traits, magic methods, dynamic/static properties, visibility contexts, references/COW, and backend parity remain open. |
...
| Broad integrated verification | **68%** | `[##############------]` | Focused gates are strong, including source/link proof for dynamic constructorless declared-class `new`, declared inheritance, named public static methods, object static-receiver dispatch, and runtime string-valued public dynamic instance methods; cross-feature linked programs, backend parity, and full end-to-end PHP proof lag. |

## Done / In Progress / Not Done

- [x] Done in primary: bounded generated-C direct variables, arrays/lvalues, selected dynamic calls, function globals, `$GLOBALS` self-imports, by-reference parameter writes, assignment expressions, direct-variable compound assignments, output buffers, finalizer transfer slices, declared-class allocation including runtime string-valued constructorless declared-class `new` and supported child allocation with ancestor metadata, constructorless argument evaluation, public and inherited public constructors, public and inherited public instance/static methods including runtime string-valued public dynamic instance methods, public object static-receiver methods, public properties including inherited public slots and unset, and named `instanceof`.
...
- [ ] Not done: general object model, dynamic class names requiring constructor dispatch, dynamic method forms outside declared public instance dispatch, non-public methods, overrides, interfaces/traits, unsupported contextual class names, closures, callable-array/object invocation, complete references/COW, by-reference returns, complete mutation/unset, full diagnostics, full cleanup/unwind, includes, variable variables, request/global parity, and direct assembly parity.

## Recent Primary-Integrated Work

- `e2d20f3`: generated-C `new $class(...)` now supports runtime string-valued direct-variable class names for generated constructorless declared classes. The compiler materializes the class-name variable as a native value, matches it against all registered constructorless declared-class candidates through the shared dynamic-name helper, and allocates matches through the existing declared-class or ancestor-aware declared-class allocation helpers. Constructorless named and dynamic `new` now evaluate and clean up argument expressions instead of rejecting constructorless argument lists. Source/link proof covers named constructorless arguments, case-insensitive runtime class names, multiple declared classes, declared child allocation through ancestor metadata, inherited public property writes/reads, named ancestor `instanceof`, `is_object()`, `gettype()`, `get_debug_type()`, and dynamic class names that require constructor dispatch staying blocked.
...
- Primary semantic baseline is `e2d20f3`; primary head was clean at `e2d20f3` before this progress update.
- Focused gates for the latest dynamic declared-class `new` slice passed using disk-backed `/tmp` targets: `cargo check -q -p phpc`, `cargo test -q -p phpc --test native_link native_executable_c_source_routes_dynamic_declared_class_new_through_declared_allocation_helpers`, `cargo test -q -p phpc --test native_link emit_exe_links_and_runs_declared_class_dynamic_new_program`, `cargo test -q -p phpc --test native_link native_executable_c_source_routes_declared_constructors_through_frame_dispatch`, `cargo test -q -p phpc --test native_link native_executable_c_source_keeps_unsupported_constructor_shapes_blocked`, `cargo test -q -p phpc --test native_runtime_abi generated_ir_routes`, `cargo test -q -p phpc native_call_diagnostics_centralizes_backend_recovery_across_call_families`, `cargo test -q -p phpc native_call_blocker_message_routes_shared_contract_by_backend_and_call_family`, `cargo test -q -p phpc --test native_link declared_class`, and `git diff --check`.
- The broader `cargo test -q -p phpc --test native_function_call_boundary` gate was re-run for this slice and still fails only in the previously noted unrelated direct-call column expectation tests: `emit_ir_routes_unsupported_direct_call_argument_results_through_call_boundary` and `native_executable_c_source_routes_unsupported_direct_call_argument_results_through_call_boundary`; 38/40 tests passed.
- The supervisor dashboard is current as of 2026-05-24 13:27 CEST and matches semantic baseline `e2d20f3`.
