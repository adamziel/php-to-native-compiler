...
The latest object slice routes generated-C named `instanceof` checks through `phpc_native_value_instanceof_class_with_diagnostic(...)` over native value handles and class-name bytes. It composes with declared-class allocation and ordinary value materialization across object variables, temporary objects, mismatched classes, case-insensitive names, and scalar subjects. Contextual class names, interfaces, inheritance, autoload, exact diagnostics, LLVM/direct assembly object lowering, and broad object/reference/COW semantics remain blocked.
...
- [x] Generated-C can allocate supported declared class objects without constructors through a shared runtime object ABI, observe their object/type/class-exists identity, execute public property reads/writes/`isset`/`empty` through a shared runtime object-property ABI, and evaluate named `instanceof` checks through a shared object/class relation ABI.
...
| Executable PHP semantics | **65%** | `[#############-------]` | Many focused linked programs run, including generated-C declared-class object allocation, public property reads/writes/presence checks, and named `instanceof`, but behavior is still selected islands. |
...
| Objects, properties, methods | **22%** | `[####----------------]` | Primary now has generated-C allocation for supported declared classes, object type identity, public property reads/writes/`isset`/`empty`, and named `instanceof`; constructors, methods, `$this`, inheritance/interfaces, dynamic/static properties, visibility contexts, and broad object execution remain open. |
...
| Broad integrated verification | **60%** | `[############--------]` | Focused gates are strong, now including output-buffer runtime/LLVM/source/link proof and declared-class object/property/`instanceof` runtime/source/link proof; cross-feature linked programs, backend parity, and full end-to-end PHP proof lag. |

## Done / In Progress / Not Done

- [x] Bounded generated-C direct variables, array/lvalue paths, selected dynamic calls, function globals, `$GLOBALS` self-imports, by-reference parameter writes, assignment expressions, direct-variable compound assignment, declared-class object allocation/properties/`instanceof`, output buffers, and finalizer transfer slices.
