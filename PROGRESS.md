...
Latest accounted source capability: `50d19f99` adds a reusable LLVM/generated-C
control-transfer cleanup report bridge over already-produced
`NativeDiagnosticResult` cleanup operands. The previous source commit,
`7891fcf3`, adds source-call result carrier selectors plus a runtime converter
from expression-owned `NativeCallResultHandle` to `NativeDiagnosticResult`.

Why the headline bars did not jump: the new source closes an important
source-call and cleanup-result ownership gap, but it does not yet wire
production source-call lowering or produce cleanup operands from real
`finally`, destructor, shutdown, unwind, or terminal control-flow frames.
Terminal/control-flow, lvalue, RMW, reference-binding, and diagnostic
call-argument operands still need their own ownership and ordering contracts.

Current critical path to 100%:

1. Finish expression-owned `NativeCallResultHandle` carriers and source-call
   lowering over the new lookup-plus-invoke ownership helpers, including
   constructor allocation and closure argument-handle blockers.
...
| Calls, functions, frames | **85%** | `[#################---]` | Runtime callable table/value dispatch, call arguments/frame/result ABI, conditional handoff, generated-C direct/dynamic callable consumers, declared-method registration/wrapper frames, callable return facts, by-reference argument transport, descriptor closures, closure returns, request-state frame handoff, access-context lookup ABI, lookup-plus-invoke exactly-once argument ownership helpers, and source-call result carrier selectors are integrated. Unknown runtime callables, production source-call emission over the new carriers, builtin return summaries, executable by-reference alias transfer, full object/method parity, namespace fallback, autoload, magic calls, named/spread breadth, broader return references, constructors, cleanup/unwind, and backend parity remain open. |
...
| Control flow, cleanup, diagnostics | **65%** | `[#############-------]` | Selected branches, loops, transfers, finalizers, output buffers, diagnostic blockers, owned diagnostic-result list contracts, consumer contracts, backend family consumers, deferred-cleanup blockers, control-transfer cleanup result consumers and report bridges, try-body call-boundary preflight, report sinks, continuation helpers, discarded statement-expression operands, and echo/print output operands exist. Broad unwind/finally/destructor/shutdown execution, cleanup result production, executable reference binding, remaining semantic diagnostic-result producer migration, and source-ordered diagnostics remain open. |
...
| --- | ---: | ---: | ---: | --- |
| Diagnostic-result carrier stack | **100%** `[####################]` | **100%** `[####################]` | **60%** `[############--------]` | Runtime/result contracts, family consumers, continuation helpers, report sinks, discarded statement-expression operands, echo/print output operands, source-call result conversion, and control-transfer cleanup result consumers/report bridges are integrated. Terminal producers, semantic cleanup result production, lvalue, reference, RMW, and call-argument operands still need exact ownership and ordering migrations. |
| Callable access context and class metadata | **100%** `[####################]` | **100%** `[####################]` | **50%** `[##########----------]` | Shared runtime access-context policy, lookup-plus-invoke argument ownership, source-call result carrier selectors, and allocatable-class metadata are integrated for function/method/static/constructor lookup preflights. Generated semantic call lowering over result carriers, constructor execution, closure argument-handle ownership, autoload, magic, and full visibility parity remain open. |
...
- Source-call production lowering over expression-owned
  `NativeCallResultHandle` carriers; constructor allocation and closure
  argument-handle invocation ownership remain blocked.
- Remaining semantic diagnostic-result operand migration for terminal, cleanup
  result production, lvalue, reference, RMW, and call-argument families; exact
  PHP diagnostics, source ordering, suppression/custom handlers, and backend
  parity across generated C, LLVM, and direct assembly.

## Latest Focused Verification

For `7891fcf3` and `50d19f99`:
...
- `cargo test -p phpc native_diagnostic_result_control_transfer_cleanup_reports_reusable_cleanup_operands -- --nocapture`
