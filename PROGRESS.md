...
Refresh: 2026-06-10T12:22Z
Measured: `ptn-nvs` rebased on `origin/master@8f0f52165`; focused
`array_walk()` closure global-swap COW evidence plus gates and manifests.
...
| Native compiled PHP snippets | 348 | 348 | 0 |
...
function-boundaries 1/4, reference-interaction 5/5. `array_walk()` now supports
anonymous closure callbacks with `use` captures and `$GLOBALS` array swaps.
Offset-form `??=` retains keyed array/string native coverage. Details live in
...
reference-aware `array_sum()`/`strtr()`/`in_array()`, closure values/captures,
`array_walk()` closure callbacks with `$GLOBALS` swaps, recursive array merge
and replace, `debug_zval_dump()`, dynamic lvalue-reference calls, append/list
...
call-result by-reference return chains, plus `array_fill_keys()` over current
boxed arrays with scalar key coercion.

## Still Needed

Remaining COW PHPT gaps are recursive by-reference return chaining,
`array_reduce()` callback return/refcount behavior, and broader closure-backed
callback edges. Broader bounded-PHPT gaps are still objects, unsupported
array/string internals, 64-bit operator exactness, foreach edge diagnostics,
object/property compound lvalues, scalar offset-lvalue fatal
...
tools/phpt-cow-manifest.txt`. Broad manifest is 151/200.
