...
Measured: `ptn-inr` rebased on `origin/master@51e3314`; anonymous callback
closures and scalar math unsupported-operand TypeErrors for `abs()`, `sqrt()`,
and `fdiv()`.
...
`func_*`, `print_r`, binary strings, string offsets, scalar diagnostics,
scalar math array/object TypeErrors, array literal references, array union `+`,
...
directory predicates, array-path
...
focused scalar-math native tests and bounded math PHPT rows;
