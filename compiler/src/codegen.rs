...
impl CGenerator {
        ...
        if self.uses_native_runtime_helpers() {
            ...
            if self.uses_native_request_state_helpers {
                ...
                if self.uses_native_array_lvalue_helpers || self.uses_native_symbol_table_helpers {
                    ...
                    output.push_str("extern phpc_NativeRequestStateOperationResult phpc_native_request_state_superglobal_keyed_reference_bind_operation(phpc_NativeRequestStateHandle request_state, const uint8_t *bag, size_t bag_len, const uint8_t *key, size_t key_len, uint8_t key_status, phpc_NativeReferenceHandle reference);\n");
                    output.push_str("extern size_t phpc_native_request_state_reference_result_report_diagnostic(phpc_NativeRequestStateReferenceResult result);\n");
                    output.push_str("extern void phpc_native_request_state_reference_result_free(phpc_NativeRequestStateReferenceResult result);\n");
                }
                output.push_str(
    ...
    fn emit_request_superglobal_path_value_read(
        ...
        self.body.push(format!(
            "phpc_NativeRequestStateOperationResult {result} = phpc_native_request_state_superglobal_path_operation({request_state}, PHPC_NATIVE_REQUEST_STATE_OP_VALUE, {bag_bytes}, {}, {}, {}, {}, {});",
            name.len(),
            path.ptrs,
            path.lens,
            path.len,
    ...
    fn emit_request_superglobal_path_presence_expr(
        ...
        self.body.push(format!(
            "phpc_NativeRequestStateOperationResult {result} = phpc_native_request_state_superglobal_path_operation({request_state}, PHPC_NATIVE_REQUEST_STATE_OP_PRESENCE, {bag_bytes}, {}, {}, {}, {}, {});",
            name.len(),
            path.ptrs,
            path.lens,
            path.len,
    ...
    fn emit_statement(&mut self, stmt: &Stmt) -> CompileResult<()> {
        ...
        match stmt {
            ...
            Stmt::ReferenceAssign {
                ...
                if self.emit_request_superglobal_keyed_reference_assignment(
                    target, source, *span, "",
                )? {
                    return Ok(());
                }

                if self.emit_request_superglobal_keyed_reference_source_assignment(
                    target, source, *span, "",
                )? {
                    return Ok(());
                }
            ...
            Expr::Index { target, span, .. } => {
                ...
                if let Expr::Index { target, index, .. } = expr {
                    if let Some(name) = request_superglobal_root_name(target) {
                        let value =
                            self.emit_request_superglobal_keyed_value_read(name, index, "")?;
                        self.retain_native_value_cleanup_handle(&value.handle);
                        return Ok(CValue::NativeValueHandle(value.handle));
                    }
                }
