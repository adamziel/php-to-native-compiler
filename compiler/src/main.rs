...
use php_compiler::include_discovery::executable_compilation_unit_with_literal_include_units;
...
fn command_compile(args: &[String]) -> CompileResult<u8> {
    ...
    if flag == "--emit-exe" {
        ...
        let unit =
            executable_compilation_unit_with_literal_include_units(&source, &input, &repo_root)
                .map_err(|error| {
        ...
        let c_source = emit_native_executable_c_source_for_include_units(&unit)
