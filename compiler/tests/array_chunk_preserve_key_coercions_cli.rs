...
#[test]
fn array_chunk_preserve_key_coercion_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dir = workspace_root.join("tests/fixtures/milestone2333");
    let mut fixtures = cli_snapshot_fixtures(&fixture_dir, workspace_root);

    fixtures.sort();
    assert!(
        !fixtures.is_empty(),
        "expected array_chunk preserve-key coercion CLI snapshot fixtures"
    );
    ...
    for fixture_arg in fixtures {
        ...
        let fixture_arg = fixture_arg
            .to_str()
            .expect("array_chunk preserve-key coercion fixture path is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path, workspace_root: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array_chunk preserve-key coercion fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array_chunk preserve-key coercion fixture entry is readable")
                .path()
        })
        ...
        .map(|path| {
            path.strip_prefix(workspace_root)
                .expect("array_chunk preserve-key coercion fixture lives under the workspace root")
                .to_path_buf()
        })
        .collect()
}
