...
#[test]
fn array_value_intersection_builtin_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dirs = [
        workspace_root.join("tests/fixtures/milestone43"),
        workspace_root.join("tests/fixtures/milestone44"),
    ];
    ...
    assert!(
        !fixtures.is_empty(),
        "expected array-value-intersection builtin CLI snapshot fixtures"
    );
    ...
    for fixture_arg in fixtures {
        ...
        let fixture_arg = fixture_arg
            .to_str()
            .expect("array-value-intersection fixture path is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path, workspace_root: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array-value-intersection fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array-value-intersection fixture entry is readable")
                .path()
        })
        ...
        .map(|path| {
            path.strip_prefix(workspace_root)
                .expect("array-value-intersection fixture lives under the workspace root")
                .to_path_buf()
        })
        .collect()
}
