...
#[test]
fn logical_xor_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dir = workspace_root.join("tests/fixtures/milestone155");
    let mut fixtures = cli_snapshot_fixtures(&fixture_dir);

    fixtures.sort();
    assert!(
        !fixtures.is_empty(),
        "expected logical-xor CLI snapshot fixtures"
    );
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("logical-xor fixture directory is readable")
        .map(|entry| entry.expect("logical-xor fixture entry is readable").path())
