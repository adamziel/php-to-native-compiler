...
#[test]
fn ternary_expression_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let mut fixtures = Vec::new();
    for fixture_dir in ["milestone150", "milestone151", "milestone152"] {
    ...
    assert!(
        !fixtures.is_empty(),
        "expected ternary-expression CLI snapshot fixtures"
    );
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("ternary-expression fixture directory is readable")
        .map(|entry| {
            entry
                .expect("ternary-expression fixture entry is readable")
                .path()
        })
