...
#[test]
fn array_numeric_aggregation_builtin_cli_snapshots_match_committed_outputs() {
    ...
    assert!(
        !fixtures.is_empty(),
        "expected array numeric aggregation builtin CLI snapshot fixtures"
    );
    ...
    for fixture_arg in fixtures {
        ...
        let fixture_arg = fixture_arg
            .to_str()
            .expect("array numeric aggregation fixture path is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path, workspace_root: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array numeric aggregation fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array numeric aggregation fixture entry is readable")
                .path()
        })
        ...
        .map(|path| {
            path.strip_prefix(workspace_root)
                .expect("array numeric aggregation fixture lives under the workspace root")
                .to_path_buf()
        })
        .collect()
}
