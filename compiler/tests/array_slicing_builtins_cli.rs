...
fn array_slicing_builtin_cli_snapshots_match_committed_outputs() {
    ...
    let fixture_dirs = [
        ...
        workspace_root.join("tests/fixtures/milestone32"),
    ];
    ...
    for fixture_arg in fixtures {
        ...
        let fixture_arg = fixture_arg
            .to_str()
            .expect("array-slicing fixture path is valid UTF-8");
    ...
    fs::read_dir(fixture_dir)
        ...
        .map(|path| {
            path.strip_prefix(workspace_root)
                .expect("array-slicing fixture lives under the workspace root")
                .to_path_buf()
        })
        .collect()
}
