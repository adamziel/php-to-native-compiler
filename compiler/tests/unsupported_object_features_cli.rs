...
#[test]
fn unsupported_object_feature_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dir = workspace_root.join("tests/fixtures/unsupported_object_features");
    let mut fixtures = cli_snapshot_fixtures(&fixture_dir);

    fixtures.sort();
    assert!(
        fixtures.len() >= 3,
        "expected representative unsupported object feature CLI fixtures"
    );
    ...
    for fixture in fixtures {
        let file_name = fixture
            ...
            .expect("unsupported object fixture file name is valid UTF-8");
        let fixture_arg = format!("tests/fixtures/unsupported_object_features/{file_name}");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("unsupported object feature fixture directory is readable")
        .map(|entry| {
            entry
                .expect("unsupported object feature fixture entry is readable")
                .path()
        })
