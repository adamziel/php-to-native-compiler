...
#[test]
fn callable_syntax_only_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dirs = [
        workspace_root.join("tests/fixtures/milestone543"),
        workspace_root.join("tests/fixtures/milestone551"),
    ];
    ...
    assert!(
        !fixtures.is_empty(),
        "expected callable syntax-only CLI snapshot fixtures"
    );
    ...
    for fixture in fixtures {
        let file_name = fixture
            ...
            .expect("callable syntax-only fixture file name is valid UTF-8");
        let fixture_dir_name = fixture
            .parent()
            .and_then(|path| path.file_name())
            .and_then(|value| value.to_str())
            .expect("callable syntax-only fixture directory name is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("callable syntax-only fixture directory is readable")
        .map(|entry| {
            entry
                .expect("callable syntax-only fixture entry is readable")
                .path()
        })
