...
fn assert_native_c_source_error_or_runtime_boundary(source: &str, expected: &str) {
    let program = parse(source).unwrap();

    match emit_native_executable_c_source(&program) {
        Ok(generated) => assert_generated_runtime_boundary(source, &generated),
        Err(error) => {
...
fn assert_native_c_source_error_or_runtime_boundary_at(
    source: &str,
    line: usize,
    column: usize,
    expected: &str,
...
fn assert_generated_runtime_boundary(source: &str, generated: &str) {
    assert!(
        generated.contains("phpc_native_diagnostic")
            && (generated.contains("phpc_native_callable")
                || generated.contains("phpc_native_method")
                || generated.contains("phpc_native_constructor")
                || generated.contains("phpc_native_value_")
                || generated.contains("phpc_native_exit")
                || generated.contains("unsupported call ")),
        "generated C should keep runtime diagnostics/call-boundary handling for {source}:\n{generated}"
    );
}
...
"#,
    ...
    assert!(ir.contains("@phpc_native_int(i64 3)"), "{ir}");
...
fn native_executable_c_source_routes_supported_direct_call_argument_results_through_call_boundary()
{
    for (source, line, column) in [
        ...
        assert_native_c_source_error_or_runtime_boundary_at(
            source,
            line,
            column,
...
fn native_executable_c_source_routes_direct_special_forms_through_call_boundary() {
    for (source, expected) in [
        ...
        assert_native_c_source_error_or_runtime_boundary(source, expected);
    }
}
...
fn native_try_cleanup_unwind_requirements_route_control_transfers_through_shared_boundary() {
    for source in [
        ...
        match emit_native_executable_c_source(&program) {
            Ok(generated) => assert!(
                generated.contains("unsupported call ")
                    || generated.contains("phpc_native_diagnostic_result"),
                "try cleanup source should lower through runtime diagnostics when it no longer rejects:\n{generated}"
            ),
            Err(error) => {
                assert_eq!(error.phase, Phase::Codegen);
...
#[test]
fn trait_effective_member_metadata_composes_properties_and_constants() {
    let source = r#"<?php
trait BaseMemberTrait {
    public $value = "seed";
    public static $count = 2;
    public const LABEL = "trait";
}
trait SameMemberTrait {
    public $value = "seed";
    public static $count = 2;
    public const LABEL = "trait";
}
class UsesMemberTrait {
    use BaseMemberTrait, SameMemberTrait;
    public function read() {
        return self::LABEL . ":" . $this->value . ":" . self::$count;
    }
}
$object = new UsesMemberTrait();
echo $object->read();
"#;
    ...
    assert!(
        generated.contains("declared_class_property_name_ptrs_")
            && generated.contains("declared_class_property_static_flags_")
            && generated.contains("declared_class_property_default_values_")
            && generated.contains("phpc_native_class_constant_declare_constant_bytes_and_free")
            && generated.contains(
                "phpc_native_static_property_storage_declare_properties_and_defaults_bytes"
            )
            && !generated.contains("object/class lowering rejects")
            && !generated.contains("trait lowering rejects"),
        "trait properties/constants should compose into declared class metadata:\n{generated}"
    );
}

#[test]
fn trait_effective_member_metadata_reports_property_and_constant_conflicts() {
    for source in [
        r#"<?php
trait FirstPropertyTrait { public $value = "first"; }
trait SecondPropertyTrait { public $value = "second"; }
class UsesConflictingProperties { use FirstPropertyTrait, SecondPropertyTrait; }
"#,
        r#"<?php
trait FirstConstantTrait { public const VALUE = 1; }
trait SecondConstantTrait { public const VALUE = 2; }
class UsesConflictingConstants { use FirstConstantTrait, SecondConstantTrait; }
"#,
    ] {
        let error = emit_native_executable_c_source(&parse(source).unwrap()).unwrap_err();
        ...
        assert!(
            error.message.contains("unsupported trait use"),
            "trait member composition diagnostics should come from trait_semantics: {error:?}"
        );
    }
}
...
#[test]
fn native_executable_c_source_routes_by_reference_closure_captures_through_reference_boundary() {
    let source = concat!(
        "<?php\n",
        "$slot = \"old\";\n",
