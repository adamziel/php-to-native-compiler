...
"#,
    ...
    assert!(
        ir.contains("@phpc_native_value_from_string_bytes_with_diagnostic"),
        "{ir}"
    );
}
...
"#,
    ...
    assert_eq!(ir.matches("@phpc_native_bool(i1 true)").count(), 3, "{ir}");
    assert!(
        ir.contains("@phpc_native_value_from_string_bytes_with_diagnostic"),
        "{ir}"
    );
...
"#,
    ...
    assert_eq!(ir.matches("@phpc_native_bool(i1 %tmp1)").count(), 5, "{ir}");
    assert!(ir.contains("@phpc_native_bool(i1 true)"), "{ir}");
}
...
"#,
    ...
    assert_eq!(
        ir.matches("call %phpc.NativeScalarValue @phpc_native_int(i64 1)")
            .count(),
        3,
        "{ir}"
    );
...
"#,
    ...
    assert!(ir.matches("@phpc_native_bool(i1").count() >= 6, "{ir}");
}
...
"#,
    ...
    assert_eq!(ir.matches("@phpc_native_bool(i1 true)").count(), 4, "{ir}");
    assert!(
        !ir.contains("call %phpc.NativeScalarValue @phpc_native_int("),
        "{ir}"
    );
...
"#,
    ...
    assert_eq!(ir.matches("@phpc_native_bool(i1 true)").count(), 2, "{ir}");
    assert!(ir.contains("c\"x\\00\""), "{ir}");
    assert!(ir.contains("c\"y\\00\""), "{ir}");
    assert!(
        !ir.contains("call %phpc.NativeScalarValue @phpc_native_float("),
...
"#,
    ...
    assert!(ir.matches("@phpc_native_bool(i1 %tmp").count() >= 1, "{ir}");
    assert!(ir.contains("c\"x\\00\""), "{ir}");
}
...
"#,
    ...
    assert!(ir.matches("@phpc_native_bool(i1 %tmp").count() >= 2, "{ir}");
    assert!(ir.contains("c\"x\\00\""), "{ir}");
}
...
"#,
    ...
    assert!(
        ir.matches("@phpc_native_bool(i1 %tmp3)").count() >= 2,
        "{ir}"
    );
    assert!(ir.contains("c\"x\\00\""), "{ir}");
}
...
"#,
    ...
    assert_eq!(ir.matches("@phpc_native_bool(i1 %tmp3)").count(), 4, "{ir}");
}
...
"#,
    ...
    assert!(ir.matches("@phpc_native_bool(i1").count() >= 8, "{ir}");
...
"#,
    ...
    assert!(ir.contains("%tmp0 = add i64 1, 2"), "{ir}");
    assert!(ir.contains("%tmp1 = icmp eq i64 %tmp0, 3"), "{ir}");
    assert!(ir.contains("@phpc_native_bool(i1 %tmp1)"), "{ir}");
