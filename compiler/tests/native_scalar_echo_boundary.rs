...
fn emit_ir_lowers_literal_assignments_and_scalar_echoes() {
    ...
    assert!(uses_native_diagnostic_result_output(&ir, 9), "{ir}");
...
fn emit_ir_keeps_false_and_null_echoes_silent() {
    ...
    assert!(ir.contains("@phpc_native_null()"), "{ir}");
    assert!(ir.contains("c\"done\\00\""), "{ir}");
    assert!(
        ir.contains("@phpc_native_value_from_string_bytes_with_diagnostic(ptr @.str.0, i64 4, ptr"),
        "{ir}"
    );
    assert!(uses_native_diagnostic_result_output(&ir, 3), "{ir}");
}
...
fn emit_ir_uses_final_static_scalar_assignment_after_reassignment() {
    ...
    assert!(uses_native_diagnostic_result_output(&ir, 2), "{ir}");
