...
fn emit_ir_lowers_literal_assignments_and_scalar_echoes() {
    ...
    assert!(
        ir.contains("call %phpc.NativeScalarValue @phpc_native_int(i64 42)")
            ...
            && ir.contains("call %phpc.NativeScalarValue @phpc_native_bool(i1 false)")
            && ir.contains("call %phpc.NativeScalarValue @phpc_native_null()"),
        "{ir}"
    );
    assert!(
    ...
    assert_ir_uses_diagnostic_result_stdout(&ir, 8);
    assert!(
...
fn emit_ir_keeps_false_and_null_echoes_silent() {
    ...
    assert!(
        ir.contains("@phpc_native_value_from_string_bytes_with_diagnostic(ptr @.str.0, i64 4"),
        "{ir}"
    );
...
fn emit_ir_uses_final_static_scalar_assignment_after_reassignment() {
    ...
    assert!(
        ir.contains("call %phpc.NativeScalarValue @phpc_native_int(i64 227)")
            && ir.contains(
                "@phpc_native_value_from_string_bytes_with_diagnostic(ptr @.str.0, i64 5"
            ),
        "{ir}"
    );
