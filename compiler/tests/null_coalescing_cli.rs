...
#[test]
fn null_coalescing_cli_snapshots_match_committed_outputs() {
    ...
    fixtures.extend(cli_snapshot_fixtures(
        &workspace_root.join("tests/fixtures/milestone128"),
    ));
    fixtures.extend(cli_snapshot_fixtures(
        &workspace_root.join("tests/fixtures/milestone129"),
    ));

    fixtures.sort();
    assert!(
        !fixtures.is_empty(),
        "expected null-coalescing CLI snapshot fixtures"
    );
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("null-coalescing fixture directory is readable")
        .map(|entry| {
            entry
                .expect("null-coalescing fixture entry is readable")
                .path()
        })
