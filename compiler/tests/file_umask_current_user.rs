...
#[test]
fn get_current_user_returns_bounded_non_empty_string_metadata() {
    let execution = run_source(
        r#"<?php
$user = get_current_user();
echo is_string($user) ? "string\n" : "bad\n";
echo strlen($user) > 0 ? "non-empty\n" : "empty\n";
echo function_exists("get_current_user") ? "exists\n" : "missing\n";
echo is_callable("get_current_user") ? "callable\n" : "not-callable\n";
echo (new ReflectionFunction("get_current_user"))->getReturnType()->getName();
"#,
    )
    .unwrap();
