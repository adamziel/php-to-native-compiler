...
"#,
    ...
    assert_eq!(error.message, LLVM_STR_STARTS_WITH_REJECTION);
}
...
#[test]
fn emit_asm_rejects_str_starts_with_before_backend_execution() {
...
#[test]
fn native_str_starts_with_emit_ir_cli_snapshot_matches_committed_output() {
    assert_cli_snapshot_matches(
        "--emit-ir",
        "tests/fixtures/milestone1188/native_str_starts_with_boundary_emit_ir.cli",
    );
}

#[test]
fn native_str_starts_with_emit_asm_cli_snapshot_matches_committed_output() {
    assert_cli_snapshot_matches(
        "--emit-asm",
        "tests/fixtures/milestone1188/native_str_starts_with_boundary_emit_asm.cli",
    );
}
...
fn assert_cli_snapshot_matches(mode: &str, snapshot_path: &str) {
    ...
    let fixture = workspace_root
        .join("tests/fixtures/milestone1188/native_str_starts_with_boundary.phpc-source");
    ...
    let expected = fs::read_to_string(workspace_root.join(snapshot_path))
        .expect("native str_starts_with CLI snapshot is readable");
