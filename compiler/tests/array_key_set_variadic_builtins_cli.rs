...
#[test]
fn array_key_set_variadic_builtin_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dir = workspace_root.join("tests/fixtures/milestone40");
    let mut fixtures = cli_snapshot_fixtures(&fixture_dir);

    fixtures.sort();
    assert!(
        !fixtures.is_empty(),
        "expected array-key-set variadic builtin CLI snapshot fixtures"
    );
    ...
    for fixture in fixtures {
        let file_name = fixture
            ...
            .expect("array-key-set variadic fixture file name is valid UTF-8");
        let fixture_arg = format!("tests/fixtures/milestone40/{file_name}");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array-key-set variadic fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array-key-set variadic fixture entry is readable")
                .path()
        })
