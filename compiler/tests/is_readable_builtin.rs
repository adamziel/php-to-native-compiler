...
fn fixture_source_file() -> String {
    ...
    manifest_dir
        ...
        .join("tests/fixtures/milestone780/is_readable.php")
        .display()
        .to_string()
}
...
#[test]
fn is_readable_checks_current_local_filesystem_subset() {
...
#[test]
fn is_readable_is_available_through_string_valued_calls() {
    let execution = run_source_with_source_file(
        r#"<?php
$call = "is_readable";
...
#[test]
fn is_readable_rejects_forms_outside_current_subset() {
...
"#,
    ...
    assert!(
        arity
            .stdout
            .contains("Too few arguments to function is_readable(), 0 passed"),
        "{}",
        arity.stdout
    );
...
"#,
    ...
    assert_eq!(
        stream.message,
        "unsupported call is_readable(): stream wrappers are not supported in the current subset"
    );
}

#[test]
fn emit_ir_rejects_is_readable_until_native_filesystem_lowering_exists() {
    let error = emit_ir_source(
        r#"<?php
echo is_readable("wp-content/fatal-error-handler.php") ? "yes" : "no";
"#,
    )
    .unwrap_err();
...
#[test]
fn emit_ir_includes_is_readable_in_native_callable_lookup_table() {
    let ir = emit_ir_source(
        r#"<?php
echo function_exists("is_readable") ? "1" : "0";
echo is_callable("is_readable") ? "1" : "0";
"#,
    )
    .unwrap();
