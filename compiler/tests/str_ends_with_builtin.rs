...
fn str_ends_with_rejects_forms_outside_current_subset() {
    ...
    assert!(
        too_few.stdout.contains(
            "Fatal error: Uncaught TypeError: Too few arguments to function str_ends_with(), 1 passed"
        ),
        "{}",
        too_few.stdout
    );
...
#[test]
fn emit_ir_routes_str_ends_with_metadata_and_direct_calls() {
...
#[test]
fn emit_asm_lowers_str_ends_with_through_shared_predicate_contract() {
    ...
    assert!(
        asm.contains("phpc_native_value_string_predicate_with_diagnostic"),
        "{asm}"
    );
    assert!(
