...
#[test]
fn array_mapping_builtin_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dirs = [
        workspace_root.join("tests/fixtures/milestone22"),
        workspace_root.join("tests/fixtures/milestone23"),
        workspace_root.join("tests/fixtures/milestone25"),
        workspace_root.join("tests/fixtures/milestone26"),
        workspace_root.join("tests/fixtures/milestone27"),
        workspace_root.join("tests/fixtures/milestone28"),
    ];
    ...
    assert!(
        !fixtures.is_empty(),
        "expected array-mapping builtin CLI snapshot fixtures"
    );
    ...
    for fixture in fixtures {
        let file_name = fixture
            ...
            .expect("array-mapping fixture file name is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array-mapping fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array-mapping fixture entry is readable")
                .path()
        })
