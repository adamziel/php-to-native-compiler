...
#[test]
fn object_property_empty_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dir = workspace_root.join("tests/fixtures/milestone119");
    let mut fixtures = cli_snapshot_fixtures(&fixture_dir);

    fixtures.sort();
    assert!(
        !fixtures.is_empty(),
        "expected object property empty CLI snapshot fixtures"
    );

    for fixture in fixtures {
        let fixture_arg = fixture
            .strip_prefix(workspace_root)
            .expect("object property empty fixture is under workspace root")
            .to_string_lossy()
            .replace('\\', "/");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("object property empty fixture directory is readable")
        .map(|entry| {
            entry
                .expect("object property empty fixture entry is readable")
                .path()
        })
