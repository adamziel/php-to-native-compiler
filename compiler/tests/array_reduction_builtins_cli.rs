...
#[test]
fn array_reduction_builtin_cli_snapshots_match_committed_outputs() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .expect("compiler has a workspace root");
    let fixture_dirs = [
        workspace_root.join("tests/fixtures/milestone51"),
        workspace_root.join("tests/fixtures/milestone52"),
    ];
    ...
    assert!(
        !fixtures.is_empty(),
        "expected array-reduction builtin CLI snapshot fixtures"
    );
    ...
    for fixture in fixtures {
        let file_name = fixture
            ...
            .expect("array-reduction fixture file name is valid UTF-8");
...
fn cli_snapshot_fixtures(fixture_dir: &Path) -> Vec<PathBuf> {
    fs::read_dir(fixture_dir)
        .expect("array-reduction fixture directory is readable")
        .map(|entry| {
            entry
                .expect("array-reduction fixture entry is readable")
                .path()
        })
