...
fn emit_type_hint_runtime_helpers(out: &mut String) {
    ...
    out.push_str(
        "\nstatic PTN_UNUSED int ptn_value_satisfies_object_type_hint(PtnValue value) {\n",
    );
    out.push_str("    value = ptn_value_deref(value);\n");
...
fn emit_class_metadata_helpers(
    ...
    out.push_str(
        "\nstatic PTN_UNUSED int ptn_declared_class_static_call_magic_applicable(const char *class_name, const char *method_name, const char *access_scope) {\n",
    );
    if classes.is_empty() {
        out.push_str("    (void)class_name;\n");
    }
    ...
    for class in classes {
        ...
        out.push_str("        if (ptn_declared_class_static_call_magic_applicable(class_name, method_name, access_scope)) {\n");
        out.push_str("            return 1;\n");
        out.push_str("        }\n");
        out.push_str("        return 0;\n");
        out.push_str("    }\n");
    ...
    out.push_str("    if (!ptn_declared_class_static_call_magic_applicable(class_name, method_name, runtime != NULL ? runtime->current_class_name : NULL)) {\n");
    out.push_str("        return 0;\n");
    out.push_str("    }\n");
    for class in classes {
    ...
    for class in classes {
        ...
        for entry in class_method_lookup_chain(class, classes) {
            ...
            if method.is_static {
                ...
                out.push_str("                int ptn_nonstatic_written = snprintf(ptn_nonstatic_message, sizeof(ptn_nonstatic_message), \"Non-static method %s::%s() cannot be called statically\", target_class_name, method_name);\n");
...
fn magic_declaration_fatal_message(
    ...
    ) && function.parameters.iter().any(|parameter| parameter.by_ref)
    {
        return Some(format!(
    ...
    match method_key.as_str() {
        "__get" | "__set" | "__isset" | "__unset" | "__call" | "__callstatic" => {
            if let Some(message) =
                magic_parameter_type_fatal(class, method, function, 0, TypeHint::String, "string")
            {
                return Some(message);
            }
        }
...
impl AssignmentTargetLine for AssignmentTarget {
    fn line(&self) -> usize {
        match self {
            ...
            | AssignmentTarget::DynamicStaticProperty { line, .. } => *line,
...
fn reference_target_mentions_variable(target: &ReferenceTarget, name: &str) -> bool {
    match target {
        ...
        ReferenceTarget::DynamicVariable { name: target, .. } => {
            value_mentions_variable(target, name)
        }
...
fn assignment_target_uses_this(target: &AssignmentTarget) -> bool {
    match target {
        ...
        AssignmentTarget::DynamicStaticProperty { receiver, .. } => value_expr_uses_this(receiver),
...
fn reference_target_uses_this(target: &ReferenceTarget) -> bool {
    match target {
        ...
        ReferenceTarget::DynamicVariable { name, .. } => value_expr_uses_this(name),
        ReferenceTarget::PropertyArrayDim {
            receiver,
            dimensions,
            ..
...
impl ValueEmitter {
        ...
        match source {
            ...
            ValueExpr::MethodCall { receiver, name, .. } => {
                let class_name = match receiver.as_ref() {
                    ValueExpr::Load { name, .. } if name == "this" => {
                        let Some(class_name) = self.current_class_name.clone() else {
                            return false;
                        };
                        if self.declared_class_has_descendant(&class_name) {
                            return false;
                    ...
                    ValueExpr::NewObject { class_name, .. } => {
                        self.class_name_fetch_name(class_name)
                    }
                    _ => return false,
                };
    ...
    fn emit_assignment(
        ...
        if let AssignmentTarget::DynamicStaticPropertyArrayDim {
            receiver,
            name,
            dimensions,
            line,
        ...
        {
            if let Some(compound_op) = assignment_compound_binary_op(op) {
                return self.emit_dynamic_static_property_array_dim_compound_assignment(
                    out,
                    receiver,
                    name,
                    dimensions,
            ...
            let value_temp = self.emit_materialized_value(out, value);
            let result_temp = self.emit_dynamic_static_property_array_dim_assignment_from_temp(
                out,
                receiver,
                name,
                dimensions,
        ...
        if let AssignmentTarget::DynamicStaticProperty {
            receiver,
            name,
            line,
        } = target
        {
            if let Some(compound_op) = assignment_compound_binary_op(op) {
                return self.emit_dynamic_static_property_compound_assignment(
                    out,
                    receiver,
                    name,
                    *line,
    ...
    fn emit_dynamic_static_property_array_dim_assignment_from_temp(
        &mut self,
        out: &mut String,
        receiver: &ValueExpr,
        name: &str,
    ...
    fn emit_dynamic_static_property_assignment_from_temp(
        &mut self,
        out: &mut String,
        receiver: &ValueExpr,
        name: &str,
    ...
    fn emit_dynamic_static_property_array_dim_compound_assignment(
        &mut self,
        out: &mut String,
        receiver: &ValueExpr,
        name: &str,
    ...
    fn emit_dynamic_static_property_compound_assignment(
        &mut self,
        out: &mut String,
        receiver: &ValueExpr,
        name: &str,
    ...
    fn emit_bind_assignment_target_reference(
        ...
        match target {
            ...
            AssignmentTarget::DynamicStaticProperty { .. } => {
                unreachable!("parser rejects by-reference assignment to dynamic static properties");
            }
            AssignmentTarget::List(_) => {
    ...
    fn emit_store_assignment_target_from_temp(
        ...
        match target {
            ...
            AssignmentTarget::DynamicStaticPropertyArrayDim {
                ...
                dimensions,
                line,
            } => self.emit_dynamic_static_property_array_dim_assignment_from_temp(
                out, receiver, name, dimensions, *line, value_temp,
            ),
            AssignmentTarget::Property {
                receiver,
            ...
            AssignmentTarget::DynamicStaticProperty {
                ...
                name,
                line,
            } => self.emit_dynamic_static_property_assignment_from_temp(
                out, receiver, name, *line, value_temp,
            ),
            AssignmentTarget::List(target) => {
    ...
    fn emit_reference_list_assignment_from_variable(
        ...
        for (index, element) in target.elements.iter().enumerate() {
            ...
            let value_temp = match &element.target {
                ListAssignmentElementTarget::Value(target) => {
                    if matches!(target.as_ref(), AssignmentTarget::List(nested) if list_assignment_has_reference(nested))
                    {
                        let element_temp = self.next_temp();
                        out.push_str("    PtnValue ");
                        out.push_str(&element_temp);
    ...
    fn emit_dynamic_class_name_cstr(
        ...
        let class_value_temp = self.emit_dynamic_class_name_fetch(out, receiver, line);
        let class_name_temp = self.next_temp();
        out.push_str("    char *");
        out.push_str(&class_name_temp);
        out.push_str(" = ptn_value_to_string(");
    ...
    fn emit_dynamic_static_property_fetch(
        &mut self,
        out: &mut String,
        receiver: &ValueExpr,
        name: &str,
    ...
    fn emit_by_ref_call_argument(
        ...
        if matches!(argument, ValueExpr::Load { name, .. } if name == "GLOBALS") {
            let temp = self.next_temp();
            out.push_str("    PtnValue ");
            out.push_str(&temp);
            out.push_str(" = ptn_null();\n");
        ...
        if value_is_temporary_write_context(argument) {
            ...
            out.push_str(&temp);
            out.push_str(" = ptn_null();\n");
            out.push_str("    ptn_throw_exception_at(&runtime, \"Error\", \"Cannot use temporary expression in write context\", \"");
            out.push_str(&c_string(&self.source_file));
            out.push_str("\", ");
            out.push_str(&line.to_string());
            out.push_str(");\n");
        ...
        match reference_target_from_value(argument) {
            ...
            None => {
                if self.source_is_declared_by_ref_call(argument)
                    || matches!(argument, ValueExpr::AssignRef { .. })
                {
