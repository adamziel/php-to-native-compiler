...
impl IncludeCollector {
    ...
    fn resolve_include_candidate(
        ...
        let path_aliases = if matches!(resolved.transform, Some(IncludeSourceTransform::PhpFilter(_))) {
    ...
    fn finalize_sources(&mut self) -> Result<()> {
        for index in 0..self.sources.len() {
            ...
            apply_include_source_transform(&mut source_bytes, self.source_transforms[index].as_ref());
            let source = decode_compiler_source_bytes(&source_bytes, &self.source_options)?;
...
fn apply_include_source_transform(source_bytes: &mut Vec<u8>, transform: Option<&IncludeSourceTransform>) {
