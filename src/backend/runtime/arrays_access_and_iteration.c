...
static PTN_UNUSED PtnValue ptn_new_object(
    ...
    if (ptn_internal_class_name_is_intl_date_formatter(lookup_class_name) ||
        ...
        if (ptn_internal_class_name_is_intl_number_range_formatter(lookup_class_name)) {
            ptn_throw_exception(runtime, "Error", "Call to private IntlNumberRangeFormatter::__construct() from global scope");
            return ptn_null();
        }
...
static PTN_UNUSED PtnValue ptn_clone_value(PtnRuntime *runtime, PtnValue value, size_t line) {
    ...
    if (ptn_internal_class_name_is_intl_list_formatter(source->class_name) ||
        ptn_internal_class_name_is_intl_number_range_formatter(source->class_name)) {
        char message[96];
        int written = snprintf(message, sizeof(message), "Trying to clone an uncloneable object of class %s", source->class_name);
        if (written < 0 || (size_t)written >= sizeof(message)) {
            ptn_abort_out_of_memory();
        }
