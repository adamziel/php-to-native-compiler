...
static PTN_UNUSED void ptn_generator_data_free(void *data) {
    ...
    if (generator->key_is_auto != NULL) {
        ptn_array_free(generator->key_is_auto);
    }
    ...
    if (generator->delegate_sources != NULL) {
        ptn_generator_detach_delegate_sources_iterative(generator);
    ...
    if (generator->send_call_output_chunks != NULL) {
        ptn_array_free(generator->send_call_output_chunks);
    }
    ...
    if (generator->send_yield_slot_source_positions != NULL) {
        ptn_array_free(generator->send_yield_slot_source_positions);
    }
    if (generator->send_yield_slot_target_positions != NULL) {
        ptn_array_free(generator->send_yield_slot_target_positions);
    }
    if (generator->send_yield_slot_kinds != NULL) {
        ptn_array_free(generator->send_yield_slot_kinds);
    }
...
static PTN_UNUSED void ptn_generator_detach_delegate_sources_iterative(PtnGenerator *generator) {
    ...
    size_t stack_len = 0;
    PtnGenerator **stack = malloc(stack_capacity * sizeof(PtnGenerator *));
    if (stack == NULL) {
        ptn_abort_out_of_memory();
    }
    stack[stack_len++] = generator;

    size_t values_capacity = 64;
    size_t values_len = 0;
    PtnValue *values = malloc(values_capacity * sizeof(PtnValue));
    if (values == NULL) {
        free(stack);
        ptn_abort_out_of_memory();
    }
    ...
    while (stack_len != 0) {
        PtnGenerator *current = stack[--stack_len];
        if (current == NULL || current->delegate_sources == NULL) {
            continue;
        }
        for (size_t i = 0; i < current->delegate_sources->len; i++) {
            PtnArrayEntry *entry = &current->delegate_sources->entries[i];
            PtnGenerator *source = ptn_generator_from_value(entry->value);
            if (source == NULL) {
                continue;
            }
            if (values_len == values_capacity) {
                if (values_capacity > SIZE_MAX / 2 ||
                    values_capacity * 2 > SIZE_MAX / sizeof(PtnValue)) {
                    free(values);
                    free(stack);
                    ptn_abort_out_of_memory();
                }
                values_capacity *= 2;
                PtnValue *replacement = realloc(values, values_capacity * sizeof(PtnValue));
                if (replacement == NULL) {
                    free(values);
                    free(stack);
                    ptn_abort_out_of_memory();
            ...
            values[values_len++] = entry->value;
            entry->value = ptn_null();

            if (stack_len == stack_capacity) {
                if (stack_capacity > SIZE_MAX / 2 ||
                    stack_capacity * 2 > SIZE_MAX / sizeof(PtnGenerator *)) {
                    free(values);
                    free(stack);
                    ptn_abort_out_of_memory();
                }
                stack_capacity *= 2;
                PtnGenerator **replacement = realloc(stack, stack_capacity * sizeof(PtnGenerator *));
                if (replacement == NULL) {
                    free(values);
                    free(stack);
                    ptn_abort_out_of_memory();
...
static PTN_UNUSED PtnValue ptn_generator_new(PtnRuntime *runtime, int yields_by_ref) {
    ...
    PtnValue key_is_auto = ptn_array_from_literal_entries(0, NULL);
    ...
    PtnValue send_call_output_chunks = ptn_array_from_literal_entries(0, NULL);
    ...
    PtnValue send_yield_slot_source_positions = ptn_array_from_literal_entries(0, NULL);
    PtnValue send_yield_slot_target_positions = ptn_array_from_literal_entries(0, NULL);
    PtnValue send_yield_slot_kinds = ptn_array_from_literal_entries(0, NULL);
    ...
    generator->key_is_auto = key_is_auto.as.array;
    ...
    generator->send_call_output_chunks = send_call_output_chunks.as.array;
    ...
    generator->send_yield_slot_source_positions = send_yield_slot_source_positions.as.array;
    generator->send_yield_slot_target_positions = send_yield_slot_target_positions.as.array;
    generator->send_yield_slot_kinds = send_yield_slot_kinds.as.array;
...
static PTN_UNUSED int ptn_generator_flush_debug_backtrace_marker(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    const char *data,
    size_t len,
...
static PTN_UNUSED void ptn_generator_flush_output_chunk(
    ...
    if (ptn_generator_flush_debug_backtrace_marker(
            runtime,
            generator,
            (const char *)chunk.as.string.data,
            chunk.as.string.len,
            line
    )) {
...
static PTN_UNUSED PtnValue ptn_generator_yield(
    ...
    if (
        ...
        !ptn_array_append_key_available(runtime, generator->key_is_auto) ||
    ...
    PtnArrayKey key_auto_key = ptn_array_int_key(generator->key_is_auto->next_auto_key);
    ...
    ptn_array_set_entry(generator->key_is_auto, key_auto_key, ptn_int(has_key ? 0 : 1));
...
static PTN_UNUSED PtnArrayIterator ptn_generator_traversable_delegate_iterator(
    PtnRuntime *runtime,
    PtnValue source,
    const char *path,
    size_t line,
...
static PTN_UNUSED int ptn_generator_aborted_without_return(PtnGenerator *generator);
static PTN_UNUSED int ptn_generator_throw_closed_delegate_into_parent(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    size_t line,
...
static PTN_UNUSED int ptn_generator_has_send_call_at_position(PtnGenerator *generator, size_t position) {
    if (generator == NULL || generator->send_call_positions == NULL) {
        return 0;
    }
    for (size_t i = 0; i < generator->send_call_positions->len; i++) {
        PtnValue value = ptn_value_deref(generator->send_call_positions->entries[i].value);
        if (value.type == PTN_INT && value.as.integer >= 0 && (size_t)value.as.integer == position) {
            return 1;
        }
    }
    return 0;
...
static PTN_UNUSED int ptn_generator_skip_exhausted_delegates(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    size_t line
) {
    while (
        ...
        if (traversable_source != NULL) {
            ...
            PtnArrayIterator iterator = ptn_generator_traversable_delegate_iterator(
                runtime,
        ...
        if (source != NULL && !ptn_generator_ensure_initialized(runtime, source, line)) {
            return 0;
        }
        if (
            source != NULL &&
        ...
        if (
            source != NULL &&
            !source->completed &&
            ptn_generator_has_send_call_at_position(source, source->position)
        ) {
            return 1;
        }
        if (
...
static PTN_UNUSED PtnValue ptn_generator_yield_delegate(
    ...
    if (!source_generator->completed) {
        return ptn_null();
    }
    return ptn_generator_get_collected_return(runtime, resolved, line);
}
...
static PTN_UNUSED int ptn_generator_flush_debug_backtrace_marker(
    ...
    static const char marker[] = "\036PTN_DEBUG_PRINT_BACKTRACE\t";
    size_t marker_len = sizeof(marker) - 1;
    if (data == NULL || len < marker_len || memcmp(data, marker, marker_len) != 0) {
        return 0;
    }
    int64_t options = 0;
    int64_t limit = 0;
    size_t debug_line = 0;
    if (sscanf(data + marker_len, "%" SCNd64 "\t%" SCNd64 "\t%zu", &options, &limit, &debug_line) != 3) {
        return 1;
    }
...
static PTN_UNUSED int ptn_generator_take_pending_exception_at_position_with_parent(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    PtnGenerator *resume_parent,
    size_t resume_parent_position,
...
static PTN_UNUSED int ptn_generator_try_throw_catch(
    ...
    for (size_t i = 0; i < generator->throw_catch_positions->len; i++) {
        ...
        if (!flushed_output) {
            ptn_generator_flush_output_chunk(runtime, generator, position, line);
            flushed_output = 1;
        }
        int handled = 0;
...
static PTN_UNUSED PtnValue ptn_generator_closed_delegate_exception(
    PtnRuntime *runtime,
    size_t line
) {
    return ptn_exception_value(ptn_exception_new_owned(
        ...
        ptn_duplicate_string("Generator yielded from aborted, no return value available"),
        strlen("Generator yielded from aborted, no return value available"),
        0,
        ptn_null(),
        PTN_E_ERROR,
        runtime != NULL ? runtime->source_path : NULL,
...
static PTN_UNUSED int ptn_generator_throw_closed_delegate_into_parent(
    ...
    PtnValue receiver = ptn_object(generator->object);
    receiver.owned = 0;
    PtnValue thrown = ptn_generator_closed_delegate_exception(runtime, line);
    PtnValue resolved_thrown = ptn_value_deref(thrown);
    if (resolved_thrown.type == PTN_EXCEPTION) {
        ptn_generator_rewrite_closed_delegate_exception_trace(
            runtime,
            generator,
            resolved_thrown.as.exception,
            line,
            resume_method_name
        );
    }
...
static PTN_UNUSED PtnValue ptn_generator_current_at_position(
    ...
    if (
        ...
        use_delegate_last_value &&
        ptn_generator_aborted_without_return(source_generator)
    ) {
        PtnValue result = ptn_null();
        if (ptn_generator_throw_closed_delegate_into_parent(
                runtime,
...
static PTN_UNUSED PtnValue ptn_generator_get_return(PtnRuntime *runtime, PtnValue receiver, size_t line) {
    ...
    return ptn_generator_get_collected_return(runtime, receiver, line);
}
...
static PTN_UNUSED PtnValue ptn_call_callable(
    PtnRuntime *runtime,
    PtnValue callable,
    size_t argc,
    const PtnValue *args,
    size_t line,
    int from_call_user_func
);
    ...
    if (ptn_generator_position_valid(generator)) {
        ...
        if (delegate_source != NULL) {
            ...
            if (
                ...
                if (ptn_generator_take_pending_exception_at_position_with_parent(
                    ...
                    if (ptn_generator_try_throw_catch(
                        ...
                        ptn_value_destroy(&pending_exception);
                        ptn_value_destroy(&source_receiver);
                        return ptn_generator_restore_resume_method_and_return(
                            runtime,
                            previous_resume_method,
            ...
            if (!delegate_frame_active || setjmp(delegate_frame.jump) == 0) {
                ...
                if (runtime != NULL) {
                    runtime->generator_resume_parent = generator;
                    runtime->generator_resume_parent_position = generator->position;
                }
                ptn_generator_push_delegated_resume(runtime);
            ...
            if (caught_delegate_exception) {
                ...
                ptn_generator_throw_pending_exception_value(
                    ...
                    delegate_exception,
                    runtime != NULL ? runtime->source_path : NULL,
                    source_generator != NULL ? source_generator->source_line : line
                );
                return ptn_generator_restore_resume_method_and_return(runtime, previous_resume_method, ptn_null());
            }
...
#endif
        if (previous_resume_method == NULL) {
            ptn_generator_apply_send_yield_slots(runtime, generator, resume_value);
        }
...
static PTN_UNUSED void ptn_generator_register_send_call_entry(
    ...
    if (
        ...
        generator->send_call_output_chunks == NULL ||
    ...
    if (
        ...
        !ptn_array_append_key_available(runtime, generator->send_call_output_chunks) ||
    ...
    ptn_array_set_entry(
        ...
        key,
        ptn_int((int64_t)(generator->values->len == 0 ? generator->position : generator->values->len - 1))
    );
    ptn_array_set_entry(
    ...
    ptn_array_set_entry(
        generator->send_call_output_chunks,
        ptn_array_int_key(generator->send_call_output_chunks->next_auto_key),
        ptn_generator_take_pending_output(generator)
    );
    ptn_array_set_entry(
...
static PTN_UNUSED void ptn_generator_register_send_yield_slot(
    PtnRuntime *runtime,
    ...
    if (
        generator == NULL ||
        generator->values == NULL ||
        generator->send_yield_slot_source_positions == NULL ||
        generator->send_yield_slot_target_positions == NULL ||
        generator->send_yield_slot_kinds == NULL ||
        generator->values->len <= source_back_offset
    ) {
        return;
    }
    if (
        !ptn_array_append_key_available(runtime, generator->send_yield_slot_source_positions) ||
        !ptn_array_append_key_available(runtime, generator->send_yield_slot_target_positions) ||
        !ptn_array_append_key_available(runtime, generator->send_yield_slot_kinds)
    ) {
        return;
    }
    size_t target_position = generator->values->len - 1;
    size_t source_position = target_position - source_back_offset;
    ptn_array_set_entry(
        generator->send_yield_slot_source_positions,
        ptn_array_int_key(generator->send_yield_slot_source_positions->next_auto_key),
        ptn_int((int64_t)source_position)
    );
    ptn_array_set_entry(
        generator->send_yield_slot_target_positions,
        ptn_array_int_key(generator->send_yield_slot_target_positions->next_auto_key),
        ptn_int((int64_t)target_position)
    );
    ptn_array_set_entry(
        generator->send_yield_slot_kinds,
        ptn_array_int_key(generator->send_yield_slot_kinds->next_auto_key),
        ptn_int(update_key ? 1 : 0)
    );
}

static PTN_UNUSED void ptn_generator_recompute_auto_keys(PtnGenerator *generator) {
    if (
        generator == NULL ||
        generator->keys == NULL ||
        generator->key_is_auto == NULL
    ...
    int64_t next_auto_key = 0;
    size_t len = generator->keys->len < generator->key_is_auto->len
        ? generator->keys->len
        : generator->key_is_auto->len;
    for (size_t i = 0; i < len; i++) {
        PtnValue auto_value = ptn_value_deref(generator->key_is_auto->entries[i].value);
        int is_auto = auto_value.type == PTN_INT && auto_value.as.integer != 0;
        if (is_auto) {
            PtnArrayEntry *entry = &generator->keys->entries[i];
            ptn_value_destroy(&entry->value);
            entry->value = ptn_int(next_auto_key);
            if (next_auto_key < INT64_MAX) {
                next_auto_key++;
            }
            continue;
        }
        PtnValue key = ptn_value_deref(generator->keys->entries[i].value);
        if (
            key.type == PTN_INT &&
            key.as.integer >= next_auto_key &&
            key.as.integer < INT64_MAX
        ) {
            next_auto_key = key.as.integer + 1;
        }
    }
    generator->next_auto_key = next_auto_key;
}

static PTN_UNUSED void ptn_generator_apply_send_yield_slots(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    PtnValue sent_value
) {
    ...
    for (size_t i = 0; i < generator->send_yield_slot_source_positions->len; i++) {
        if (!ptn_generator_entry_matches_position(
                generator->send_yield_slot_source_positions,
                i,
                generator->position
        )) {
        ...
        PtnValue target_value = ptn_value_deref(
            ptn_generator_array_entry_value(generator->send_yield_slot_target_positions, i)
        );
        PtnValue kind_value = ptn_value_deref(
            ptn_generator_array_entry_value(generator->send_yield_slot_kinds, i)
        );
        if (target_value.type != PTN_INT || target_value.as.integer < 0) {
            continue;
        }
        size_t target_position = (size_t)target_value.as.integer;
        PtnArray *slot_array = (
            kind_value.type == PTN_INT &&
            kind_value.as.integer == 1
        ) ? generator->keys : generator->values;
        if (slot_array == NULL || target_position >= slot_array->len) {
            continue;
        }
        PtnArrayEntry *entry = &slot_array->entries[target_position];
        ptn_value_destroy(&entry->value);
        entry->value = ptn_value_clone_deref(sent_value);
        if (
            kind_value.type == PTN_INT &&
            kind_value.as.integer == 1 &&
            generator->key_is_auto != NULL &&
            target_position < generator->key_is_auto->len
        ) {
            PtnArrayEntry *auto_entry = &generator->key_is_auto->entries[target_position];
            ptn_value_destroy(&auto_entry->value);
            auto_entry->value = ptn_int(0);
            ptn_generator_recompute_auto_keys(generator);
        }
        (void)runtime;
    }
}
...
static PTN_UNUSED PtnValue ptn_generator_replay_array_value_yield_expr(
    ...
    size_t line = fallback_line;
    int mode = 0;
    PtnValue static_operand = ptn_null();
    ...
    if (descriptor != NULL && descriptor->len >= 4) {
        PtnValue mode_value = ptn_value_deref(descriptor->entries[2].value);
        if (mode_value.type == PTN_INT) {
            mode = (int)mode_value.as.integer;
        }
        static_operand = ptn_value_deref(descriptor->entries[3].value);
    }
    ...
    if (mode == 1) {
        has_key = 1;
        key = ptn_value_clone_deref(static_operand);
        value = ptn_value_clone_deref(sent_value);
    } else if (mode == 2) {
        has_key = 1;
        key = ptn_value_clone_deref(sent_value);
        value = ptn_value_clone_deref(static_operand);
    } else {
        value = ptn_value_clone_deref(sent_value);
    }
...
static PTN_UNUSED void ptn_generator_mark_send_call_consumed(PtnGenerator *generator, size_t index) {
    if (
        generator == NULL ||
        generator->send_call_positions == NULL ||
        index >= generator->send_call_positions->len
    ) {
        return;
    }
    PtnArrayEntry *entry = &generator->send_call_positions->entries[index];
    ptn_value_destroy(&entry->value);
    entry->value = ptn_int(-1);
}

static PTN_UNUSED void ptn_generator_flush_send_call_output_chunk(
    PtnRuntime *runtime,
    PtnGenerator *generator,
    size_t index
) {
    if (
        generator == NULL ||
        generator->send_call_output_chunks == NULL ||
        index >= generator->send_call_output_chunks->len
    ) {
        return;
    }
    PtnArrayEntry *entry = &generator->send_call_output_chunks->entries[index];
    PtnValue chunk = ptn_value_deref(entry->value);
    if (chunk.type == PTN_STRING && chunk.as.string.len > 0) {
        PtnGenerator *saved_current_generator = runtime == NULL ? NULL : runtime->current_generator;
        if (runtime != NULL && saved_current_generator == generator) {
...
static PTN_UNUSED int ptn_generator_replay_send_calls(
    ...
    for (size_t i = 0; i < generator->send_call_positions->len; i++) {
        ...
        if (runtime != NULL) {
            runtime->replaying_generator_send_call = 1;
        }
...
static PTN_UNUSED PtnValue ptn_generator_send(PtnRuntime *runtime, PtnValue receiver, PtnValue sent_value, size_t line) {
    ...
    ptn_generator_apply_send_yield_slots(runtime, generator, sent_value);
...
static PTN_UNUSED PtnValue ptn_generator_rewind(PtnRuntime *runtime, PtnValue receiver, size_t line) {
    ...
    if (generator != NULL) {
        ...
        if (generator->pending_exception_on_rewind) {
            generator->pending_exception_on_rewind = 0;
            ptn_generator_flush_output_chunk(runtime, generator, generator->pending_exception_position, line);
            if (ptn_generator_throw_pending_exception_at_position(
                    runtime,
                    generator,
                    generator->pending_exception_position,
...
static PTN_UNUSED void ptn_array_unpack_iterator_into(
    ...
    while (iterator->valid) {
        ...
        PtnValue resolved_key = ptn_value_deref(key_value);
        if (
            resolved_key.type != PTN_INT &&
            resolved_key.type != PTN_STRING &&
            iterator->generator != NULL &&
            iterator->generator->has_pending_exception &&
            iterator->generator->pending_exception_position == iterator->index
        ) {
...
static PTN_UNUSED int ptn_arrayaccess_append_reference_temporary(
    ...
    if (
        ...
        (ptn_ascii_case_equal(value.as.object->class_name, "DOMNodeList") ||
            ...
            ptn_ascii_case_equal(value.as.object->class_name, "Dom\\HTMLCollection") ||
...
static PTN_UNUSED PtnArrayIterator ptn_array_iterator_from_traversable_object(
    ...
    if (
        ...
        return ptn_array_iterator_from_protocol_iterator(runtime, value, access_scope, line, 1);
    }
...
static PTN_UNUSED int ptn_value_is_protocol_iterator(
    PtnRuntime *runtime,
    PtnValue value
) {
    value = ptn_value_deref(value);
...
static PTN_UNUSED PtnArrayIterator ptn_generator_traversable_delegate_iterator(
    ...
    PtnValue resolved = ptn_value_deref(source);
    if (ptn_value_is_protocol_iterator(runtime, resolved)) {
        return ptn_array_iterator_from_protocol_iterator(runtime, resolved, NULL, line, rewind);
    }
    return ptn_array_iterator_from_value(runtime, resolved, NULL, path, line);
}
...
static PTN_UNUSED PtnValue ptn_generator_yield_from(
    ...
    if (
        ...
        if (ptn_value_is_protocol_iterator(runtime, resolved)) {
            ...
            int delegate_valid = iterator.valid;
            ptn_array_iterator_destroy(&iterator);
            if (!delegate_valid) {
                return ptn_null();
            }
        }
        if (ptn_generator_append_delegate_entry(runtime, runtime->current_generator, resolved, line)) {
            return ptn_null();
        }
    }
...
static PTN_UNUSED PtnValue ptn_array_iterator_current_key(PtnArrayIterator *iterator) {
    ...
    if (iterator->generator != NULL) {
        ptn_generator_flush_output_chunk(iterator->runtime, iterator->generator, iterator->index, iterator->line);
        if (ptn_generator_throw_pending_delegate_exception_at_position(
                iterator->runtime,
                iterator->generator,
                iterator->index,
        ...
        PtnValue *traversable_source =
            ptn_generator_traversable_delegate_source_value(iterator->generator, iterator->index);
        if (traversable_source != NULL) {
            PtnValue source_receiver = ptn_value_clone_deref(*traversable_source);
            PtnArrayIterator delegate_iterator = ptn_generator_traversable_delegate_iterator(
                iterator->runtime,
                ptn_value_deref(source_receiver),
                iterator->runtime != NULL ? iterator->runtime->source_path : NULL,
                iterator->line,
                0
            );
            PtnValue key = delegate_iterator.valid
                ? ptn_array_iterator_current_key(&delegate_iterator)
                : ptn_null();
            PtnValue result = ptn_value_clone_deref(key);
            ptn_value_destroy(&key);
            ptn_array_iterator_destroy(&delegate_iterator);
            ptn_value_destroy(&source_receiver);
            return result;
        }
    }
...
static PTN_UNUSED int ptn_array_iterator_generator_delegate_current_value_iterative(
    PtnArrayIterator *iterator,
    PtnValue *result_out
) {
    if (
    ...
    size_t previous_resume_parent_position =
        runtime == NULL ? 0 : runtime->generator_resume_parent_position;
#define PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT() do { \
        if (runtime != NULL) { \
            runtime->generator_resume_parent = previous_resume_parent; \
            runtime->generator_resume_parent_position = previous_resume_parent_position; \
        } \
    } while (0)
    PtnGenerator *generator = iterator->generator;
    size_t index = iterator->index;
    for (size_t depth = 0; depth < 1000000; depth++) {
        ptn_generator_flush_output_chunk(runtime, generator, index, iterator->line);
        if (ptn_generator_throw_pending_delegate_exception_at_position(
                runtime,
                generator,
                index,
            ...
            *result_out = ptn_null();
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 1;
        }
        PtnValue *delegate_source = ptn_generator_delegate_source_value(generator, index);
        if (delegate_source == NULL) {
            ptn_generator_emit_pending_reference_notice(runtime, generator, index);
            if (
                generator->values == NULL ||
                index >= generator->values->len
            ) {
                *result_out = ptn_null();
            } else {
                *result_out = ptn_value_clone_deref(generator->values->entries[index].value);
            }
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 1;
        }
        ...
        PtnValue source_receiver = ptn_value_clone_deref(*delegate_source);
        PtnGenerator *source = ptn_generator_from_value(source_receiver);
        if (source == NULL) {
            ptn_value_destroy(&source_receiver);
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 0;
        ...
        if (runtime != NULL) {
            runtime->generator_resume_parent = generator;
            runtime->generator_resume_parent_position = index;
        }
        if (!ptn_generator_ensure_initialized(runtime, source, iterator->line)) {
            ptn_value_destroy(&source_receiver);
            *result_out = ptn_null();
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 1;
        }
        if (!ptn_generator_skip_exhausted_delegates(runtime, source, iterator->line)) {
            ptn_value_destroy(&source_receiver);
            *result_out = ptn_null();
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 1;
        ...
        source->started = 1;
        if (!ptn_generator_position_valid(source)) {
            ptn_value_destroy(&source_receiver);
            *result_out = ptn_null();
            PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
            return 1;
    ...
    PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT();
#undef PTN_RESTORE_ITERATIVE_GENERATOR_RESUME_PARENT
    return 0;
}
...
static PTN_UNUSED PtnValue ptn_array_iterator_current_value(PtnArrayIterator *iterator) {
    ...
    if (iterator->generator != NULL) {
        ...
        PtnValue *delegate_source =
            ptn_generator_delegate_source_value(iterator->generator, iterator->index);
        if (delegate_source != NULL) {
            PtnValue iterative = ptn_null();
            if (ptn_array_iterator_generator_delegate_current_value_iterative(iterator, &iterative)) {
                return iterative;
            }
            PtnValue source_receiver = ptn_value_clone_deref(*delegate_source);
            PtnGenerator *previous_resume_parent =
        ...
        if (traversable_source != NULL) {
            ...
            PtnValue current = delegate_iterator.valid
                ? ptn_array_iterator_current_value(&delegate_iterator)
                : ptn_null();
            PtnValue result = ptn_value_clone_deref(current);
            ptn_value_destroy(&current);
            ptn_array_iterator_destroy(&delegate_iterator);
...
static PTN_UNUSED int ptn_array_iterator_generator_advance_iterative(PtnArrayIterator *iterator) {
    if (iterator == NULL || iterator->generator == NULL) {
        return 0;
    }
    size_t capacity = 64;
    size_t len = 0;
    ...
    for (size_t depth = 0; depth < 1000000; depth++) {
        if (len == capacity) {
            if (capacity > SIZE_MAX / 2 || capacity * 2 > SIZE_MAX / sizeof(PtnGenerator *)) {
                free(chain);
                ptn_abort_out_of_memory();
            }
            capacity *= 2;
            PtnGenerator **replacement = realloc(chain, capacity * sizeof(PtnGenerator *));
            if (replacement == NULL) {
                free(chain);
                ptn_abort_out_of_memory();
            }
    ...
    PtnGenerator *leaf = chain[len - 1];
    PtnObject *leaf_object = leaf->object;
    PtnValue receiver = leaf_object == NULL
        ? ptn_null()
        : ptn_value_borrow(ptn_object(leaf_object));
    PtnValue advanced = ptn_generator_next(runtime, receiver, iterator->line);
    ptn_value_destroy(&advanced);

    for (size_t i = len; i > 0; i--) {
        PtnGenerator *current = chain[i - 1];
        if (!ptn_generator_skip_exhausted_delegates(runtime, current, iterator->line)) {
            break;
        }
    }
    free(chain);
...
static PTN_UNUSED void ptn_array_iterator_advance(PtnArrayIterator *iterator) {
    ...
    if (iterator->generator != NULL) {
        if (!ptn_array_iterator_generator_advance_iterative(iterator)) {
            PtnObject *generator_object = iterator->object != NULL
                ? iterator->object
                : iterator->generator->object;
            PtnValue receiver = generator_object == NULL
                ? ptn_null()
                : ptn_value_borrow(ptn_object(generator_object));
            PtnValue advanced = ptn_generator_next(iterator->runtime, receiver, iterator->line);
            ptn_value_destroy(&advanced);
        }
        ptn_array_iterator_clear_current_key(iterator);
        iterator->index = iterator->generator->position;
