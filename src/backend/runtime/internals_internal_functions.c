...
static PtnArrayKey ptn_array_stringified_key_from_value(PtnRuntime *runtime, PtnValue value, size_t line) {
    value = ptn_value_deref(value);
    if (value.type == PTN_ARRAY) {
...
static PtnValue ptn_internal_array_combine(PtnRuntime *runtime, size_t argc, const PtnValue *args, size_t line) {
    ...
    for (size_t i = 0; i < keys->len; i++) {
        PtnArrayKey key = ptn_array_stringified_key_from_value(runtime, keys->entries[i].value, line);
