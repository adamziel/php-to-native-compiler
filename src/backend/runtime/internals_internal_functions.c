...
static void ptn_compact_emit_undefined_variable_warning(
    ...
    int needed = snprintf(NULL, 0, "compact(): Undefined variable $%s", name);
    if (needed < 0) {
        ptn_abort_out_of_memory();
    }
    char *message = malloc((size_t)needed + 1);
    ...
    snprintf(message, (size_t)needed + 1, "compact(): Undefined variable $%s", name);
    ptn_emit_warning(&runtime->diagnostics, message, line);
    free(message);
}
...
static int ptn_compact_collect_value(
    ...
    if (value.type == PTN_STRING) {
        char *name = ptn_duplicate_string_len((const char *)value.as.string.data, value.as.string.len);
        PtnLookupResult lookup = ptn_runtime_read_variable_quiet(runtime, name);
        if (lookup.exists) {
            ptn_array_set_entry(
                result,
                ptn_array_string_key_len((const char *)value.as.string.data, value.as.string.len),
