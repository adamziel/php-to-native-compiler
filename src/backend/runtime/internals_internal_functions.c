...
static PtnValue ptn_internal_dirname(PtnRuntime *runtime, size_t argc, const PtnValue *args, size_t line) {
    ...
    int64_t levels = argc >= 2 ? ptn_value_to_integer(args[1]) : 1;
    if (levels < 1) {
        ptn_string_operand_free(path);
        ptn_throw_exception(
            runtime,
    ...
    char *current = ptn_duplicate_string_len(path.data, path.len);
    size_t current_len = path.len;
    ptn_string_operand_free(path);
    for (int64_t level = 0; level < levels; level++) {
        size_t dirname_len = 0;
        char *dirname = ptn_dirname_string(current, current_len, &dirname_len);
        free(current);
        current = dirname;
        current_len = dirname_len;
        if (current_len == 1 && ptn_is_path_separator(current[0])) {
            break;
        }
        if (current_len == 1 && current[0] == '.') {
            break;
        }
    }
    return ptn_owned_string_len(current, current_len);
}
