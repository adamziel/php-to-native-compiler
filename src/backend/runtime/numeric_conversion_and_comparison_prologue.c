...
static PTN_UNUSED PtnValue ptn_runtime_read_class_constant_impl(
    ...
    const char *interval_boundary_case = ptn_ascii_case_equal(resolved_class_name, "Random\\IntervalBoundary")
        ? ptn_random_interval_boundary_case_name(constant)
        : NULL;
    if (interval_boundary_case != NULL) {
        return ptn_enum_case(runtime, "Random\\IntervalBoundary", interval_boundary_case);
    }
...
static PTN_UNUSED PtnValue ptn_call_method(
    ...
    if (
        receiver.type == PTN_OBJECT
        && ptn_internal_class_name_is_randomizer(receiver.as.object->class_name)
        && ptn_internal_class_method_exists(receiver.as.object->class_name, name)
    ) {
        return ptn_randomizer_call_method(runtime, receiver, name, argc, args, line);
    }
    if (
        receiver.type == PTN_OBJECT
        && ptn_internal_class_name_is_random_engine_builtin(receiver.as.object->class_name)
        && ptn_internal_class_method_exists(receiver.as.object->class_name, name)
    ) {
        return ptn_random_engine_call_method(runtime, receiver, name, argc, args, line);
    }
    if (
        receiver.type == PTN_OBJECT
