...
#endif
#if defined(LC_PAPER)
    if (strcmp(name, "LC_PAPER") == 0) {
        *out = ptn_int(LC_PAPER);
        return 1;
    }
#endif
#if defined(LC_NAME)
    if (strcmp(name, "LC_NAME") == 0) {
        *out = ptn_int(LC_NAME);
        return 1;
    }
#endif
#if defined(LC_ADDRESS)
    if (strcmp(name, "LC_ADDRESS") == 0) {
        *out = ptn_int(LC_ADDRESS);
        return 1;
    }
#endif
#if defined(LC_TELEPHONE)
    if (strcmp(name, "LC_TELEPHONE") == 0) {
        *out = ptn_int(LC_TELEPHONE);
        return 1;
    }
#endif
#if defined(LC_MEASUREMENT)
    if (strcmp(name, "LC_MEASUREMENT") == 0) {
        *out = ptn_int(LC_MEASUREMENT);
        return 1;
    }
#endif
#if defined(LC_IDENTIFICATION)
    if (strcmp(name, "LC_IDENTIFICATION") == 0) {
        *out = ptn_int(LC_IDENTIFICATION);
        return 1;
    }
#endif
