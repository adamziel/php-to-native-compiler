...
$autoload = $wpdb->prepare_two(
    'SELECT `option_value` FROM `' . $wpdb->options . '` WHERE `autoload` IN (?, ?)',
    'yes',
    'on'
);
echo 'autoload_values=' . implode(',', $wpdb->get_col_prepared($autoload));

echo '|';
$named = $wpdb->get_col("SELECT option_value FROM wp_options WHERE option_name IN ('theme_mods','missing','home')");
echo 'named_values=' . implode(',', $named);

echo '|';
$all = $wpdb->get_col_prepared_query('SELECT option_value FROM wp_options');
echo 'all_values_prepared=' . implode(',', $all);
