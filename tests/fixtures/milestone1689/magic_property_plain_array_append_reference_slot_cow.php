...
class Milestone1689_MagicPlainArrayAppendBox {
...
$box = new Milestone1689_MagicPlainArrayAppendBox();
$box->missing[] = [
    "id" => $node,
...
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-magic-array-append";
        $callback["label"] = "via-magic-array-label";
