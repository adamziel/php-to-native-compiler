<?php
class Milestone1693_MagicTwoKeyNestedAppendBag implements ArrayAccess {
...
class Milestone1693_MagicTwoKeyNestedAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$bag = new Milestone1693_MagicTwoKeyNestedAppendBag();
$box = new Milestone1693_MagicTwoKeyNestedAppendBox($bag);
...
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-magic-arrayaccess-two-key-append";
        $callback["label"] = "via-magic-arrayaccess-two-key-label";
