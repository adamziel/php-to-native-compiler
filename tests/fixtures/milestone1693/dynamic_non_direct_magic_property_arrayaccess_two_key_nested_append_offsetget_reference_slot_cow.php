<?php
class Milestone1693_DynamicNonDirectMagicTwoKeyNestedAppendBag implements ArrayAccess {
...
class Milestone1693_DynamicNonDirectMagicTwoKeyNestedAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$bag = new Milestone1693_DynamicNonDirectMagicTwoKeyNestedAppendBag();
$holders = ["box" => new Milestone1693_DynamicNonDirectMagicTwoKeyNestedAppendBox($bag)];
$property = "missing";
...
$bucket = $holders["box"]->{$property}["outer"]["inner"][0];
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-dynamic-non-direct-magic-arrayaccess-two-key-append";
        $callback["label"] = "via-dynamic-non-direct-magic-arrayaccess-two-key-label";
