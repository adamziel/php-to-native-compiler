<?php
class Milestone1693_NonDirectMagicTwoKeyNestedAppendBag implements ArrayAccess {
...
class Milestone1693_NonDirectMagicTwoKeyNestedAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$bag = new Milestone1693_NonDirectMagicTwoKeyNestedAppendBag();
$holders = ["box" => new Milestone1693_NonDirectMagicTwoKeyNestedAppendBox($bag)];
...
$bucket = $holders["box"]->missing["outer"]["inner"][0];
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-non-direct-magic-arrayaccess-two-key-append";
        $callback["label"] = "via-non-direct-magic-arrayaccess-two-key-label";
