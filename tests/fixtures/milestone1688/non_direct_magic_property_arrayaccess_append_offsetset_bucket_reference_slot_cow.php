<?php
class Milestone1688_NonDirectMagicAppendStoredBucket implements ArrayAccess {
    private $items = [];
...
class Milestone1688_NonDirectMagicAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$holders = ["box" => new Milestone1688_NonDirectMagicAppendBox(new Milestone1688_NonDirectMagicAppendStoredBucket())];
$holders["box"]->missing[] = [
    "id" => $node,
...
$bucket = $holders["box"]->missing[0];
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-non-direct-magic-append";
        $callback["label"] = "via-non-direct-magic-label";
