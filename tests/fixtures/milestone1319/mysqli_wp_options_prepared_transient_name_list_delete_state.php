...
class wpdb {
    ...
    public function prepare($query, $one, $two) {
        return array($query, array($one, $two));
    }

    public function query_prepared($prepared) {
...
function delete_transient_prepared_probe($transient) {
    global $wpdb;
    ...
    $result = $wpdb->query_prepared($prepared);
...
echo '|';
echo delete_transient_prepared_probe('feed_mod') ? 'deleted' : 'failed';
echo ':';
