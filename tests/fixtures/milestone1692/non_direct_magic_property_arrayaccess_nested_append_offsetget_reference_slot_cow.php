<?php
class Milestone1692_NonDirectMagicNestedAppendBag implements ArrayAccess {
    public $items = ["outer" => []];
...
class Milestone1692_NonDirectMagicNestedAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$bag = new Milestone1692_NonDirectMagicNestedAppendBag();
$holders = ["box" => new Milestone1692_NonDirectMagicNestedAppendBox($bag)];
...
$bucket = $holders["box"]->missing["outer"][0];
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-non-direct-magic-arrayaccess-nested-append";
        $callback["label"] = "via-non-direct-magic-arrayaccess-nested-label";
