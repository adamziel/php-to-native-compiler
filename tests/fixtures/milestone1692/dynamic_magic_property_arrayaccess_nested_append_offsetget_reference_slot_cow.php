<?php
class Milestone1692_DynamicMagicNestedAppendBag implements ArrayAccess {
    public $items = ["outer" => []];
...
class Milestone1692_DynamicMagicNestedAppendBox {
    public $gets = 0;
    public $sets = 0;
    private $store;
...
$bag = new Milestone1692_DynamicMagicNestedAppendBag();
$box = new Milestone1692_DynamicMagicNestedAppendBox($bag);
$property = "missing";
$box->{$property}["outer"][] = [
    "id" => $node,
...
$bucket = $box->{$property}["outer"][0];
foreach ($bucket as $id => &$callback) {
    if ($id === "id") {
        $callback["function"] = "via-dynamic-magic-arrayaccess-nested-append";
        $callback["label"] = "via-dynamic-magic-arrayaccess-nested-label";
