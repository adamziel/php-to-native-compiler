...
fn compile_setlocale_constants_and_candidates_to_native_binary() {
    let root = temp_dir("ptn-native-setlocale-constants");
    fs::create_dir_all(&root).unwrap();
    let input = root.join("setlocale-constants.php");
    let output = root.join("setlocale-constants-bin");
    fs::write(
        &input,
        "<?php\n\
var_dump(function_exists('setlocale'));\n\
var_dump(defined('LC_ALL'), defined('LC_NUMERIC'), defined('LC_CTYPE'));\n\
var_dump(setlocale(LC_ALL, 'C'));\n\
var_dump(setlocale(LC_NUMERIC, 0));\n\
var_dump(setlocale(999, 'C'));\n\
var_dump(setlocale(LC_ALL, ['definitely_not_a_ptn_locale', 'C']));\n\
