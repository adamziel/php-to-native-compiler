    ...
    ptn_phpt_section "$path" FILE | LC_ALL=C awk -v ptn_path="$path" -v ptn_has_override_attribute="$ptn_has_override_attribute" -v ptn_reflection_property_simple_metadata_row="$ptn_reflection_property_simple_metadata_row" -v ptn_reflection_property_typed_metadata_row="$ptn_reflection_property_typed_metadata_row" '
        ...
        function ptn_supported_property_hook_metadata_row() {
            return ptn_path ~ /Zend\/tests\/asymmetric_visibility\/gh19044[.]phpt$/ ||
                ...
                ptn_path ~ /Zend\/tests\/property_hooks\/(abstract_get_set_readonly|abstract_hook(_in_non_abstract_class|_not_implemented)?|abstract_prop_(final|hooks|not_implemented|plain|without_hooks)|backed_invariant|bug00[45]|default_on_virtual(_with_inheritance)?|duplicate_hook|explicit_iter|final|final_private_prop|final_prop(_2|_final_hook|_promoted_[1234])?|get_by_ref_implemented_by_plain|gh15419_[12]|interface(_explicit_abstract|_final_hook|_final_prop|_get_by_ref_(backed|plain)|_get_only(_readonly)?|_get_set_readonly|_invalid_explicitly_abstract|_not_implemented|_not_public|_set_only)?|invalid_(abstract_(body|final|indirect(_2)?|private)|empty_hooks|final_private|hook_visibility|static|static_prop)|no_default_value_untyped_002|no_get_parameters|parent_get_not_in_class|parent_outside_property|private_prop_final_hook|readonly|set_by_ref|set_variadic|syntax|traits_abstract|unknown_hook(_private)?)[.]phpt$/ ||
