...
write_prompt() {
...

  {
    ...
    printf 'Use four coordinated work lanes for planning and handoff, while still running heavyweight commands serially:\n\n'
    printf -- '1. Runtime lane: migrate one bounded runtime/COW/lvalue-handle behavior slice.\n'
    printf -- '2. Regression lane: add or preserve focused fixtures/unit tests for the touched behavior and adjacent guards.\n'
    printf -- '3. Docs lane: update `docs/PROGRESS.md`, `docs/SUPPORT.md`, `docs/ARCHITECTURE.md`, `docs/NEXT_TASKS.md`, and `docs/LOOP_MEMORY.md` accurately.\n'
    printf -- '4. Gate lane: run focused checks first, then the full checkpoint gate only after the first three lanes are coherent.\n\n'
