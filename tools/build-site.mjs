...
const taskMeta = new Map([
  ...
  ['Array/lvalue execution', ['array-lvalue-execution', ['array', 'lvalue', 'runtime-handles', 'callback-containers', 'dynamic-holders'], ['source/progress.html', 'source/cow-coverage-matrix.html', 'source/next-tasks.html']]],
  ['References/COW', ['references-cow', ['reference', 'cow', 'runtime-handles', 'dynamic-holders'], ['source/progress.html', 'source/cow-coverage-matrix.html']]],
  ...
  ['Symbols/request/globals', ['symbols-request-globals', ['symbol', 'global', 'request', 'link-symbol'], ['source/progress.html', 'source/support.html', 'source/architecture.html']]],
  ...
  ['Control flow/cleanup/diagnostics', ['control-flow-cleanup-diagnostics', ['diagnostic', 'error', 'control-flow', 'exit', 'backlog'], ['source/progress.html', 'source/support.html']]],
  ['Calls/functions', ['calls-functions', ['call', 'function-frame'], ['source/progress.html', 'source/architecture.html', 'source/support.html']]],
  ...
  ['Broad verification', ['broad-verification', ['regression', 'evaluator', 'backlog', 'integration'], ['source/progress.html', 'source/roadmap.html', 'source/cow-coverage-matrix.html']]],
]);

for (const rel of ['assets', 'lanes', 'source', 'subtasks', 'index.html', 'progress.html', '404.html', '.nojekyll', 'README.md']) {
...
function barPercent(value, fallback = null) {
  const match = /\[([#-]+)\]/.exec(String(value));
  if (!match) return fallback;
  const filled = [...match[1]].filter((ch) => ch === '#').length;
  return match[1].length ? Math.round((filled / match[1].length) * 100) : fallback;
}

function visualProgress(value, label = '') {
  const pct = Math.max(0, Math.min(100, Math.round(Number(value) || 0)));
  return `<div class="visual-progress"${label ? ` aria-label="${esc(label)}"` : ''}><div><span>${label ? esc(label) : 'Progress'}</span><strong>${pct}%</strong></div><progress max="100" value="${pct}">${pct}%</progress></div>`;
}

function segmentedBar(value, fallback = null) {
  const match = /\[([#-]+)\]/.exec(String(value));
  const pct = barPercent(value, fallback) ?? 0;
  if (!match) return visualProgress(pct);
  const cells = [...match[1]]
    .map((ch) => `<span class="${ch === '#' ? 'filled' : ''}"></span>`)
    .join('');
  return `<div class="segmented-bar" aria-label="${pct}% complete"><div class="segments">${cells}</div><strong>${pct}%</strong></div>`;
}

function renderTableCell(cell, header, row, head) {
  const lower = header.toLowerCase();
  const estimateIndex = head.findIndex((h) => h.toLowerCase().includes('estimate'));
  const fallback = estimateIndex >= 0 ? percentFrom(row[estimateIndex]) : null;
  const pct = percentFrom(cell);
  if (lower === 'bar' && /\[[#-]+\]/.test(cell)) {
    return segmentedBar(cell, fallback);
  }
  if (pct !== null && /^\s*\d+(?:\.\d+)?\s*%\s*$/.test(cell) && (lower.includes('estimate') || lower.includes('progress') || lower.includes('primary') || lower.includes('lane'))) {
    return visualProgress(pct, header);
  }
  return inline(cell);
}
...
function table(lines) {
  ...
  return `<div class="table-wrap"><table><thead><tr>${head.map((cell) => `<th>${inline(cell)}</th>`).join('')}</tr></thead><tbody>${body.map((row) => `<tr>${row.map((cell, idx) => `<td>${renderTableCell(cell, head[idx] || '', row, head)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
}
...
function labeledBlock(md, label) {
...
function statusBars(md) {
  const roadmap = parseTableRows(section(md, 'Roadmap Position'));
  if (roadmap.rows.length) {
    const area = roadmap.head.findIndex((cell) => /^area$/i.test(cell));
    const estimate = roadmap.head.findIndex((cell) => /estimate/i.test(cell));
    return roadmap.rows
      .map((row) => [row[area >= 0 ? area : 0], percentFrom(row[estimate >= 0 ? estimate : 1])])
      .filter((row) => row[0] && row[1] !== null);
  }
  const block = /```([\s\S]*?)```/.exec(section(md, 'Overall Status'))?.[1] || '';
  return block
    .split('\n')
    .map((line) => /^(.+?)\s+\[[#-]+\]\s+(\d+)%/.exec(line))
    .filter(Boolean)
    .map((m) => [m[1].trim(), Number(m[2])]);
}

function activeRows(md) {
  const current = parseTableRows(section(md, 'Active Roadmap Items'));
  if (current.rows.length) {
    const itemIndex = current.head.findIndex((cell) => /active item/i.test(cell));
    const estimateIndex = current.head.findIndex((cell) => /estimate/i.test(cell));
    const statusIndex = current.head.findIndex((cell) => /status/i.test(cell));
    const nextIndex = current.head.findIndex((cell) => /next/i.test(cell));
    return current.rows.map((cells) => {
      const item = cells[itemIndex >= 0 ? itemIndex : 0];
      const meta = taskMeta.get(item);
      return {
        item,
        primary: percentFrom(cells[estimateIndex >= 0 ? estimateIndex : 1]) ?? 0,
        lane: null,
        status: cells[statusIndex >= 0 ? statusIndex : 2] || '',
        read: cells[nextIndex >= 0 ? nextIndex : 3] || '',
        slug: meta?.[0] || slug(item),
        keywords: meta?.[1] || [],
        docs: meta?.[2] || ['source/progress.html'],
      };
    });
  }
  const legacy = parseTableRows(section(md, 'Active Roadmap Estimates'));
  return legacy.rows.map((cells) => {
    const meta = taskMeta.get(cells[0]);
    return {
      item: cells[0],
      primary: percentFrom(cells[1]) ?? 0,
      lane: percentFrom(cells[2]) ?? null,
      status: '',
      read: cells[3],
...
function checklist(md) {
  return section(md, 'Done / In Progress / Not Done')
    ...
    .map((m) => {
      const text = m[2].trim();
      let state = m[1].toLowerCase() === 'x' ? 'done' : 'not-done';
      if (m[1] === '~' || m[1] === '-') state = 'in-progress';
...
function recent(md) {
  const simple = section(md, 'Recent Primary Progress')
    .split('\n')
    .map((line) => /^-\s+`([0-9a-f]+)`\s+(.+)$/.exec(line))
    .filter(Boolean)
    .map((m) => ({ hash: m[1], title: m[2].trim(), detail: '' }));
...
const overall = percentFrom(/Overall estimated progress[^\n]*\*\*(\d+(?:\.\d+)?)%\*\*/.exec(progress)?.[0] || '') ?? 0;
...
const activeInProgress = rows.filter((row) => /in progress/i.test(row.status || '')).length;
const primary = bullets(section(progress, 'Current Primary State'), 8).concat(bullets(labeledBlock(progress, 'Current product repo state'), 8)).slice(0, 8);
const candidates = bullets(section(progress, 'Lane-Local Candidate Work'), 8).concat(bullets(section(progress, 'Current Lane-Local Candidate Work Not Yet Counted'), 8)).slice(0, 8);
const steeringBlock = section(progress, 'Steering Notes') || section(progress, 'Near-Term Steering');
const steering = steeringBlock.split('\n').map((line) => /^\d+\.\s+(.*)$/.exec(line)).filter(Boolean).map((m) => m[1]);
const steeringHtml = steering.length ? `<ol>${steering.map((x) => `<li>${inline(x)}</li>`).join('\n')}</ol>` : markdown(steeringBlock, { shift: 1 });
...
const roadmapHtml = rows.map((row) => {
  ...
  const tags = [`<span class="tag ${cls(row.primary)}">primary ${row.primary}%</span>`];
  if (row.lane !== null) tags.push(`<span class="tag ${cls(row.lane)}">lane ${row.lane}%</span>`);
  if (row.status) tags.push(`<span class="tag ${cls(row.primary)}">${esc(row.status)}</span>`);
  return `<tr data-filter-row><td><a href="subtasks/${row.slug}.html"><strong>${esc(row.item)}</strong></a><div>${tags.join(' ')}</div></td><td>${mini(row.primary, 'Primary')}${row.lane !== null ? mini(row.lane, 'Lane-local') : ''}</td><td>${inline(row.read)}</td><td>${laneLinks}</td></tr>`;
}).join('\n');
...
<section class="hero">
  ...
  <aside class="panel overall"><div><span class="eyebrow">Overall estimate</span><span class="number">${overall}%</span><progress max="100" value="${overall}">${overall}%</progress></div><div class="stats"><div class="metric"><span>Checklist done</span><strong>${counts.done || 0}</strong></div><div class="metric"><span>Active in progress</span><strong>${activeInProgress || counts['in-progress'] || 0}</strong></div><div class="metric"><span>Checklist not done</span><strong>${counts['not-done'] || 0}</strong></div><div class="metric"><span>Lane reports</span><strong>${lanes.length}</strong></div></div></aside>
</section>
<section class="panel"><h2>Capability Snapshot</h2>${bars.map(([label, value]) => meter(value, label)).join('\n')}</section>
<section><div class="toolbar"><h2 id="roadmap">Active Roadmap Items</h2><input class="search" type="search" placeholder="Filter roadmap" data-filter="#roadmap-table"></div><div class="table-wrap"><table id="roadmap-table" class="roadmap"><thead><tr><th>Sub-task</th><th>Progress</th><th>Current read</th><th>Related lane pages</th></tr></thead><tbody>${roadmapHtml}</tbody></table></div></section>
<section><h2>Current Primary State</h2><details open><summary>Integrated baseline and resource notes</summary><ul>${primary.map((x) => `<li>${inline(x)}</li>`).join('\n')}</ul></details><details><summary>Done, in progress, and not done</summary><ul>${checks.map((x) => `<li><span class="tag ${x.state === 'done' ? 'good' : x.state === 'in-progress' ? 'mixed' : 'early'}">${esc(x.state)}</span> ${inline(x.text)}</li>`).join('\n')}</ul></details><details><summary>Lane-local candidates not yet counted</summary><ul>${candidates.map((x) => `<li>${inline(x)}</li>`).join('\n')}</ul></details><details><summary>Near-term steering</summary>${steeringHtml}</details><div class="toolbar"><button type="button" data-expand-all>Expand all sections</button><button type="button" data-collapse-all>Collapse all sections</button></div></section>
<section><h2>Sub-task Documents</h2><div class="tasks">${rows.map((row) => `<article class="card"><h3><a href="subtasks/${row.slug}.html">${esc(row.item)}</a></h3>${mini(row.primary, 'Primary')}${row.lane !== null ? mini(row.lane, 'Lane-local') : ''}</article>`).join('\n')}</div></section>
<section><h2>Recent Primary-Integrated Work</h2>${commits.map((c, i) => `<details ${i < 2 ? 'open' : ''}><summary><a href="${REPO}/commit/${c.hash}"><code>${esc(c.hash)}</code></a> ${esc(c.title)}</summary>${c.detail ? `<p>${inline(c.detail)}</p>` : ''}</details>`).join('\n')}</section>
...
write('progress.html', shell('PHP Native Compiler Progress', index));
...
const canonicalProgressVisual = `<h2>Visual Progress</h2><div class="tasks">${bars.map(([label, value]) => `<article class="card"><h3>${esc(label)}</h3>${visualProgress(value, 'Estimate')}</article>`).join('\n')}</div><h2>Active Work Bars</h2><div class="tasks">${rows.map((row) => `<article class="card"><h3>${esc(row.item)}</h3>${mini(row.primary, 'Primary')}${row.status ? `<span class="tag ${cls(row.primary)}">${esc(row.status)}</span>` : ''}</article>`).join('\n')}</div>`;

for (const [rel, out, title, maxLines] of sourceDocs) {
  const body = `<section class="source"><p><a href="../index.html">Back to dashboard</a> | <a href="${REPO}/blob/master/${rel}">Open on GitHub</a></p><span class="eyebrow">Source document</span><h1>${esc(title)}</h1>${rel === 'PROGRESS.md' ? canonicalProgressVisual : ''}${markdown(src(rel), { shift: 1, maxLines })}</section>`;
