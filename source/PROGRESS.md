...
Updated: 2026-05-22 14:13 CEST
Evaluation marker: `20260522T114515Z`
Primary HEAD: `a72df84f runtime: store direct request root references`
Current primary semantic baseline: `a72df84f runtime: store direct request root references`
...
Overall estimated progress toward the current generalized native-compiler roadmap: **71%** `[##############------]`

Momentum remains positive and still uneven. Primary now has stronger request/symbol storage, generated-C array/lvalue consumers, direct request-root snapshot consumers, direct request-root value/reference storage, null-callback array value consumers, and a runtime ABI for nested array reference paths over owned value handles. This is real generalized infrastructure. It is not yet broad PHP compatibility: executable `$GLOBALS`, keyed/path superglobal lowering, generated reference assignment, full references/COW, user calls, objects, structured control flow, and exact diagnostics remain the major blockers.
...
| Executable generalized PHP semantics | 61% | `[############--------]` | Selected scalar, string, array, lvalue, symbol-runtime, request-runtime, direct request-root value/reference storage and snapshots, and null-callback array value behavior works. Broad PHP programs still hit structural blockers. |
...
| Symbols, globals, request state | 47% | `[#########-----------]` | Runtime symbol/request roots can snapshot, mutate, store direct scalar/null/object/resource root values, store direct root reference cells, clear stale keyed slots, re-enter keyed storage where safe, share reference cells, and participate in reference-path ABI tests. Compiler-level `$GLOBALS`, keyed/path superglobal lowering, request lifetime, and frame propagation are not done. |
...
- `master` and `origin/master` are synced at `a72df84f`.
...
- The latest semantic addition is runtime-only direct request-root reference-cell storage; generated-C/LLVM keyed/path request mutations, `$GLOBALS` reconciliation, and PHP reference assignment still need primary compiler consumers.
...
| Symbols/request/globals | 47% | In progress | Keyed/path request/superglobal reads and probes, compiler-lowered mutations over the new direct-root value/reference storage, `$GLOBALS` aliasing, request-root replacement consumers, or request lifetime threading. |
