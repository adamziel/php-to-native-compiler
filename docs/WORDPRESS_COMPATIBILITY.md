...
After Milestone 1294, that synthetic add-option smoke also updates only the
reinserted option's autoload flag with the exact
`UPDATE wp_options SET autoload = ... WHERE option_name = ...` shape. The
generated bootstrap shim and front-controller path prove the database-backed
option value remains `added-db` while autoload changes to `auto-off`, and the
full option-row-with-id set observes that flag, adding
...
After Milestone 1324, the placeholder MySQLi `wp_options` state island also
covers exact prepared option-name-list result-set reads for
...
`mysqli_execute_query($handle, $query, array(...))` when each placeholder
value is a string option name. A committed `wpdb`-shaped fixture proves
deterministic parameter-ordered rows, skipped missing names, and backticked
column/table spelling support for this prepared read path. This remains a
bounded state-island probe only; it does not claim arbitrary prepared
projections, non-string option-name params, broad SQL parsing, real MySQL,
...
After Milestone 1334, the placeholder MySQLi `wp_options` state island also
covers exact prepared autoload-list result-set reads for
...
through both `mysqli_stmt_execute()`/`mysqli_stmt_get_result()` and
`mysqli_execute_query($handle, $query, array(...))` for string autoload
parameters. A committed `wpdb`-shaped fixture proves deterministic
autoload-filtered rows and backticked column/table spelling support. This
remains a bounded state-island probe only; it does not claim arbitrary
prepared projections, non-string autoload params, broad SQL parsing, real
MySQL, persistent object cache, broad `wpdb`, full WordPress option APIs,
plugin/theme loading, request/SAPI fidelity, references/copy-on-write, or
native support.

After Milestone 1339, the placeholder MySQLi `wp_options` state island also
covers exact `SELECT option_name, autoload FROM wp_options ...` row-set reads
for all rows, literal autoload filters, literal name lists, prepared
...
`mysqli_fetch_object()` for the prepared name-list path and `mysqli_fetch_assoc()`
for the prepared autoload-list path. This remains a bounded state-island probe
only; it does not claim arbitrary prepared projections, non-string params,
broad SQL parsing, real MySQL, persistent object cache, broad `wpdb`, full
...
After Milestone 1349, the placeholder MySQLi `wp_options` state island also
covers exact `SELECT option_value FROM wp_options ...` value-only row-set
reads for all rows, literal name lists, literal autoload filters, prepared
`WHERE option_name IN (?, ...)`, and prepared `WHERE autoload IN (?, ...)`
through both `mysqli_stmt_execute()`/`mysqli_stmt_get_result()` and
`mysqli_execute_query($handle, $query, array(...))` when every placeholder is
a string option name or autoload value. A committed `wpdb`-shaped fixture uses
a `get_col()`-style wrapper with `mysqli_fetch_column()` over the prepared
autoload-list path, a literal name-list path, and an all-row prepared path.
This remains a bounded state-island probe only; it does not claim arbitrary
prepared projections, non-string params, broad SQL parsing, real MySQL,
persistent object cache, broad `wpdb`, full WordPress option APIs,
plugin/theme loading, request/SAPI fidelity, references/copy-on-write, or
native support.
...
seed. The current parser handles bracketed request names, repeated append
segments, duplicate scalar keys as last-write-wins, and top-level dotted/spaced
name normalization. This supports deterministic WordPress-shaped request
bootstrap probes without claiming real web-server SAPI fidelity. It does not
implement cookie merging into `$_REQUEST`, multipart uploads, upload metadata,
`variables_order`/`request_order`, host SAPI imports, exact warning behavior,
or native support.

After Milestone 1293, `phpc run` seeds `$_COOKIE` as a deterministic
auto-global array and can parse an explicit `PHPC_COOKIE` semicolon-delimited
cookie header seed into `$_COOKIE`, with the raw seed exposed as
`$_SERVER['HTTP_COOKIE']`. This supports executable WordPress-shaped cookie
guards such as seeded `wordpress_test_cookie` checks in the current request
...
diagnostics, and native lowering explicit gaps. Milestone 1020 adds bounded
direct object-property `ArrayAccess` dispatch when a visible property value is
an `ArrayAccess` object, covering single-key read/write, `isset`, `empty`,
`??`, and `unset` while leaving nested chains, append offsets, compound
...
`offsetSet($key, $value)`. Milestone 1023 adds pre/post increment and
decrement for that property-held `ArrayAccess` slice through `offsetGet($key)`
and PHP's current by-value temporary result behavior without dispatching
...
and native lowering explicit gaps.
  ...
  warning/error fidelity, or native database lowering. Milestone 948 wires
  ...
  execution, warning/error fidelity, or native database lowering. Milestone
  958 wires those boundaries through synthetic WordPress-shaped `wpdb`
  ...
  database lowering. Milestone 981 adds deterministic
  `mysqli_stmt_store_result()` and `mysqli_stmt_num_rows()` behavior for the
  current unbound seed-post WordPress SELECT execution path, including a
  ...
  database lowering. Milestone 985 adds deterministic direct-variable
  `mysqli_stmt_bind_result()` and buffered `mysqli_stmt_fetch()` placeholder
  ...
  native database lowering. Milestone 989 adds bounded positional params-array
  support for `mysqli_stmt_execute($stmt, array(...))`, including a
  ...
  lowering. Milestone 999 tightens that optional params-array path to require
  ...
  database lowering. Milestone 991 adds bounded `b` type-marker support for
