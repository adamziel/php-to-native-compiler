# PHPT Broad 1k Zend Root Current Frontier: 2026-06-14 ptn-xgk8

Issue: `ptn-xgk8`

This slice refreshes the current broad 1k root-level `Zend/tests/*.phpt`
runnable frontier and records focused execution evidence. It complements the
current residual rollup by giving the root-level Zend family its own pass/fail
map. No runtime behavior changed here: the remaining failures cross array
lvalue semantics, object assignment, control-flow diagnostics, lifecycle, and
dispatch surfaces that should be implemented as generic PHP runtime/compiler
features.
...
tools/run-phpt-baseline.sh --tier 1000 --classify-only \
  --out-dir .runtime/ptn-xgk8-baseline
```

Source state:

- PTN commit: `bb6b6ec428c0`
...
Broad runnable rows by source family:
...
```text
tools/phpt-zend-root-current-ptn-xgk8-manifest.txt
```
...
awk -F'\t' '$2=="runnable" && $1 ~ /^Zend\/tests\/[^/]+\.phpt$/ {print $1}' \
  .runtime/phpt-progress/classification-20260614T090208Z.tsv \
  > .runtime/ptn-xgk8-zend-root-runnable.txt
```

Focused classify-only run:
...
Focused execution run:

```sh
tools/run-bounded-phpt.sh tools/phpt-zend-root-current-ptn-xgk8-manifest.txt
```

Artifacts:

```text
.runtime/ptn-xgk8-zend-root-classify-post/classification-20260614T091231Z.tsv
.runtime/ptn-xgk8-zend-root-classify-post/runnable-20260614T091231Z.txt
.runtime/ptn-xgk8-zend-root-classify-post/excluded-20260614T091231Z.tsv
.runtime/ptn-xgk8-zend-root-classify-post/summary-20260614T091231Z.txt
.runtime/ptn-xgk8-zend-root-run-post/classification-20260614T091232Z.tsv
.runtime/ptn-xgk8-zend-root-run-post/runnable-20260614T091232Z.txt
.runtime/ptn-xgk8-zend-root-run-post/excluded-20260614T091232Z.tsv
.runtime/ptn-xgk8-zend-root-run-post/run-20260614T091232Z-manifest.log
.runtime/ptn-xgk8-zend-root-run-post/summary-20260614T091232Z.txt
```
...
Current broad classifier exclusion from the previous root-level manifest:
...
```text
unsupported-diagnostics-runtime: requires ErrorException severity and trace metadata, outside PTN modeled built-in exception values
```

The committed focused manifest contains only the 81 root-level Zend rows that
remain runnable under the current classifier.

Green signal in this slice includes arithmetic/addition rows, array addition
and many COW/reference rows, assignment-by-reference cleanup rows, basic
method/default-argument historical rows, simple error suppression, and several
older object/reference regressions such as `bug31177*`, `bug35163.phpt`,
`bug35239.phpt`, and `bug38808.phpt`.
...
tools/run-bounded-phpt.sh --classify-only \
  tools/phpt-zend-root-current-ptn-xgk8-manifest.txt
tools/run-bounded-phpt.sh .runtime/ptn-xgk8-zend-root-runnable.txt
cargo fmt --check
```
