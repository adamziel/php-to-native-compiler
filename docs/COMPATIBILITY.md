...
| Fixture manifest text audit | `cargo run -p phpc -- test --list-fixtures` | `pass` | Emits sorted fixture metadata, comparison eligibility, `.phpc-only` marker reason text and empty/whitespace-only reason gap counts, recognized fixture/sidecar byte counts including `.cli` snapshot exercise files, CLI exercise gap counts, optional compatibility-target `source-pin.md` audit metadata, and compatibility `.expected` probe expectation artifact metadata without running fixtures or proving compatibility. |
| Fixture manifest JSON audit | `cargo run -p phpc -- test --list-fixtures-json` | `pass` | Emits sorted fixture metadata, compatibility-target counts, CLI exercise gap counts, `.phpc-only` marker reason text and empty/whitespace-only reason gap counts, optional compatibility-target `source-pin.md` audit metadata, compatibility `.expected` probe expectation artifact metadata, recognized fixture/sidecar byte counts, and recognized fixture/sidecar SHA-256 digests including `.cli` snapshot exercise files without running fixtures or proving compatibility. |
