...
| Fixture manifest JSON audit | `cargo run -p phpc -- test --list-fixtures-json` | `pass` | Emits sorted fixture metadata, compatibility-target counts, `.phpc-only` marker reason text, and recognized fixture/sidecar byte counts without running fixtures or proving compatibility. |
