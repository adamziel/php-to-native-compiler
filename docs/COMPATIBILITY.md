...
| Fixture manifest text audit | `cargo run -p phpc -- test --list-fixtures` | `pass` | Emits sorted fixture metadata, comparison eligibility, `.phpc-only` marker reason text, and recognized fixture/sidecar byte counts without running fixtures or proving compatibility. |
| Fixture manifest JSON audit | `cargo run -p phpc -- test --list-fixtures-json` | `pass` | Emits sorted fixture metadata, compatibility-target counts, `.phpc-only` marker reason text, recognized fixture/sidecar byte counts, and recognized fixture/sidecar SHA-256 digests without running fixtures or proving compatibility. |
