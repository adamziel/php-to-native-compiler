...
| WordPress inventory output harness | `cargo test -p phpc --test wordpress_inventory_cli -- --test-threads=1` | `pass` | Synthetic WordPress-shaped tree pins normalized inventory output and the current unsupported `require` bootstrap blocker without vendoring WordPress core. |
