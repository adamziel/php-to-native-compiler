...
are currently visible through `function_exists()`, `is_callable()`, dynamic
string-valued function lookup, and native function-table introspection so
WordPress' early database startup paths can move to the next real bootstrap
blocker.
...
real resources or connected objects beyond the placeholder shape, no queries,
no result sets, no escaping, no charset handling, no errors/warnings, no
