...
diagnostic locations, all UTF-16/UTF-8 corner cases beyond the covered bounded
invalid-byte repair lanes, exact escaped-string invalid-byte parity, broad
partial-output interaction parity beyond current
non-finite/unsupported/recursion/depth placeholders, multi-line or non-ASCII
error-location parity, host-extension option catalogs, `JSON_THROW_ON_ERROR`
exception behavior, native lowering, or JSON functions beyond the listed
bounded subset.
