...
- Refreshed on `ptn-4yt.8` rebased after `ptn-4yt.6`.
...
- Row-level status after focused bucket reruns: excluding `bug38469`, 28 total,
  13 pass, 15 fail. Counting `bug38469` as a fail gives 29 total, 13 pass,
  16 fail.
- Fixed rows after the latest merges: `assign_to_var_003` passes through shared
  ...
  string-offset reference rows pass through array literal reference elements and
  location-aware runtime `Error` diagnostics, and `return_reference_separation`
  passes through scalar type-hint parsing plus typed by-reference return
  separation.
...
Assignment-aliasing improved from 3/4 to 4/4, string-offsets improved from 2/4
to 4/4, and function-boundaries improved from 0/4 to 1/4 since the
2026-06-09T20:32Z measurement.
...
| function-boundaries | `Zend/tests/return_types/return_reference_separation.phpt` | Scalar `int`/`string` type hints parse and coerce, typed by-reference return declarations lower, and direct-variable returns write coerced values through reference-bound slots. | `typed_by_ref_return_separation` |
...
| `Zend/tests/return_types/return_reference_separation.phpt` | `function &f(int &$x): string { return $x; }` | fixed |
