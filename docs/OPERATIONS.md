...
counts for fixtures, summaries, orphan sidecars, and compatibility targets.
...
`phpc_only_reason` fields, source and recognized sidecar byte counts, and
per-target counts for `compat/<target>` directories so PHP and WordPress
compatibility fixture coverage can be audited without executing code.
