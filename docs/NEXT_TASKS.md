...
- [x] Parser lane: add a dedicated parse diagnostic and CLI fixture for
  multiple simple class imports in one `use` declaration, such as
  `use App\A, App\B;`, without broadening namespace/import support claims.

## Milestone 1152: Next Runtime Value/Object Slice

- [x] Runtime lane: enforce the current supported non-static method shape for
  declared user-interface method obligations, including inherited
  `implements` metadata on concrete children, with focused object-model and
  fixture coverage.

## Milestone 1153: Next Native Boundary

- [x] IR/lowering lane: split `new` object-instantiation and constructor
  dispatch into a dedicated native rejection for `--emit-ir` and `--emit-asm`,
  keeping other object/class boundaries separate.

## Milestone 1154: Next Compiler-Output Contract

- [x] Compiler-output lane: include `.phpc-only` marker reason text in the
  deterministic text fixture manifest while leaving JSON and comparison
  behavior unchanged.

## Milestone 1155: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1151-1154 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1161 adds a dedicated lex boundary for short echo tags such as
  `<?= $value ?>`, replacing the broad unexpected-character error while
  leaving echo-tag expansion, source mapping, inline-HTML interaction beyond
  the named rejection, and native lowering unsupported.

## Milestone 1162: Runtime Interface Parameter-Type Compatibility Slice

- [x] Runtime lane: enforce a bounded declared-interface method parameter-type
  metadata compatibility rule for concrete classes, including inherited
  `implements` metadata on concrete children. Implementations may omit an
  ...
  different type for a typed interface parameter. This does not implement full
  PHP signature variance, return type compatibility, class/interface type
  subtyping, type aliases/imports, union/intersection canonicalization,
  built-in/internal interface method enforcement, exact PHP `Error` objects,
  or native object/interface lowering.

## Milestone 1163: Next Native Boundary

- [x] IR/lowering lane: split documented object/class metadata builtins into a
  dedicated native object-metadata rejection for `phpc compile --emit-ir` and
  `--emit-asm`, while preserving existing native scalar false-folds for
  `is_object`, `get_debug_type`, metadata-exists calls, member checks, and
  relationship checks where no native class table is needed.
...
- [x] Tests/docs lane: after Milestones 1161-1164 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.

## Milestone 1166: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1166 adds dedicated parse diagnostics for namespace-qualified
  ...
  replacing the misleading namespace-qualified-function diagnostic while
  leaving namespace-aware constant lookup, fallback behavior, constant imports,
  exact PHP diagnostics, and native lowering unsupported.

## Milestone 1167: Next Runtime Value/Object Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1167 adds a bounded declared-interface method return-type metadata
  compatibility check for concrete classes, including inherited `implements`
  metadata on concrete children. Implementations may add a return type when
  the interface method is untyped, but a typed interface method requires the
  same return type text case-insensitively. This does not implement full PHP
  signature variance, class/interface return covariance, type aliases/imports,
  union/intersection canonicalization, built-in/internal interface method
  enforcement, call-time type enforcement, exact PHP `Error` objects, or
  native object/interface lowering.

## Milestone 1168: Next Native Boundary

- [x] IR/lowering lane: choose one precise native rejection or tiny lowering
  refinement from interpreter behavior that is already documented. `phpc
  compile --emit-ir` and `--emit-asm` must either lower the exact supported
  slice or reject it before misleading backend output.
  Milestone 1168 adds a dedicated native interpolated-string rejection for
  documented double-quoted interpolation instead of reporting the broader
  concatenation boundary.

## Milestone 1169: Next Compiler-Output Contract

- [x] Compiler-output lane: choose one deterministic CLI, fixture-runner,
  compatibility-manifest, or backend artifact contract that improves
  auditability without broadening PHP support claims. Milestone 1169 bumps
  the JSON fixture manifest to `contract_version` 5 with SHA-256 digests for
  fixture sources, recognized sidecars, and recognized orphan sidecars while
  leaving fixture execution, PHP comparison, text manifests, native lowering,
  and PHP support claims unchanged.

## Milestone 1170: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1166-1169 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1176 adds a dedicated parse diagnostic for parenthesized dynamic
  class-name instantiation such as `new ($class)()`, replacing the broad
  `expected class name after 'new'` parse error while keeping the existing
  direct-variable `new $class(...)` runtime slice unchanged.
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1177 adds bounded internal `Iterator` and `IteratorAggregate`
  method-shape registration checks. Concrete `Iterator` implementors,
  including concrete children inheriting that metadata from abstract parents,
  must expose public non-static `current()`, `key()`, `next()`, `rewind()`,
  and `valid()` methods with no required parameters; concrete
  `IteratorAggregate` implementors must expose a public non-static
  `getIterator()` method with no required parameters. Direct concrete
  `implements Traversable` now reports a stable runtime boundary. This does
  not implement object `foreach`, iterator method execution,
  `IteratorAggregate::getIterator()` dispatch, tentative return-type notices,
  return-declaration compatibility, full built-in/internal interface signature
  enforcement, exact PHP `Error` objects, references/copy-on-write, or native
  object/interface lowering.

## Milestone 1178: Next Native Boundary

- [x] IR/lowering lane: choose one precise native rejection or tiny lowering
  refinement from interpreter behavior that is already documented. Milestone
  1178 adds a dedicated native `instanceof` rejection for the documented
  bounded runtime class/interface relationship checks, so `phpc compile
  --emit-ir` and `--emit-asm` reject the AST node before lowering the left
  operand or invoking an assembly backend.

## Milestone 1179: Next Compiler-Output Contract

- [x] Compiler-output lane: choose one deterministic CLI, fixture-runner,
  compatibility-manifest, or backend artifact contract that improves
  auditability without broadening PHP support claims. Milestone 1179 bumps
  the JSON fixture manifest to `contract_version` 7 and reports deterministic
  `compat/<target>/**/*.expected` probe expectation artifacts in compatibility
  target manifests with path, byte count, and SHA-256 while leaving fixture
  execution, PHP comparison, parser/runtime behavior, native lowering, and
  PHP/WordPress support claims unchanged.

## Milestone 1180: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1176-1179 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.

## Milestone 1181: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1181 refines unsupported named call arguments such as
  `call(name: $value)` from a broad "not implemented" parse error into a
  dedicated diagnostic naming missing parameter-name metadata, duplicate and
  unknown-name diagnostics, positional/named ordering, by-reference binding,
  variadic collection, unpacking interaction, and native lowering. This does
  not implement named-argument runtime dispatch, method/constructor/static-call
  named arguments, unpacking interaction, or native lowering.

## Milestone 1182: Next Runtime Value/Object Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1182 adds bounded `str_starts_with()` support for the current
  scalar/null string-convertible haystack and needle subset, including empty
  needle behavior, dynamic string-valued calls, function-table introspection,
  fixture CLI coverage, and system PHP comparison. This does not implement
  binary string edge cases beyond represented UTF-8 runtime strings,
  array/object/resource coercions, exact PHP diagnostics, or native lowering
  beyond function-table introspection.

## Milestone 1183: Next Native Boundary

- [x] IR/lowering lane: choose one precise native rejection or tiny lowering
  refinement from interpreter behavior that is already documented. `phpc
  compile --emit-ir` and `--emit-asm` must either lower the exact supported
  slice or reject it before misleading backend output. Milestone 1183 adds a
  dedicated native class-name constant rejection for `ClassName::class`,
  `self::class`, `parent::class`, and `static::class`, keeping class constants
  and static properties on the existing static-member boundary.

## Milestone 1184: Next Compiler-Output Contract

- [x] Compiler-output lane: choose one deterministic CLI, fixture-runner,
  compatibility-manifest, or backend artifact contract that improves
  auditability without broadening PHP support claims. Milestone 1184 bumps
  the JSON fixture manifest to `contract_version` 8 and reports deterministic
  `.cli` snapshot exercise sidecars in text and JSON fixture manifests with
  per-fixture and aggregate counts, byte counts, SHA-256 digests in JSON, and
  orphan sidecar detection, while leaving fixture execution, CLI snapshot
  execution, parser/runtime behavior, native lowering, and PHP support claims
  unchanged.

## Milestone 1185: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1181-1184 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.

## Milestone 1186: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1186 refines the PHP 8 `match` expression parse boundary so it
  names the missing strict arm matching, default/exhaustiveness handling,
  throw arms, value evaluation order, references/copy-on-write, and native
  lowering instead of the older broad expression-branching message.

## Milestone 1187: Next Runtime Value/Object Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1187 adds bounded `is_file()` support for one local string path,
  including host regular-file metadata checks, false results for directories
  and missing paths, dynamic string-valued calls, function-table
  introspection, fixture CLI coverage, and system PHP comparison. This does
  not implement include-path lookup, stream wrappers, symlink/canonicalization
  policy, portable file-type details, permission/open_basedir behavior,
  non-string coercions, stat-cache behavior, exact PHP diagnostics, or native
  lowering beyond function-table introspection.

## Milestone 1188: Next Native Boundary

- [x] IR/lowering lane: choose one precise native rejection or tiny lowering
  refinement from interpreter behavior that is already documented. `phpc
  compile --emit-ir` and `--emit-asm` must either lower the exact supported
  slice or reject it before misleading backend output. Milestone 1188 adds a
  dedicated native `str_starts_with(...)` rejection before argument lowering or
  backend selection, while preserving the existing native
  `function_exists()`/`is_callable()` metadata folds for the known builtin
  name and leaving runtime behavior unchanged.

## Milestone 1189: Next Compiler-Output Contract

- [x] Compiler-output lane: choose one deterministic CLI, fixture-runner,
  compatibility-manifest, or backend artifact contract that improves
  auditability without broadening PHP support claims. Milestone 1189 bumps
  the JSON fixture manifest to `contract_version` 9, adds aggregate recognized
  orphan sidecar byte totals to text/JSON manifest summaries and compatibility
  target summaries, and includes SHA-256 digests on text orphan sidecar rows.
  This does not execute or validate orphan sidecars, alter fixture execution,
  change system PHP comparison behavior, or broaden parser/runtime/native PHP
  support claims.

## Milestone 1190: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1186-1189 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.

## Milestone 1191: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1191 adds a dedicated lex diagnostic for unsupported PHP backtick
  shell execution operators such as `` `whoami` ``, replacing the broad
  unexpected-character error while leaving command interpolation, process I/O,
  shell/platform behavior, references/copy-on-write, exact PHP diagnostics,
  and native lowering unsupported.

## Milestone 1192: Next Runtime Value/Object Slice

- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  map, preferably a remaining reference/COW, object-semantics, request-state,
  filesystem, database, or WordPress-probe blocker already reached by fixtures
  or external probes. Milestone 1192 adds bounded `basename()` runtime support
  for the current lexical Unix-style string path and optional string suffix
  subset, covers direct and dynamic string-valued calls plus a
  WordPress-shaped path probe, keeps direct native calls at the existing
  function-call lowering rejection, and documents unsupported Windows/UNC,
  stream-wrapper, filesystem-resolution, null-byte, locale/codepage,
  broad-coercion, exact diagnostic, and native-lowering gaps.

## Milestone 1193: Native `str_ends_with()` Boundary

- [x] IR/lowering lane: add a dedicated native `str_ends_with(...)`
  string-suffix rejection for the already documented bounded interpreter
  builtin. `phpc compile --emit-ir` and `--emit-asm` reject direct calls before
  argument lowering or backend output, while function-table introspection keeps
  recognizing the builtin name.

## Milestone 1194: Next Compiler-Output Contract

- [x] Compiler-output lane: refine the deterministic fixture manifest contract
  so text and JSON `phpc test --list-fixtures* [fixture-dir]` output reports
  aggregate and compatibility-target CLI exercise gap counts for fixtures
  without `.cli` snapshot sidecars. JSON is now `contract_version` 10. This
  does not execute or validate `.cli` snapshots, alter fixture execution,
  change system PHP comparison behavior, or broaden parser/runtime/native PHP
  support claims.

## Milestone 1195: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1191-1194 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing.

## Milestone 1196: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1196 keeps simple no-argument PHP attributes as syntax-only
  metadata but turns constructor-style attribute argument payloads such as
  `#[Route('/wp-json/demo')]` into a dedicated lex diagnostic naming missing
  constructor argument evaluation, attribute metadata, target validation,
  reflection visibility, namespace-aware attribute names, repeatability rules,
  references/copy-on-write, and native lowering.

## Milestone 1197: Next Runtime Value/Object Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1197 extends `file_get_contents()` from the deterministic
  `php://input` placeholder to a bounded local UTF-8 text file read for one
  string path, using the existing filesystem relative-path policy, dynamic
  string-valued calls, fixture CLI coverage, and system PHP comparison. Binary
  byte fidelity, missing-file warning/`false` recovery, other stream wrappers,
...
## Milestone 1198: Native `basename()` Boundary

- [x] IR/lowering lane: add a dedicated native `basename(...)` path rejection
  for the documented bounded interpreter builtin. `phpc compile --emit-ir` and
  `--emit-asm` reject direct calls before argument lowering or backend output,
  while function-table introspection keeps recognizing the builtin name.

## Milestone 1199: Next Compiler-Output Contract

- [x] Compiler-output lane: refine the deterministic fixture manifest contract
  so text and JSON `phpc test --list-fixtures* [fixture-dir]` output reports
  aggregate and compatibility-target `.phpc-only` reason gap counts for marker
  files whose text is empty or whitespace-only. JSON is now
  `contract_version` 11. This does not reject fixtures, alter fixture
  execution, change system PHP comparison behavior, or broaden
  parser/runtime/native PHP support claims.

## Milestone 1200: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1196-1199 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing. Full gate passed with `1353`
  fixture tests, `768` system PHP comparisons, and `585` skipped `phpc-only`
  fixtures.

## Milestone 1201: Static Arrow Parse Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1201 gives PHP `static fn (...) => expr` arrow closures a dedicated
  parse diagnostic instead of the broad `expected expression, found static`
  fallback, while preserving existing `static function (...) { ... }` inert
  closure values, non-static arrow closure values, and static-member syntax
  paths. Static arrow execution, no-`$this` binding, implicit capture
  metadata, callback integration, references/copy-on-write, exact PHP
  diagnostics, and native lowering remain unsupported.

## Milestone 1202: Runtime `getcwd()` Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1202 adds bounded `getcwd()` runtime support for the current CLI
  process: no arguments, UTF-8 current working directory string result,
  dynamic string-valued calls, function/callability lookup metadata, fixture
  CLI coverage, and system PHP comparison. This does not implement `chdir()`,
  failure returning `false`, non-UTF-8 working-directory paths, SAPI-specific
  working-directory policy, include-path interaction, `open_basedir`, exact
  warning behavior, or native lowering beyond function-table introspection.

## Milestone 1203: Native `file_get_contents()` Boundary

- [x] IR/lowering lane: add a dedicated native `file_get_contents(...)`
  filesystem/stream-read rejection for the documented bounded interpreter
  builtin. `phpc compile --emit-ir` and `--emit-asm` reject direct calls before
  argument lowering or backend output, while function-table introspection keeps
  recognizing the builtin name.

## Milestone 1204: Fixture Manifest Unrecognized Sidecars

- [x] Compiler-output lane: refine the deterministic fixture manifest contract
  so text and JSON `phpc test --list-fixtures* [fixture-dir]` output reports
  unrecognized sidecar-like files whose extension is not part of the fixture
  contract but whose corresponding `.php` fixture exists. JSON is now
  `contract_version` 12. This does not execute or validate those files, alter
  fixture execution, change system PHP comparison behavior, or broaden
  parser/runtime/native PHP support claims.

## Milestone 1205: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1201-1204 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing. Full gate passed with `1355`
  fixture tests, `769` system PHP comparisons, and `586` skipped `phpc-only`
  fixtures.

## Milestone 1206: Next Parser Boundary
...
- [x] Parser lane: choose the next small unsupported syntax or
  ...
  misleading diagnostic. Add stable focused coverage, CLI fixture evidence
  where applicable, and keep runtime/native support claims unchanged.
  Milestone 1206 refines unsupported call-site argument unpacking such as
  `handler(...$args)` from the broad variadic-call parse error into a
  dedicated diagnostic naming missing iterable expansion order, string-keyed
  named-argument interaction, by-reference argument propagation, variadic
  collection, duplicate argument diagnostics, and native lowering. Runtime and
  native support claims remain unchanged.

## Milestone 1207: Next Runtime Value/Object Slice
...
- [x] Runtime lane: choose one bounded runtime slice from the refreshed gap
  ...
  or external probes. Prove it with focused tests, CLI coverage, system PHP
  comparison where applicable, and named unsupported edges.
  Milestone 1207 adds bounded `realpath($path)` runtime support for one string
  local path: stream-wrapper rejection, existing path resolution through the
  current process-path-then-repo-root policy, UTF-8 resolved path strings,
  `false` for unresolved local paths, dynamic-call coverage, and function-table
  metadata. Symlink policy fidelity, exact warning/`false` behavior,
  include-path lookup, `open_basedir`, stream wrappers, non-UTF-8 paths,
  stat-cache behavior, and native lowering remain unsupported.

## Milestone 1208: Next Native Boundary

- [x] IR/lowering lane: choose one precise native rejection or tiny lowering
  refinement from interpreter behavior that is already documented. `phpc
  compile --emit-ir` and `--emit-asm` must either lower the exact supported
  slice or reject it before misleading backend output. Milestone 1208 adds a
  dedicated native `getcwd()` current-directory rejection for the documented
  bounded interpreter builtin: direct `--emit-ir` and `--emit-asm` calls stop
  before argument lowering or backend output, while native
  `function_exists()`/`is_callable()` metadata still recognizes `getcwd`.
  Native process/request cwd state, UTF-8/path policy, SAPI cwd behavior,
  `chdir()` interaction, failure returning `false`, references/copy-on-write,
  and exact native diagnostics remain unsupported.

## Milestone 1209: Next Compiler-Output Contract

- [x] Compiler-output lane: choose one deterministic CLI, fixture-runner,
  compatibility-manifest, or backend artifact contract that improves
  auditability without broadening PHP support claims. Milestone 1209 reports
  missing recognized expectation sidecars for `.stdout`, `.stderr`, `.exit`,
  and `.cli` in deterministic text and JSON fixture manifests. JSON is now
  `contract_version` 13, fixture entries include ordered
  `missing_expectation_sidecars` lists, and aggregate plus compatibility-target
  summaries include `missing_expectation_sidecars` counts. This does not create
  required sidecars, reject fixtures, execute or validate `.cli` snapshots,
  alter fixture execution, change system PHP comparison behavior, or broaden
  parser/runtime/native PHP support claims.

## Milestone 1210: Next Tests/Docs Queue Refresh

- [x] Tests/docs lane: after Milestones 1206-1209 land, refresh the lane
  queue, progress log, support docs, and compatibility-gap notes, then run the
  serialized full gate before checkpointing. Full gate passed with `1357`
...
- [x] Parser/runtime lane: advance one concrete trait/interface blocker beyond
  ...
  deterministic. Milestone 1236 accepts explicit-public qualified trait method
  aliases such as `use HasHooks, HasLabel { HasHooks::hooks as public
  register_hooks; }`, clones the selected public instance method under the
  alias, leaves the original method available, and lets the alias satisfy the
  current interface method-presence checks. Protected/private visibility
  changes, visibility-only adaptations, `insteadof`, conflict resolution,
  trait properties/constants, non-public/static/abstract/final trait methods,
...
- [x] Runtime request-state lane: seed bounded `$_FILES` as a deterministic
  empty auto-global upload bag routed through the root symbol table, add
  explicit native request-superglobal rejection for direct `$_FILES` lowering,
  prove the WordPress-shaped upload guard fixture with system PHP comparison,
  and document the remaining multipart/upload/SAPI gaps.
...
- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  ...
  support. Milestone 1239 adds a synthetic `update_option()` option-cache
  smoke after the existing `get_option()` probe: the bootstrap shim and
  front-controller paths seed a non-autoloaded `blogdescription` row, read it
  through cache-backed `get_option()`, update it through the current exact
  direct `wp_options` UPDATE state island, refresh the bounded in-memory cache
  with `wp_cache_set()`, and prove the next `get_option()` returns the updated
  value. It does not claim persistent object-cache behavior, real database
  connectivity, arbitrary SQL, broad `wpdb`, full WordPress option APIs,
  plugins/themes, request/SAPI fidelity, or native support.

## Milestone 1240: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1236-1239 land, refresh the queue
  around the next largest WordPress blockers, update support/compatibility docs
  with exact unsupported edges, run the serialized full gate, and checkpoint
  the batch. Milestone 1240 keeps the next split focused on trait/interface
  semantics, reference/COW fidelity, request/SAPI/filesystem behavior, and
  `wpdb`/bootstrap evidence. Full gate passed at checkpoint: `1372` fixture
  tests, `784` system PHP comparisons, and `588` skipped `phpc-only`
  fixtures.

## Milestone 1241: Next Trait/Interface WordPress Slice

- [x] Parser/runtime lane: advance one concrete trait/interface blocker beyond
  explicit-public qualified aliases. Prefer one WordPress-reached behavior
  such as a stable `insteadof`/visibility diagnostic, a trait property/constant
  boundary, or a small interface/trait interaction required by a current probe.
  Prove it with focused object-model tests, a CLI fixture, and PHP comparison
  when deterministic. Milestone 1241 implements a bounded executable
  single-loser trait conflict adaptation:
  `use TraitA, TraitB { TraitA::method insteadof TraitB; }` for public
  instance methods from traits listed in the same class-body `use`
  declaration. The interpreter validates the winning and losing trait methods,
  composes the winner, skips the named loser method, and lets the selected
  method satisfy the current interface method-presence checks. Unsupported
  edges remain: unqualified `insteadof`, multi-loser loser lists, aliases or
  visibility changes combined with precedence beyond the existing alias slice,
  non-public/static/abstract/final methods, trait properties/constants,
  `__TRAIT__`, conditional/nested trait registration, references/COW, exact
  PHP diagnostics, and native lowering.

## Milestone 1242: Next Reference/COW WordPress Slice
...
- [x] Runtime lane: advance one remaining WordPress-shaped reference or
  ...
  Prefer a real execution boundary over diagnostics-only work. Keep full PHP
  reference containers and native lowering unsupported unless implemented and
  tested. Milestone 1242 extends the literal
  `call_user_func_array($callback, array(&$value, ...))` reference-binding
  slice from string user-function callbacks to public instance-method array
  callables such as `array($object, "method")`, while keeping closure
  callbacks, static method by-reference array callbacks, keyed/non-literal
  argument arrays for reached by-reference parameters, stored reference arrays,
  full reference/COW containers, exact PHP warnings, and native lowering
  unsupported.

## Milestone 1243: Next Request/SAPI/Filesystem WordPress Slice

- [x] Runtime or IR/lowering lane: implement one bounded request-state,
  include-path, stream/filesystem, output-buffering, header/cookie, upload, or
  SAPI behavior that advances executable WordPress bootstrap/request progress.
  Native lowering should reject unsupported runtime-only behavior explicitly
  when a direct lowering would otherwise be misleading.
  Milestone 1243 adds deterministic interpreter-owned CLI header state:
  `header(...)` appends accepted raw header lines to an in-process request log,
  `headers_list()` returns that log as an ordered array, and direct native
  `header()`/`header_remove()`/`headers_list()`/`headers_sent()` calls reject
  through a header-state lowering boundary. Duplicate/replacement policy,
  `header_remove()` mutation, cookie formatting, status-code state,
  output-started tracking, web-server/SAPI emission, and native header state
  remain unsupported.

## Milestone 1244: Next WordPress Database/Bootstrap Evidence Slice

- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  WordPress probe to a later database/bootstrap point, or implement one small
  `mysqli`/`wpdb`/object-cache/option behavior required by that probe. Avoid
  claiming real MySQL, broad SQL, plugin/theme loading, or full request
  support. Milestone 1244 adds a synthetic `delete_option()`-shaped
  option-cache bootstrap smoke after the update path. It proves the exact
  supported direct `wp_options` DELETE shape, bounded cache deletion, and
  default fallback after deletion without claiming real database connectivity,
  broad `wpdb`, persistent object cache, plugins/themes, full request support,
  or native lowering.

## Milestone 1245: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1241-1244 land, refresh the queue
  around the next largest WordPress blockers, update support/compatibility docs
  with exact unsupported edges, run the serialized full gate, and checkpoint
  the batch. Milestone 1245 keeps the next split focused on trait/interface
  semantics, reference/COW fidelity, request/SAPI/filesystem behavior, and
  `wpdb`/bootstrap evidence. Full gate passed at checkpoint: `1375` fixture
  tests, `786` system PHP comparisons, and `589` skipped `phpc-only`
  fixtures.

## Milestone 1246: Next Trait/Interface WordPress Slice

- [x] Parser/runtime lane: advance one concrete trait/interface blocker beyond
  bounded single-loser `insteadof`. Prefer a WordPress-reached executable
  behavior such as trait precedence plus alias interaction, interface
  inheritance/method-presence refinement, or a stable trait property/constant
  boundary. Milestone 1246 proves trait precedence plus explicit-public alias
  interaction in the same class-body adaptation block:
  `use TraitA, TraitB { TraitA::method insteadof TraitB; TraitA::method as
  public alias; }`. The winning public instance method remains callable under
  its original name, the same winning method can be exposed under a public
  alias, both method names satisfy current interface method-presence checks,
  and the named loser method remains skipped. Non-public/static/abstract/final
  trait methods, broad multi-loser conflict resolution, protected/private
  visibility changes, visibility-only adaptations, trait properties/constants,
  `__TRAIT__`, exact PHP diagnostics, and native lowering remain unsupported.

## Milestone 1247: Next Reference/COW WordPress Slice

- [x] Runtime lane: advance one remaining WordPress-shaped reference or
  copy-on-write behavior around object-property arrays, include scope,
  callback dispatch, globals, request bags, or mutable option/cache state.
  Prefer an executable mutation path over diagnostics-only work. Keep full PHP
  reference containers, stored reference arrays, broad copy-on-write, and
  native lowering unsupported unless implemented and tested. Milestone 1247
  adds a bounded direct public object-property array-offset by-reference
  parameter path for user functions and public instance methods, covering
  option/object-cache style state such as
  `cache_mark($cache->cache["options"]["alloptions"], "suffix")`. The
  interpreter materializes the selected public property array slot, copies it
  into the callee parameter, and writes the final parameter value back after a
  normal or explicit return. Unsupported edges remain: in-call reference
  container identity, non-public or dynamic properties, append-offset
  arguments, ArrayAccess, callback-dispatched object-property array arguments,
  reference-returning functions with object-property array reference
  arguments, stored reference arrays, broad copy-on-write, exact PHP warnings,
  and native lowering.

## Milestone 1248: Next Request/SAPI/Filesystem WordPress Slice

- [x] Runtime or IR/lowering lane: implement one bounded request-state,
  output-buffering, header/cookie, upload, include-path, stream/filesystem, or
  SAPI behavior that advances executable WordPress bootstrap/request progress.
  Native lowering must reject unsupported runtime-only behavior explicitly when
  direct lowering would be misleading. Milestone 1248 makes `header_remove()`
  mutate the deterministic interpreter-owned CLI header log: no-argument calls
  clear the log, and one string removes entries whose raw header line has
  exactly that field name before the first colon. Case folding, whitespace
  normalization, status-header removal, duplicate/replacement policy,
  output-started tracking, full SAPI/web-server emission, exact warning
  behavior, and native header-state lowering remain unsupported.

## Milestone 1249: Next WordPress Database/Bootstrap Evidence Slice

- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  WordPress probe to a later database/bootstrap point, or implement one small
  `mysqli`/`wpdb`/object-cache/option behavior required by that probe. Avoid
  claiming real MySQL, arbitrary SQL, broad `wpdb`, plugin/theme loading, full
  request support, or native support. Milestone 1249 adds a synthetic
  `add_option()`-shaped option-cache bootstrap smoke after the delete path. It
  proves the exact supported direct `wp_options` INSERT shape after a
  cache-backed read/update/delete sequence, refreshes the bounded in-memory
  cache, and verifies the next `get_option()` returns the added value without
  claiming persistent object-cache behavior, duplicate-option rejection
  fidelity, real database connectivity, arbitrary SQL, broad `wpdb`, full
  WordPress option APIs, plugins/themes, full request support, or native
  lowering.

## Milestone 1250: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1246-1249 land, refresh the queue
  around the next largest WordPress blockers, update support/compatibility docs
  with exact unsupported edges, run the serialized full gate, and checkpoint
  the batch. Milestone 1250 keeps the next split focused on trait/interface
  semantics, reference/COW fidelity, request/SAPI/filesystem behavior, and
  `wpdb`/bootstrap evidence. Full gate passed at checkpoint: `1378` fixture
  tests, `788` system PHP comparisons, and `590` skipped `phpc-only`
  fixtures.

## Milestone 1251: Next Trait/Interface WordPress Slice

- [x] Parser/runtime lane: advance one concrete trait/interface blocker beyond
  the current bounded `insteadof` plus public-alias slice. Prefer executable
  interface inheritance/method-presence refinement, a trait property/constant
  boundary, or another WordPress-reached trait/interface interaction. Prove it
  with object-model tests, a CLI fixture, and PHP comparison when deterministic.

## Milestone 1252: Next Reference/COW WordPress Slice

- [x] Runtime lane: advance one remaining WordPress-shaped reference or
  copy-on-write behavior around public object-property arrays, include scope,
  callback dispatch, globals, request bags, or mutable option/cache state.
  Prefer an executable mutation path. Keep full PHP reference containers,
  broad copy-on-write, stored reference arrays, exact warnings, and native
  lowering unsupported unless implemented and tested. Milestone 1252 extends
  the bounded direct public object-property array-offset by-reference
  parameter write-back bridge to named static method calls such as
  `Cache_Marker::tag($cache->cache["options"]["alloptions"], "static")`.
  The selected public property array slot is still materialized, copied into
  the callee parameter, and written back after normal static method
  completion. Dynamic static receivers, `self::`, `parent::`, `static::`,
  callback dispatch, reference-returning methods, non-public or dynamic
  property arguments, append-offset arguments, ArrayAccess, stored reference
  arrays, full PHP reference containers, broad copy-on-write, exact warnings,
  and native lowering remain unsupported.

## Milestone 1253: Next Request/SAPI/Filesystem WordPress Slice

- [x] Runtime or IR/lowering lane: implement one bounded request-state,
  output-buffering, header/cookie, upload, include-path, stream/filesystem, or
  SAPI behavior that advances executable WordPress bootstrap/request progress.
  Native lowering must reject unsupported runtime-only behavior explicitly when
  direct lowering would be misleading. Milestone 1253 adds bounded
  `setcookie($name, $value = "")` support to the deterministic interpreter
  CLI header log and keeps expiration, path/domain/security flags, SameSite,
  duplicate/replacement policy, output-started tracking, web-server emission,
  exact warnings, and native header-state lowering unsupported.

## Milestone 1254: Next WordPress Database/Bootstrap Evidence Slice

- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  WordPress probe to a later database/bootstrap point, or implement one small
  `mysqli`/`wpdb`/object-cache/option behavior required by that probe. Avoid
  claiming real MySQL, arbitrary SQL, broad `wpdb`, plugin/theme loading, full
  request support, or native support. Milestone 1254 adds bounded exact
  duplicate `add_option()` rejection evidence over the synthetic WordPress
  option-cache probe and exact direct/prepared plain `wp_options` insert
  duplicate-name rejection in the placeholder MySQLi state island.

## Milestone 1255: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1251-1254 land, refresh the queue
  around the next largest WordPress blockers, update support/compatibility docs
  with exact unsupported edges, run the serialized full gate, and checkpoint
  the batch. Milestone 1255 keeps the next split focused on closing larger
  WordPress blockers in parallel: deeper trait/interface semantics, reference
  and copy-on-write fidelity, request/SAPI/filesystem behavior, and executable
  `wpdb`/bootstrap evidence.

## Milestone 1256: Next Trait/Interface WordPress Slice

- [x] Parser/runtime lane: advance one larger trait/interface blocker still in
  the WordPress path. Milestone 1256 extends declared user interface
  inheritance from the prior single-parent slice to multiple already-declared
  user parent interfaces such as `interface Child extends A, B`. Concrete
  implementors of the child interface must expose public methods required by
  the child and all declared parents, including methods supplied by the current
  public trait composition and alias subset, and relationship metadata is
  flattened for `instanceof`, `is_a()`, and `is_subclass_of()`. Forward
  parent-interface resolution, interface constants, full variance/signature
  compatibility beyond existing checks, internal interface inheritance
  catalogs, exact PHP diagnostics, and native lowering remain unsupported.

## Milestone 1257: Next Reference/COW WordPress Slice

- [x] Runtime lane: advance one larger reference or copy-on-write blocker that
  affects WordPress-style mutable state. Milestone 1257 extends the bounded
  direct public object-property array-offset by-reference parameter writeback
  bridge to `self::` static method calls in active class context and
  late-bound `static::` static method calls in active called-class context,
  reusing the Milestone 1252 named-static bridge. The selected public property
  array slot is still materialized, copied into the callee parameter, and
  written back after normal static method completion. Callback-dispatched
  object-property array reference arguments, dynamic static receivers,
  `parent::` object-property array reference arguments, reference-returning
  methods with object-property array arguments, non-public or dynamic property
  arguments, append-offset arguments, ArrayAccess, stored reference arrays,
  full PHP reference containers, broad COW, exact warnings, and native
  lowering remain unsupported.

## Milestone 1258: Next Request/SAPI/Filesystem WordPress Slice

- [x] Runtime or IR/lowering lane: implement one bounded request, SAPI,
  output-buffering, include-path, stream, filesystem, cookie, upload, or
  superglobal behavior that moves real WordPress bootstrap/request execution
  forward. Native lowering must reject runtime-only state explicitly when direct
  lowering would be misleading. Milestone 1258 adds bounded interpreter-owned
  output buffering for `ob_start()`, `ob_get_level()`, and `ob_get_clean()`;
  PHP-visible output is captured while a buffer is active, `ob_get_clean()`
  returns and closes the innermost buffer, unclosed buffers flush at normal
  completion or bounded `exit()`, and direct native calls reject under an
  output-buffer lowering boundary. Callbacks, chunk sizes, flags, flush/end/
  status/list variants, output-started/header interaction, fatal-error cleanup,
  SAPI emission, exact warnings, and native output-buffer lowering remain
  unsupported.

## Milestone 1259: Next WordPress Database/Bootstrap Evidence Slice

- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  WordPress probe to a later bootstrap point, or implement one small
  `wpdb`/MySQLi/object-cache/option behavior needed by such a probe. Avoid
  claiming real MySQL, arbitrary SQL, broad `wpdb`, plugin/theme loading, full
  request support, or native support. Milestone 1259 adds an exact
  `SELECT option_value, autoload FROM wp_options WHERE option_name = ... LIMIT 1`
  row read over the placeholder `wp_options` state island, including direct
  `mysqli_query()` and prepared/`mysqli_execute_query()` paths, and advances
  the synthetic add-option probe to prove the cached value and database
  autoload flag after duplicate rejection.

## Milestone 1260: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1256-1259 land, refresh support and
  compatibility docs around the remaining WordPress blockers, record focused
  verification, run the serialized full gate, checkpoint the batch, and open
  the next parallel implementation queue. Milestone 1260 keeps the next split
  focused on the remaining large WordPress blockers: trait constants and
  interface constants/forward resolution, callback/reference/COW fidelity,
  output-buffer/request/filesystem breadth, and executable `wpdb`/bootstrap
  evidence. Full gate passed at checkpoint: `1384` fixture tests, `793`
  system PHP comparisons, and `591` skipped `phpc-only` fixtures.

## Milestone 1261: Next Trait/Interface WordPress Slice

- [x] Parser/runtime lane: advance one large trait/interface blocker still in
  the WordPress path. Prefer trait constants, interface constants, forward
  parent-interface resolution, or another trait/interface construct reached by
  WordPress bootstrap. Prove the behavior or rejection with object-model or
  syntax-boundary tests, fixtures, docs, and PHP comparison when deterministic.
  Milestone 1261 adds the bounded public trait-constant composition slice:
  `const NAME = ...` and `public const NAME = ...` in top-level traits compose
  into consuming classes and resolve through the existing class-constant paths.
  Non-public, typed, abstract/final/static, multi-constant declarations,
  conflicting trait/class constants, constant adaptations, interface
  constants, forward parent-interface resolution, exact diagnostics, and
  native lowering remain unsupported.

## Milestone 1262: Next Reference/COW WordPress Slice

- [x] Runtime lane: advance one large reference or copy-on-write blocker that
  affects WordPress mutable state. Milestone 1262 extends
  `call_user_func_array()` by-reference callback dispatch so string
  user-function callbacks and public `[object, method]` callbacks can receive a
  by-reference direct public object-property array-offset argument such as
  `array(&$cache->cache["options"]["alloptions"], "suffix")`. The callback
  path reuses the existing copy-in/writeback output-parameter bridge. Full PHP
  reference containers, stored reference arrays, keyed reference argument
  arrays, non-public/dynamic/append/ArrayAccess property arguments, dynamic
  static receivers, `parent::`, static method array-callable reference
  arguments, reference-returning function or method forms, broad COW, exact
  warnings, and native lowering remain unsupported.

## Milestone 1263: Next Request/SAPI/Filesystem WordPress Slice

- [x] Runtime or IR/lowering lane: implement one bounded request, SAPI,
  output-buffering, include-path, stream, filesystem, cookie, upload,
  superglobal, or header/output-started behavior that moves real WordPress
  bootstrap/request execution forward. Native lowering must reject runtime-only
  state explicitly when direct lowering would be misleading. Milestone 1263
  adds bounded `ob_get_contents()` support over the interpreter-owned output
  buffer stack: it returns the innermost active buffer contents without closing
  the buffer, returns `false` when no buffer is active, participates in
  function-table/dynamic-call lookup, and rejects direct native lowering under
  the existing output-buffer boundary. Output callbacks, chunk sizes, flags,
  flush/end variants, handler status/list APIs, output-started/header
  interaction, fatal-error cleanup, exact warnings, SAPI emission, and native
  output-buffer lowering remain unsupported.

## Milestone 1264: Next WordPress Database/Bootstrap Evidence Slice

- [x] Compatibility/runtime lane: advance the synthetic or operator-supplied
  WordPress probe to a later bootstrap point, or implement one small
  `wpdb`/MySQLi/object-cache/option behavior needed by such a probe. Avoid
  claiming real MySQL, arbitrary SQL, broad `wpdb`, persistent object cache,
  plugin/theme loading, full request support, or native support. Milestone
  1264 records deterministic per-handle `wp_options.option_id` values in the
  placeholder state island, exposes the exact
  `SELECT option_id FROM wp_options WHERE option_name = ... LIMIT 1` direct
  and prepared read shapes, and advances the synthetic add-option probe to
  prove the reinserted row's id survives duplicate rejection.

## Milestone 1265: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1261-1264 land, refresh support and
  compatibility docs around the remaining WordPress blockers, record focused
  verification, run the serialized full gate, checkpoint the batch, and open
  the next parallel implementation queue. Milestone 1265 keeps the next split
  focused on the explicit compatibility ledger in `GOAL.MD`: interface
  constants/forward resolution, broader references/COW, output-buffer and
  request-state breadth, and executable `wpdb`/database evidence. Full gate
  passed at checkpoint: `1388` fixture tests, `796` system PHP comparisons,
  and `592` skipped `phpc-only` fixtures.
...
- [x] Parser/runtime lane: advance one large trait/interface/object blocker
  ...
  WordPress-reached object-model construct. Milestone 1271 adds bounded forward
  parent-interface resolution for user interfaces declared later in the same
  parsed program. Parent methods and constants
  now participate in concrete class enforcement, relationship metadata,
  `Interface::CONST`, implementing-class `Class::CONST`, and
  `self::`/`static::` class-constant lookup after registration sees the full
  interface set. Missing parents and cyclic parent-interface inheritance remain
  stable runtime boundaries, and typed/static/non-public/abstract/final or
  multi-constant interface declarations, full variance, broad built-in/internal
  interface catalogs, exact PHP diagnostics, and native lowering remain
  unsupported. Proved with object-model tests, a milestone fixture, direct CLI
  run, and system PHP comparison.

## Milestone 1272: Next Reference/COW WordPress Slice

- [x] Runtime lane: add bounded `parent::` object-property array reference
  arguments. In active child class context, `parent::method(...)` and
  `parent::staticMethod(...)` now route direct public object-property
  array-offset arguments for reached by-reference parameters through the
  existing copy-in/writeback bridge, so WordPress-shaped cache slots such as
  `$cache->cache["options"]["alloptions"]` mutate after the parent call
  returns. This keeps non-public/dynamic/append/ArrayAccess object-property
  arguments, callback-dispatched `parent::` shapes, keyed callback reference
  argument arrays, stored reference arrays, full PHP reference containers,
  broad COW, exact warnings, and native lowering unsupported.
...
- [x] Compatibility/runtime lane: advance executable WordPress evidence from
  ...
  support, or native support. Milestone 1274 adds the exact direct and
  prepared
  `SELECT option_name, option_value, autoload FROM wp_options WHERE option_name = ... LIMIT 1`
  placeholder state-island shape and advances the synthetic add-option probe
  to print the full reinserted row after duplicate rejection.

## Milestone 1275: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1271-1274 land, refresh `GOAL.MD`,
  support, compatibility, progress, and lane-worker docs around the remaining
  blockers, record focused verification, run the serialized full gate,
  checkpoint the batch, and open the next parallel implementation queue.
  Milestone 1275 keeps the next split focused on the largest WordPress
  blockers that still prevent real bootstrap/request progress: coherent
  object/interface semantics, real reference/COW behavior, request/SAPI and
  filesystem/stream fidelity, and broader database/bootstrap execution
  evidence. Full gate passed at checkpoint: `1396` fixture tests, `802`
  system PHP comparisons, and `594` skipped `phpc-only` fixtures.

## Milestone 1276: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance a large object-model blocker from the
  `GOAL.MD` ledger. Prefer interface inheritance compatibility, method
  signature/visibility enforcement, trait/interface interactions, constructor
  and destructor behavior, autoloadable class/interface lookup, or another
  WordPress-reached object construct. Do not claim broad PHP object support
  unless executable code, focused tests, fixture CLI coverage, docs, and
  system-PHP comparison prove it. Milestone 1276 added bounded
  child-interface method compatibility checks for redeclared inherited methods
  and simple multi-parent inherited method conflicts, using the current
  required-parameter, parameter-type, and return-type metadata rules. Focused
  object-model tests and the `milestone1276` CLI fixture passed, including
  system PHP comparison.

## Milestone 1277: Reference/COW WordPress Blocker

- [x] Runtime lane: advance a large reference or copy-on-write blocker from
  the `GOAL.MD` ledger. Prefer keyed callback reference argument arrays,
  request-bag/global mutation, stored reference arrays, reference-returning
  paths, or a WordPress-reached mutable option/cache state path. Keep full PHP
  reference containers, broad COW, exact warnings, and native lowering
  unsupported unless implemented and tested.
  Closed by the bounded integer-keyed `call_user_func_array()` reference
  argument slice: literal callback argument arrays such as
  `array(10 => &$value, 20 => "suffix")` now route reached by-reference
  parameters through the existing direct-variable cell or public
  object-property array-offset copy-in/writeback paths for string, public
  object-method, and public class-string static callbacks. String-keyed named
  callback arguments, stored reference arrays, full reference containers, broad
  COW, exact warnings, and native lowering remain unsupported.

## Milestone 1278: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance a request/runtime integration
  blocker from the `GOAL.MD` ledger. Prefer `php://input`, request body/query
  parsing, filesystem or stream-wrapper behavior, include-path state, upload
  metadata, cookie/session behavior, shutdown-time buffer visibility, or a
  concrete WordPress request bootstrap obstacle. Milestone 1278 adds explicit
  CLI request seeds for flat URL-encoded query/form data and `php://input`:
  `PHPC_QUERY_STRING` initializes `$_GET`, `PHPC_REQUEST_METHOD=POST` plus
  `PHPC_CONTENT_TYPE=application/x-www-form-urlencoded` and
  `PHPC_REQUEST_BODY` initializes `$_POST`, `$_REQUEST` starts as GET merged
  with POST, and `file_get_contents("php://input")` returns the same bounded
  request body. Nested/bracketed request names, duplicate-key array
  collection, cookie merging, multipart uploads, variables/request-order
  policy, host SAPI state, exact warnings, and native lowering remain
  unsupported.

## Milestone 1279: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advance executable WordPress evidence by
  adding one broader `wpdb`/MySQLi/object-cache/option behavior or moving a
  deterministic WordPress bootstrap/request probe past its next real blocker.
  Avoid broad MySQL, arbitrary SQL, persistent cache, plugin/theme loading,
  full request support, or native support claims unless the behavior is
  actually implemented and covered. Milestone 1279 adds exact direct
  multi-row
  `SELECT option_name, option_value, autoload FROM wp_options` support over
  the placeholder MySQLi `wp_options` state island for all rows,
  autoload-filtered rows, and explicit `WHERE option_name IN (...)` row sets.
  The synthetic add-option inventory probe now seeds a second autoloaded row
  and prints the deterministic full-row set through a minimal `wpdb`
  `get_results()` wrapper. Real MySQL connectivity, arbitrary SQL, broad
  projections, persistent object cache, full WordPress option APIs,
  plugin/theme loading, full request support, references/COW fidelity, and
  native support remain unsupported.

## Milestone 1280: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1276-1279 land, refresh the
  ...
  checkpoint the batch. Milestone 1280 keeps the next split on the same
  large WordPress blockers rather than polishing peripheral surfaces:
  broader object/interface/trait compatibility, deeper reference and COW
  behavior, request/SAPI/filesystem/stream fidelity, and executable
  WordPress database/bootstrap evidence. Full gate passed at checkpoint:
  `1400` fixture tests, `804` system PHP comparisons, and `596` skipped
  `phpc-only` fixtures.

## Milestone 1281: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance another large object/interface blocker
  from `GOAL.MD`. Prefer trait/interface method conflict handling, class
  method compatibility against inherited interface signatures, constructor or
  destructor behavior reached by WordPress, autoloadable class/interface
  lookup, magic object hooks, or another WordPress-reached object construct.
  Keep broad object support, exact PHP diagnostics, autoloading, and native
  lowering unsupported unless executable code and tests prove them. Milestone
  1281 adds class-method precedence over same-named public trait methods and
  aliases in the effective method table, so concrete classes can override
  trait fallback methods, resolve same-name trait fallback collisions, and
  satisfy current interface method checks with the class-declared method.
  Broader trait conflict resolution, non-public/static/abstract/final trait
  methods, visibility-changing adaptations, exact PHP diagnostics,
  references/COW, autoloading, and native trait lowering remain unsupported.

## Milestone 1282: Reference/COW WordPress Blocker

- [x] Runtime lane: advance another large reference/COW blocker from
  `GOAL.MD`. Prefer stored reference arrays, request-bag/global mutation with
  aliases, reference-returning paths, callback reference arguments beyond the
  current literal-array slice, or mutable WordPress option/cache state.
  Continue naming full reference containers, broad COW, exact warnings, and
  native lowering as unsupported until implemented and tested. Milestone 1282
  adds bounded `call_user_func_array()` support for direct stored argument
  arrays whose reached by-reference slots were assigned by reference through
  covered direct array-offset targets, including direct variables, request-bag
  aliases, and public object-property array slots. Stored by-value arrays,
  named string keys, non-direct stored array expressions, full reference
  containers, broad COW, exact warnings, and native lowering remain
  unsupported.

## Milestone 1283: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: add bounded bracketed URL-encoded request parsing for the
  explicit CLI request seed. `PHPC_QUERY_STRING` and form POST
  `PHPC_REQUEST_BODY` now materialize keyed bracket arrays, repeated `[]`
  values, and duplicate scalar last-write-wins behavior for `$_GET`/`$_POST`,
  while `$_REQUEST` keeps the existing top-level GET-then-POST merge. Native
  request-state lowering remains explicitly rejected. Exact malformed
  `parse_str()` behavior, max-input-vars limits, dotted/spaced name
  normalization, cookie/session request imports, multipart uploads, and full
  SAPI state remain unsupported.

## Milestone 1284: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advance executable WordPress evidence by
  adding one broader `wpdb`/MySQLi/object-cache/option behavior or moving a
  deterministic WordPress bootstrap/request probe past its next real blocker.
  Avoid broad MySQL, arbitrary SQL, persistent cache, plugin/theme loading,
  full request support, and native support claims unless implemented and
  covered. Milestone 1284 adds exact direct and prepared
  `SELECT option_id, option_name, option_value, autoload FROM wp_options ...`
  placeholder state-island support for single option rows and direct all-row,
  autoload-filtered, and explicit-name-list row sets. The synthetic add-option
  inventory probe now prints deterministic option IDs for the final row set
  after delete/re-add and duplicate rejection. Real MySQL connectivity,
  arbitrary SQL, broad projections, persistent object cache, full WordPress
  option APIs, plugin/theme loading, full request support, references/COW
  fidelity, and native support remain unsupported.

## Milestone 1285: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1281-1284 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1285 keeps the next split focused on
  coherent subsystems needed for WordPress: broader trait/interface/class
  behavior, deeper reference/COW aliasing, request/SAPI/filesystem/stream
  fidelity, and executable WordPress database/bootstrap evidence. Full gate
  passed at checkpoint: `1404` fixture tests, `806` system PHP comparisons,
  and `598` skipped `phpc-only` fixtures.

## Milestone 1286: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advanced a bounded visibility-changing trait
  adaptation slice. Public instance trait methods may now be aliased as
  `protected` or `private`, with the aliases composed into class metadata,
  callable from valid class method context, visible to `method_exists()`, and
  omitted from global-context `get_class_methods()`. Visibility-only
  adaptations, original-method visibility changes without an alias, broad trait
  conflict handling, exact PHP diagnostics, autoloading, and native lowering
  remain unsupported.

## Milestone 1287: Reference/COW WordPress Blocker

- [x] Runtime lane: advance another reference/COW blocker from `GOAL.MD`.
  Prefer stored arrays routed through alias metadata, request-bag/global
  aliasing beyond the current callback slice, reference-returning paths,
  callback reference arguments beyond string/public array callables, or mutable
  WordPress option/cache state. Continue naming full reference containers,
  broad COW, exact warnings, and native lowering as unsupported until
  implemented and tested. Milestone 1287 lets normal
  `call_user_func_array()` invocation of user-function, public object-method,
  and public class-string static callbacks declared as returning by reference
  use the existing stored-array and public object-property array-offset
  by-reference argument bridge. Covered stored slots still must have been
  assigned by reference through existing alias metadata. Using
  `call_user_func_array()` itself as a statement-form reference-return source,
  closure/`__invoke` callbacks, dynamic static receiver callback shapes,
  non-public/dynamic/append/ArrayAccess property arguments, reference array
  literals stored by value, full reference containers, broad COW, exact
  warnings, and native lowering remain unsupported.

## Milestone 1288: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI, filesystem, or stream
  behavior that blocks real WordPress requests. Milestone 1288 normalizes dots
  and spaces in top-level URL-encoded request names from explicit
  `PHPC_QUERY_STRING` and form POST `PHPC_REQUEST_BODY` seeds before
  materializing `$_GET`, `$_POST`, and merged `$_REQUEST`. Bracket segment
  names remain literal, and cookies, sessions, multipart uploads,
  `variables_order`, host SAPI imports, references/COW, and native request
  lowering remain unsupported.

## Milestone 1289: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advance executable WordPress evidence by
  adding one broader `wpdb`/MySQLi/object-cache/option behavior. Milestone
  1289 adds exact direct and prepared `wp_options` value/autoload updates for
  the placeholder MySQLi state island and advances the deterministic
  add-option inventory probe so a reinserted option can be updated and read
  back through value/autoload and full-row paths. Real MySQL connectivity,
  arbitrary SQL, broad `WHERE`/expression support, persistent object cache,
  full WordPress option APIs, plugin/theme loading, full request support, and
  native support remain unsupported.

## Milestone 1290: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1286-1289 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1290 keeps the next split focused on larger
  missing pieces for WordPress: object/interface semantics, real
  reference/COW behavior, request/SAPI/filesystem/stream breadth, and
  executable database/bootstrap evidence. Full gate passed before checkpoint:
  `1408` fixture tests, `808` system PHP comparisons, and `600` skipped
  `phpc-only` fixtures.

## Milestone 1291: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advanced a bounded visibility-only trait adaptation
  slice. Public instance trait methods composed into a class can now use
  `as protected;` or `as private;` without creating an alias, changing the
  original composed method visibility while preserving `method_exists()` and
  global-context `get_class_methods()` filtering. Non-public/static/abstract/
  final trait methods, trait properties, broad conflict resolution, exact PHP
  diagnostics, autoloading, references/COW, and native lowering remain
  unsupported.

## Milestone 1292: Reference/COW WordPress Blocker

- [x] Runtime lane: advance a larger reference/COW blocker. Prefer request-bag
  or `$GLOBALS` alias mutation, reference-return alias binding, object-property
  or array-offset alias lifetimes beyond the current metadata bridges,
  by-reference iteration, or callback/property shapes reached by WordPress.
  Continue naming full reference containers, broad copy-on-write, exact
  warnings, and native lowering as unsupported until covered. Completed with a
  bounded `call_user_func_array()` reference-return alias source for
  user-function, public object-method array, and public static array callbacks
  over literal direct-variable reference argument elements.

## Milestone 1293: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added a bounded explicit cookie request seed. `PHPC_COOKIE`
  is parsed as a semicolon-delimited cookie header string into `$_COOKIE`, the
  raw seed is exposed as `$_SERVER["HTTP_COOKIE"]`, and top-level cookie names
  reuse the current dotted/spaced normalization. `$_REQUEST` still follows the
  existing GET-then-POST merge only. Host SAPI cookie imports, exact
  browser/raw-cookie parsing, quoted values, `variables_order`/`request_order`,
  cookie emission, references/COW, and native request lowering remain
  unsupported.

## Milestone 1294: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advance executable WordPress database or
  bootstrap evidence. Milestone 1294 adds the exact direct
  `UPDATE wp_options SET autoload = ... WHERE option_name = ...` shape to the
  placeholder MySQLi `wp_options` state island and advances the deterministic
  add-option inventory probe so a value-preserving autoload-only update is
  observed through value/autoload and full-row-with-id reads. Real MySQL,
  arbitrary SQL, prepared autoload-only updates, persistent cache,
  plugin/theme loading, full request support, and native support remain
  unsupported.

## Milestone 1295: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1291-1294 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1295 reconciles the missing full PHP and
  WordPress compatibility pieces into `GOAL.MD` as subsystem work rather than
  isolated reached slices, and keeps the next lanes focused on object
  semantics, references/COW, request/SAPI/filesystem/stream behavior, and
  executable database/bootstrap evidence. Full gate passed before checkpoint:
  `1412` fixture tests, `810` system PHP comparisons, and `602` skipped
  `phpc-only` fixtures.

## Milestone 1296: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance another object/interface blocker from
  `GOAL.MD`. Prefer constructor/destructor behavior, inherited method
  compatibility, trait conflict resolution beyond the current slices,
  class/interface autoload lookup scaffolding, magic object hooks, or
  reflection-visible metadata reached by WordPress. Keep broad reflection,
  exact diagnostics, references/COW, autoload fidelity, and native lowering
  unsupported until implemented and tested. Milestone 1296 implements the
  bounded multi-loser public instance trait conflict shape
  `TraitA::method insteadof TraitB, TraitC` for traits listed in the same
  class-body `use` declaration. The parser records each loser through the
  existing precedence metadata; composition keeps the selected winner,
  skips every named loser, and preserves current method dispatch,
  `method_exists()`, `get_class_methods()`, and interface method-presence
  behavior. Unqualified `insteadof`, broader conflict resolution,
  non-public/static/abstract/final trait methods, trait properties,
  `__TRAIT__`, exact diagnostics, references/COW, and native lowering remain
  unsupported.

## Milestone 1297: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced a bounded callback/property reference-return alias
  slice. Statement-form reference assignment from reference-returning
  `call_user_func_array()` user-function, public object-method array, and
  public static array callbacks now accepts literal direct public
  object-property array-offset reference elements for reached by-reference
  parameters and binds the assigned alias to the returned property slot when
  the callback returns that parameter. Full reference containers, broad
  copy-on-write, stored argument arrays for this reference-return alias path,
  non-public/dynamic/append/ArrayAccess property bridges, exact warnings, and
  native lowering remain unsupported.

## Milestone 1298: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI/filesystem/stream
  behavior needed for real WordPress requests. Prefer session state,
  uploaded-file metadata, include-path resolution, `php://temp`, stream
  resource handles, shutdown-time output/header visibility, or host SAPI import
  scaffolding. Milestone 1298 adds explicit `PHPC_FILES` URL-encoded uploaded
  file metadata seeding into `$_FILES`, with integer coercion for `error` and
  `size` leaves and root auto-global routing for function-scope reads. Multipart
  upload parsing, temporary uploaded files, host-upload validation,
  `is_uploaded_file()`/`move_uploaded_file()`, malformed metadata diagnostics,
  full SAPI import, references/COW, and native request-state lowering remain
  unsupported.

## Milestone 1299: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with the exact prepared
  `UPDATE wp_options SET autoload = ? WHERE option_name = ?` shape over the
  placeholder MySQLi `wp_options` state island. Existing option values are
  preserved while autoload mutates, missing option names report zero affected
  rows, and the synthetic add-option inventory probe now proves the prepared
  autoload-only change through value/autoload and full-row reads. Real MySQL,
  arbitrary SQL, broad prepared mutation SQL, persistent cache, plugin/theme
  loading, full request support, and native support remain unsupported.

## Milestone 1300: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1296-1299 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1300 keeps the next split focused on the
  same large missing pieces rather than tiny reached-shape cleanup: coherent
  object/interface/trait semantics, general references/COW, web/SAPI and stream
  state, and executable WordPress database/bootstrap evidence. Manual full gate
  passed before checkpoint: `cargo test` completed successfully, `phpc test`
  reported `1416` fixture tests passed with `0` failures, and `phpc test
...
## Milestone 1301: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance a larger object/interface blocker from
  `GOAL.MD`. Prefer constructor/destructor behavior, inherited method
  compatibility, autoloadable class/interface lookup scaffolding, magic object
  hooks, reflection-visible metadata reached by WordPress, or another broad
  trait/interface gap. Keep broad reflection, exact diagnostics,
  references/COW, autoload fidelity, and native lowering unsupported until
  implemented and tested. Milestone 1301 extends inherited non-private method
  compatibility beyond required-parameter counts: child overrides now reject
  adding a parameter type where the inherited method is untyped, changing a
  typed inherited parameter to a different type text, omitting a typed
  inherited return declaration, or changing that return type text. Child
  methods may still omit inherited parameter types and may add a return type
  when the inherited method is untyped. Full PHP variance, class/interface
  subtyping, union/intersection/DNF type compatibility, runtime type
  enforcement, exact PHP diagnostics, references/COW, autoload fidelity, and
  native lowering remain unsupported.

## Milestone 1302: Reference/COW WordPress Blocker

- [x] Runtime lane: advance a larger reference/COW blocker from `GOAL.MD`.
  Prefer `$GLOBALS` or request-bag alias mutation, reference-return alias
  binding beyond literal property slots, stored argument arrays for
  reference-return sources, by-reference iteration, or general alias lifetime
  cleanup. Continue naming full reference containers, broad copy-on-write,
  exact warnings, and native lowering as unsupported until covered.
  Completed as a bounded stored-argument-array reference-return alias-binding
  slice for `call_user_func_array()`: direct stored arrays whose reached
  by-reference slots were assigned by reference now bind statement-form
  reference-return aliases back to the covered direct-variable,
  request/global-bag, and public object-property alias group.

## Milestone 1303: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI, filesystem, or stream
  behavior that blocks real WordPress requests. Prefer sessions, header/output
  state, `php://input`/`php://temp`, stream resources, include-path behavior,
  host filesystem metadata, or multipart/upload follow-up work. Milestone
  1303 adds bounded `get_include_path()`, `set_include_path($path)`, and
  `PATH_SEPARATOR` support for `phpc run`, with relative include/require
  resolution checking the current source-file directory and then configured
  include-path entries. Exact PHP include-path ordering, host php.ini defaults,
  missing-file warning/`false` recovery, stream wrappers, phar/URL includes,
  opcache, autoload interaction, open_basedir, stat cache, references/COW, and
  native request/filesystem lowering remain unsupported.

## Milestone 1304: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with prepared no-placeholder `wp_options` row-set reads for the
  exact option-name/value/autoload and option-id/name/value/autoload SELECT
  shapes already accepted by direct `mysqli_query()`. A committed
  `wpdb`-shaped fixture proves a prepared `get_results()` wrapper can read all
  rows, autoload-filtered rows, and literal option-name-list rows over the
  deterministic placeholder state island. Real MySQL, arbitrary SQL, arbitrary
  projections, persistent cache, plugin/theme loading, full request support,
  references/COW, and native support remain unsupported.

## Milestone 1305: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1301-1304 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1305 keeps the next split focused on broad
  WordPress blockers: object/interface signature and lifecycle semantics,
  general reference/COW behavior, include/request/stream breadth, and
  database/bootstrap evidence. Manual full gate passed before checkpoint:
  `cargo test` completed successfully, `phpc test` reported `1420` fixture
  tests passed with `0` failures, and `phpc test --compare-php` reported
  `1420` fixture tests passed with `0` failures, `814` system PHP
  comparisons, and `606` skipped `phpc-only` fixtures.

## Milestone 1306: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded inherited/interface method
  signature compatibility slice beyond exact type text. Class registration now
  accepts simple declared class/interface contravariant parameter types and
  covariant return types for inherited methods, interface implementations,
  child-interface redeclarations, and simple multi-parent interface method
  conflicts when both type names resolve through current metadata. Keep full
  PHP variance, namespace-aware type-name resolution, union/intersection
  canonicalization, runtime type enforcement, exact diagnostics,
  references/COW, autoload fidelity, and native lowering unsupported until
  implemented and tested.

## Milestone 1307: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced a bounded by-reference iteration slice for direct
  string-keyed `$GLOBALS` roots. `foreach ($GLOBALS["bag"] as $key => &$value)`
  now routes the loop value to the selected root global array slot, visits
  appended tail entries, preserves post-loop lingering alias behavior until
  `unset($value)`, and works from function scope against the root symbol table.
  Nested `$GLOBALS["bag"]["child"]` iterables, non-string `$GLOBALS` roots,
  object/Traversable by-reference iteration, offset/property loop variables,
  full reference containers, broad copy-on-write, exact warnings, and native
  lowering remain unsupported.

## Milestone 1308: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advanced a bounded local filesystem
  metadata slice with `filesize($path)` for one string local path. Existing
  regular files return integer byte lengths; missing paths and non-file paths
  return `false`; stream-wrapper paths reject with a stable runtime boundary;
  string-valued dynamic calls and native metadata introspection recognize the
  builtin. Include-path metadata lookup, stream-wrapper metadata,
  `php://temp`, resources, stat-cache behavior, open_basedir, exact warnings,
  non-UTF-8 paths, broad coercions, and native filesystem metadata lowering
  remain unsupported.

## Milestone 1309: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with prepared no-placeholder `wp_options` row-set reads for the
  exact alloptions-style
  `SELECT option_name, option_value FROM wp_options ...` projection, including
  all rows, autoload-filtered rows, literal option-name-list rows, and the
  current backticked column/table spelling. A committed `wpdb`-shaped fixture
  proves a prepared `get_results()` wrapper can read deterministic
  option-name/value rows from the placeholder state island. Real MySQL,
  arbitrary SQL, arbitrary projections beyond the documented exact option
  shapes, persistent cache, plugin/theme loading, full request support,
  references/COW, and native support remain unsupported.

## Milestone 1310: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1306-1309 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Milestone 1310 keeps the next split focused on broad
  WordPress blockers: object/interface signature and lifecycle semantics,
  general reference/COW behavior, filesystem/stream/request breadth, and
  database/bootstrap evidence. Manual full gate passed before checkpoint:
  `cargo test` completed successfully, `phpc test` reported `1424` fixture
  tests passed with `0` failures, and `phpc test --compare-php` reported
  `1424` fixture tests passed with `0` failures, `817` system PHP
  comparisons, and `607` skipped `phpc-only` fixtures.

## Milestone 1311: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advanced object lifecycle behavior with public
  non-static no-argument `__destruct()` shutdown dispatch. `phpc run` now
  tracks allocated and shallow-cloned objects whose class declares or inherits
  a supported destructor and runs them at normal shutdown or after `exit` in
  reverse allocation order. Static/private/protected destructors, destructor
  parameters, runtime-error destructor paths, exact fatal/shutdown/GC ordering,
  cyclic GC, handle reuse, `__clone` dispatch, full references/COW, and native
  lowering remain unsupported.

## Milestone 1312: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced by-reference iteration beyond direct string-keyed
  global roots for a bounded nested array/request-bag shape. `phpc run` now
  supports by-reference `foreach` over direct array-offset roots such as
  `$items["outer"]`, root request bags such as `$_REQUEST["payload"]`, and
  string-keyed nested `$GLOBALS` paths such as `$GLOBALS["bag"]["child"]`,
  preserving loop-body mutation, appended tail visitation, and post-loop
  lingering aliases for those selected slots. Full PHP reference containers,
  broad copy-on-write, object-property/ArrayAccess iterables, reference-return
  iterables, exact warnings, and native lowering remain unsupported.

## Milestone 1313: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI, filesystem, or stream
  behavior that blocks real WordPress requests. Prefer `filemtime()`/stat
  metadata, include-path follow-up behavior, `php://input`, `php://temp`,
  stream resources, sessions, header/output state, multipart/upload handling,
  shutdown visibility, or host filesystem fidelity. Keep native lowering
  rejected for request/stream/filesystem state unless runtime support and tests
  exist. Milestone 1313 adds bounded `filemtime($path)` stat metadata for one
  string local path. Existing local filesystem entries return host
  modification times as Unix-timestamp integers; missing paths return `false`;
  stream-wrapper paths reject with a stable runtime boundary; string-valued
  dynamic calls and native metadata introspection recognize the builtin.
  Include-path metadata lookup, stream-wrapper metadata, `php://temp`, stream
  resources, stat-cache behavior, `open_basedir`, exact warnings, non-UTF-8
  paths, pre-Unix-epoch timestamps, broad coercions, and native filesystem
  metadata lowering remain unsupported.

## Milestone 1314: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with exact `DELETE FROM wp_options WHERE option_name IN (...)`
  support over the placeholder MySQLi `wp_options` state island, including
  deterministic affected-row metadata and the no-params
  `mysqli_execute_query($handle, $query)` path. A committed transient-shaped
  `wpdb`/object-cache fixture proves deleting value and timeout option rows
  while clearing the bounded in-memory cache slot. Real MySQL, arbitrary SQL,
  prepared name-list binding, persistent cache, plugin/theme loading, full
  request support, references/COW, and native support remain unsupported.

## Milestone 1315: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1311-1314 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1428` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1428`
  fixture tests passed with `0` failures, `820` system PHP comparisons, and
  `608` skipped `phpc-only` fixtures.
...
- [x] Parser/runtime lane: advanced a larger object lifecycle blocker from
  `GOAL.MD`: `clone $object` now allocates a fresh handle, shallow-copies the
  current property slots, and dispatches declared or inherited visible
  non-static `__clone()` methods on the cloned object. Coverage includes Rust
  object-model tests, a committed `milestone1321` CLI fixture, system-PHP
  comparison, removal of the obsolete unsupported clone fixture, documentation,
  and named gaps. Full PHP clone lifecycle fidelity, exact `Error`
  objects/wording/timing, destructor/GC ordering, references/COW, autoload
  fidelity, and native lowering remain unsupported.

## Milestone 1322: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced a reusable reference-lifetime cleanup slice for
  function-scope `global` imports: importing `global $name;` now clears a
  same-name stale local direct array-offset alias binding so reads and writes
  rebind to the root global symbol slot, matching PHP's global rebinding shape
  instead of continuing through local alias metadata. Coverage includes a Rust
  runtime test plus a committed `milestone1322` CLI fixture exercised through
  `phpc run` and system-PHP comparison. This does not add full reference
  containers, broad copy-on-write, exact warning text/timing, full alias
  destruction ordering, resource/object edge cases, or native lowering.

## Milestone 1323: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advanced a bounded request/filesystem slice:
  `file_get_contents($path, $use_include_path = false)` now accepts an optional
  bool include-path flag and, for relative local paths with `true`, follows the
  current include-path candidate order shared with `include`/`require`.
  Coverage includes Rust tests, a committed `milestone1323` CLI fixture,
  system-PHP comparison, documentation, and named gaps. Stream resources,
  `php://temp`/`php://memory`, stream contexts, offset/length args beyond this
  bool flag, multipart/uploads, missing-file warning plus `false` parity,
  session persistence/cookies, and native lowering remain unsupported.

## Milestone 1324: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence by adding exact prepared option-name-list result-set reads over the
  placeholder MySQLi `wp_options` state island. The slice supports
  ...
  through `mysqli_stmt_execute()`/`mysqli_stmt_get_result()` and
  `mysqli_execute_query(...)` for string option-name placeholders, preserving
  placeholder order and skipping missing names. Real MySQL, arbitrary SQL,
  non-string option-name params, full request support, plugin/theme support,
  references/COW, and native support remain unsupported.

## Milestone 1325: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1321-1324 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1435` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1435`
  fixture tests passed with `0` failures, `825` system PHP comparisons, and
  `610` skipped `phpc-only` fixtures.
...
- [x] Parser/runtime lane: advance the next larger object/interface blocker
  ...
  reflection-visible metadata, trait/interface edge cases, or destructor/clone
  lifecycle ordering beyond the current slices. Keep full variance, runtime
  type enforcement, exact diagnostics, references/COW, autoload fidelity, and
  native lowering unsupported until implemented and tested.
...
- [x] Parser/runtime lane: advance a larger object/interface subsystem gap
  ...
  executable code and tests prove them. Closed with a bounded autoloadable
  class/interface lookup slice: string user-function callbacks registered by
  `spl_autoload_register()` are stored, honor the current boolean `prepend`
  flag, and are invoked by truthy-autoload `class_exists()`/`interface_exists()`
  misses so local include/require callbacks can register class/interface
  metadata.

## Milestone 1332: Reference/COW WordPress Blocker

- [x] Runtime lane: advance a larger reference/COW subsystem gap from
  ...
  PHP alias destruction ordering, resource/object edge cases, and native
  lowering unsupported until implemented and tested. Closed with a bounded
  by-reference `foreach` extension for direct visible instance-method and
  direct named-static-method reference-return iterable roots, where the
  returned direct variable is backed by a caller variable cell through the
  current by-reference parameter path.

## Milestone 1333: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advanced a bounded stream/filesystem slice:
  `phpc run` now supports interpreter-owned local UTF-8 file stream resources
  for simple `fopen($path, $mode)` modes alongside the existing
  `php://memory`/`php://temp` resources. `fwrite()`, `fread()`, `rewind()`,
  `stream_get_contents()`, and `fclose()` operate on local file handles, with
  append-mode writes routed to EOF and native stream-resource calls still
  rejected at the dedicated codegen boundary. Unsupported wrappers, sockets,
  HTTP/FTP/phar, stream contexts/filters, binary/non-UTF-8 byte fidelity,
  permissions policy, locking, exact warning/`false` recovery, native stream
  resources, and broader PHP resource metadata remain explicit gaps.

## Milestone 1334: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with exact prepared `wp_options` autoload-list result-set reads for
  name/value, name/value/autoload, and id/name/value/autoload projections
  through both `mysqli_stmt_execute()`/`mysqli_stmt_get_result()` and
  `mysqli_execute_query($handle, $query, array(...))`. A committed
  `milestone1334` `wpdb`-shaped fixture proves deterministic
  autoload-filtered rows and backticked column/table spelling support over the
  placeholder state island. Real MySQL, arbitrary SQL, full request support,
  plugin/theme support, references/COW, and native support remain unsupported.

## Milestone 1335: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1331-1334 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1443` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1443`
  fixture tests passed with `0` failures, `832` system PHP comparisons, and
  `611` skipped `phpc-only` fixtures.

## Milestone 1336: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance the next larger object/interface subsystem
  gap from `GOAL.MD`. Selected candidate: missing named and direct-variable
  string class names in `new` expressions now invoke the existing string
  user-function autoload callback stack before class metadata is rechecked.
  `extends`, `implements`, trait/enum lookup, namespace/import
  canonicalization beyond current string lookup, closure/array-callable
  autoload invocation, full type enforcement, full variance, exact PHP
  diagnostics, references/COW, and native lowering remain unsupported until
  executable code and tests prove them.

## Milestone 1337: Reference/COW WordPress Blocker

- [x] Runtime lane: extend by-reference `foreach` reference-return iterable
  roots to the existing bounded `call_user_func_array()`, method-context
  `self::`, `parent::`, `static::`, and dynamic static receiver
  reference-return source machinery. The executable slice requires the called
  function or method to return by reference, return a direct variable, and bind
  that variable to a caller cell through the existing by-reference parameter or
  direct static-local path. Keep real reference containers, broader
  array/object copy-on-write, alias lifetime across include/function
  boundaries, ArrayAccess references, request/session-state alias behavior,
  exact warning behavior, full PHP alias destruction ordering, resource/object
  edge cases, magic `__callStatic` iterable roots, property/offset/expression
  reference-return iterable roots, and native lowering unsupported until
  implemented and tested.

## Milestone 1338: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advanced a bounded stream status/position
  slice over the existing interpreter-owned memory/temp/local-file stream
  resources. `phpc run` now supports `feof($stream)`, `ftell($stream)`, and
  `fseek($stream, $offset, $whence = SEEK_SET)`, including the `SEEK_SET`,
  `SEEK_CUR`, and `SEEK_END` built-in constants, and direct native
  stream-resource calls still reject at the dedicated codegen boundary.
  Unsupported wrappers, sockets, HTTP/FTP/phar, stream contexts/filters,
  `fstat()`/`stream_get_meta_data()`, permissions policy, locking, exact
  warning/`false` recovery, binary/non-UTF-8 byte fidelity, native stream
  resources, and broader PHP resource metadata remain explicit gaps.

## Milestone 1339: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with exact `SELECT option_name, autoload FROM wp_options ...`
  result-set reads over the deterministic MySQLi placeholder state island.
  Direct and prepared paths cover all rows, literal autoload filters, literal
  name lists, prepared name lists, and prepared autoload lists for the current
...
## Milestone 1340: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1336-1339 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1447` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1447`
  fixture tests passed with `0` failures, `835` system PHP comparisons, and
  `612` skipped `phpc-only` fixtures.

## Milestone 1341: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: advance the next larger object/interface subsystem
  gap from `GOAL.MD`, preferably autoload-triggered `extends`/`implements`
  dependency lookup, constructor/default fidelity reached by WordPress,
  interface/trait edge cases, magic object hooks, reflection metadata, or
  lifecycle ordering beyond the current destructor/clone slices. Keep full
  type enforcement, broad variance, exact diagnostics, references/COW, and
  native lowering unsupported until executable code and tests prove them.
  Completed with a bounded included-declaration autoload dependency slice:
  class declarations loaded by executed include/require paths now invoke
  current string user-function autoload callbacks for missing `extends`
  parent classes and direct `implements` interface names, and autoloaded
  interface declarations do the same for missing parent interfaces before
  registration validation.

## Milestone 1342: Reference/COW WordPress Blocker

- [x] Runtime lane: advance the next real reference/COW blocker, preferably
  ...
  reference-return expression shape that removes a current WordPress blocker.
  Added bounded alias mirroring when copying literal-key nested direct array
  paths containing covered reference elements, including request-bag paths such
  as `$_REQUEST["payload"]`; variable/dynamic/append copied path keys, broad
  COW containers, ArrayAccess, exact alias ordering, and native lowering remain
  unsupported.

## Milestone 1343: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime/IR lane: add bounded stream metadata for current
  interpreter-owned memory/temp/local-file resources. `phpc run` now supports
  `fstat($stream)` and `stream_get_meta_data($stream)` for the existing
  `php://memory`, `php://temp`, and local UTF-8 file handles. The covered
  metadata reports memory/temp buffer sizes, local host-file stat arrays,
  wrapper/type/mode/URI fields, seekability, unread bytes, and the current
  bounded EOF flag. Direct native lowering continues to reject stream-resource
  calls through the dedicated codegen boundary, and function-table
  introspection recognizes the new names. This does not add unsupported
  wrappers, sockets, directory handles, stream contexts/filters, binary byte
  fidelity, `php://temp` spillover, stat-cache behavior, permissions policy,
  locking, exact warnings/`false` recovery, native resources, or broad request
  fidelity.

## Milestone 1344: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with exact `SELECT option_name FROM wp_options ...` name-only
...
## Milestone 1345: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1341-1344 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1451` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1451`
  fixture tests passed with `0` failures, `838` system PHP comparisons, and
  `613` skipped `phpc-only` fixtures.
...
- [x] Parser/runtime lane: added a bounded array-callable autoload slice for
  public `[object, "method"]` instance callbacks and public
  `["ClassName", "method"]` static callbacks registered through
  `spl_autoload_register()`. The callbacks honor the current prepend flag and
  participate in truthy-autoload class/interface/trait metadata lookup,
  missing `new` class instantiation, and included class/interface/trait
  declaration dependency loading. Remaining gaps include closure invocation,
  invokable objects, non-public methods, class-string non-static methods,
  object static methods, arbitrary callable arrays, enum autoloading, exact PHP
  warning/throw behavior, references/COW, and native lowering.

## Milestone 1352: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded `call_user_func_array()` reference bridge
  for literal argument arrays containing direct array-offset by-reference
  elements. String user-function callbacks, public object array callbacks, and
  public static array callbacks can now write back reached ordinary,
  `$GLOBALS`, and request-bag array-offset paths through the existing bounded
  alias/writeback bridge. This remains a copy-in/writeback slice, not real
  reference containers, broad COW, ArrayAccess references, append-offset
  callback roots, exact warnings, or native lowering.

## Milestone 1353: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: added bounded `fopen("php://input", $mode)`
  support through interpreter-owned stream resources. `phpc run` seeds the
  read buffer from `PHPC_REQUEST_BODY`, exposes deterministic metadata, reuses
  current read/seek/cursor/close paths, and keeps writable modes non-writable.
  Native lowering still rejects stream resources. Real SAPI body lifetime,
  contexts, filters, wrappers, uploads, cookies/sessions, binary fidelity,
  exact warnings, and native stream resources remain unsupported.

## Milestone 1354: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added exact `SELECT * FROM wp_options`
  result rows over the deterministic MySQLi `wp_options` placeholder island.
  Direct, prepared, and `mysqli_execute_query()` paths now cover single-row
  name reads, all rows, literal and prepared option-name lists, and literal and
  prepared WordPress autoload filters for the current string-parameter subset.
  Arbitrary SQL, joins, aliases, ordering, real MySQL, persistent cache,
  plugin/theme loading, request fidelity, references/COW, and native database
  lowering remain unsupported.

## Milestone 1355: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1351-1354 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1459` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1459`
  fixture tests passed with `0` failures, `844` system PHP comparisons, and
  `615` skipped `phpc-only` fixtures.

## Milestone 1356: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded static-method string autoload
  callback slice for `spl_autoload_register("ClassName::method")`. Public
  static methods registered with that string callable shape now honor current
  callback ordering/prepend storage and are invoked for truthy-autoload
  `class_exists()`/`interface_exists()`/`trait_exists()` misses, missing `new`
  class instantiation, and included class/interface/trait declaration
  dependencies through the existing bounded autoload path. Closure invocation,
  invokable objects, non-public methods, class-string non-static methods,
  object static methods, `self::`/`parent::`/`static::` callback strings,
  `spl_autoload_functions()`, exact callable diagnostics, enum autoloading,
  references/COW, and native lowering remain unsupported.

## Milestone 1357: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced a bounded request/session alias blocker:
  `$_SESSION` now participates in the same root auto-global routing as the
  request bags after `session_start()`, so direct function-scope reads/writes
  update the root session array and covered nested reference aliases such as
  `$alias =& $_SESSION["payload"]["slot"]` observe those writes across
  function and `session_write_close()` boundaries. This does not add real
  reference containers, persistent sessions, session cookies/locking/save
  handlers, exact warning behavior, full alias destruction ordering, broader
  copy-on-write, or native lowering.

## Milestone 1358: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI/filesystem/stream
  breadth that blocks real WordPress requests, such as upload/session/cookie
  persistence, header/output/shutdown fidelity, include-path/stat-cache
  behavior, stream context metadata, wrapper behavior, request-body lifetime,
  or host filesystem edge cases. Native lowering must continue to reject
  unsupported resource behavior explicitly. Closed by the bounded stream
  context resource slice: `stream_context_create()` stores array options,
  `stream_context_get_options()` returns them, and current local/`php://input`
  `file_get_contents()` plus local `fopen()` paths accept context resources
  while leaving wrapper-specific effects unsupported.

## Milestone 1359: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database and
  bootstrap evidence with bounded transient-shaped `wp_options` option-name
  prefix scans over the deterministic MySQLi placeholder state island.

## Milestone 1360: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1356-1359 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1463` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1463`
  fixture tests passed with `0` failures, `847` system PHP comparisons, and
  `616` skipped `phpc-only` fixtures.

## Milestone 1361: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: implemented a bounded invokable-object autoload
  callback slice for `spl_autoload_register($loader)` when `$loader` has a
  public non-static `__invoke($name)` method. The callback participates in the
  existing truthy-autoload class/interface/trait lookup path, missing `new`
  class instantiation, and included declaration dependency loading, with
  unsupported gaps kept explicit for non-public/static `__invoke`, general
  invokable-object calls, unregister/function-list APIs, closure invocation,
  exact PHP callable diagnostics, enum autoloading, and native lowering.

## Milestone 1362: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded direct free-function reference-return
  assignment bridge for covered direct array-offset and public object-property
  array-offset by-reference arguments. Remaining reference/COW blockers include
  real reference containers, broader array/object copy-on-write, ArrayAccess
  reference roots, alias cleanup across include/function boundaries,
  method/static direct-call array-slot reference-return arguments, non-direct
  reference-return expression shapes, stored callback/direct-call argument
  arrays beyond the covered slices, and request/session alias behavior.

## Milestone 1363: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advanced request/SAPI/filesystem/stream
  breadth that blocks real WordPress requests, such as upload/session/cookie
  persistence, header/output/shutdown fidelity, default stream context
  behavior, stream option effects, filters, include-path/stat-cache behavior,
  wrapper behavior, request-body lifetime, or host filesystem edge cases.
  Closed by the bounded default stream-context mutation slice:
  `stream_context_get_default()` now returns one request-local default context,
  `stream_context_set_default()` and `stream_context_set_option()` persist
  string-keyed wrapper options, and `stream_context_get_options()` exposes the
  merged options. Context option effects on wrappers, filters, context params,
  HTTP/FTP/phar/socket wrappers, binary/non-UTF-8 fidelity, exact warning
  recovery, and native resources remain unsupported. Native lowering continues
  to reject stream resources explicitly.

## Milestone 1364: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advance executable WordPress database or
  bootstrap evidence, preferably broader `wpdb`/MySQLi result and mutation
  behavior, prepared transient prefix scans, object-cache/transient
  persistence, option API state, hooks under realistic callback shapes,
  deterministic plugin/theme loading probes, or a committed bootstrap/request
  probe that moves past its next real blocker. Closed with bounded prepared
  `wp_options` `option_name LIKE ?` prefix result scans over the deterministic
  MySQLi placeholder state island, including a committed `wpdb`-shaped
  transient scan fixture.

## Milestone 1365: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1361-1364 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1467` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1467`
  fixture tests passed with `0` failures, `850` system PHP comparisons, and
  `617` skipped `phpc-only` fixtures.

## Milestone 1366: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target a larger object/interface blocker such as
  constructor/default fidelity, interface/trait edge cases, magic object hooks
  reached by WordPress, reflection metadata, static member lookup
  interactions, or autoload lifecycle ordering beyond the current bounded
  callback slices. Keep unsupported callable shapes and native lowering
  explicit. Closed with bounded autoload lifecycle support:
  `spl_autoload_functions()` exposes the current stored callback list for
  function strings, public static-method strings/arrays, public object-method
  arrays, invokable objects, and inert closures; `spl_autoload_unregister()`
  removes matching bounded callbacks so later class/interface/trait lookup and
  missing `new` autoload dispatch observe the new order. Closure invocation,
  nonexistent string callback validation, broader callable shapes, exact PHP
  diagnostics, enum autoloading, references/COW, and native lowering remain
  unsupported.

## Milestone 1367: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded method/static direct-call
  reference-return assignment bridge for covered direct array-offset and
  public object-property array-offset by-reference arguments, building on the
  Milestone 1362 free-function bridge. Current dispatch coverage includes
  direct visible object methods, named static methods, method-context
  `self::`/`parent::`/`static::` static calls, and dynamic static receivers
  when the existing method metadata already reports by-reference parameters
  and return-by-reference. Remaining reference/COW blockers include real
  reference containers, broader array/object copy-on-write, ArrayAccess
  reference roots, alias lifetime cleanup across function/include boundaries,
  non-direct reference-return expression shapes, non-public/dynamic property
  reference roots, stored direct-call argument arrays beyond the covered
  slices, request/session alias behavior, and native lowering.

## Milestone 1368: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI/filesystem/stream
  breadth that blocks real WordPress requests, such as upload/session/cookie
  persistence, header/output/shutdown fidelity, include-path/stat-cache
  behavior, stream context params/effects, filters, additional wrappers,
  request-body lifetime, or host filesystem edge cases. Native lowering must
  continue to reject unsupported resource behavior explicitly. Milestone 1368
  makes deterministic CLI `header()` state honor bounded ordinary
  colon-header replacement: default replacement removes earlier
  ASCII-case-insensitive matches before appending the new raw line, while
  `$replace = false` appends duplicates. The committed fixture exercises
  `header()`, `headers_list()`, and `headers_sent()` output-started
  bookkeeping. Real web SAPI emission, status-code/state handling,
  output-buffer edge cases beyond current output-started tracking, exact PHP
  warnings, and native lowering remain explicit gaps.

## Milestone 1369: WordPress DB/Bootstrap Evidence Blocker
...
- [x] Compatibility/runtime lane: advance executable WordPress database or
  ...
  persistence, option API state, hooks under realistic callback shapes,
  deterministic plugin/theme loading probes, or a committed bootstrap/request
  probe that moves past its next real blocker. Closed with bounded direct and
  prepared `wp_options` `option_name LIKE` prefix deletes over the
  deterministic MySQLi placeholder state island, including a committed
  `wpdb`-shaped transient cleanup fixture.

## Milestone 1370: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1366-1369 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1471` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1471`
  fixture tests passed with `0` failures, `852` system PHP comparisons, and
  `619` skipped `phpc-only` fixtures.

## Milestone 1371: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded `spl_autoload_call($class)` lifecycle
  slice beyond the list/unregister surface. Manual calls now accept one string
  class/interface/trait name, invoke the current stored bounded non-closure
  callback list in registration order, stop once any class-like metadata with
  that name exists, and return `null`. The fixture proves string and static
  method callbacks, unregister interaction, class/interface/trait loads, and
  system PHP comparison. Closure invocation, exact callable validation,
  `spl_autoload_extensions`, enum autoload lookup, namespace/import
  canonicalization beyond current string lookup, exact PHP diagnostics,
  references/COW, and native lowering remain unsupported.

## Milestone 1372: Reference/COW WordPress Blocker

- [x] Runtime lane: closed a bounded non-direct reference-return expression
  shape for WordPress-like parent array slots. Statement-form reference
  assignment from direct free-function, visible object-method, and named static
  reference-return calls can now bind `return $param[$key];` back to a covered
  by-reference parent array/property-slot argument such as
  `$_REQUEST["payload"]`, `$items["outer"]`, or
  `$object->cache["options"]`. Remaining reference/COW gaps still include real
  reference containers, direct-variable parent containers for this child-slot
  return shape, ArrayAccess reference roots, alias lifetime cleanup across
  function/include boundaries, broader array/object copy-on-write,
  non-public/dynamic property reference roots, stored direct-call argument
  arrays beyond the covered slices, request/session alias breadth, exact alias
  destruction ordering, and native lowering.

## Milestone 1373: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: advance request/SAPI/filesystem/stream
  breadth that blocks real WordPress requests, such as upload/session/cookie
  persistence, status-code/header edge cases, output/shutdown fidelity,
  include-path/stat-cache behavior, stream context params/effects, filters,
  additional wrappers, request-body lifetime, or host filesystem edge cases.
  Native lowering must continue to reject unsupported resource behavior
  explicitly. Closed by the bounded response-status slice:
  `http_response_code()` now reads/writes request-local status state,
  accepted `header()` calls update status from explicit non-zero integer
  response-code arguments and `HTTP/... NNN` status lines, `Location:`
  defaults to `302` unless the current status is `201` or already a redirect,
  zero response-code arguments are treated as no explicit status, and
  `header_remove($name)` removes ordinary header names
  ASCII-case-insensitively. Direct native header-state calls still reject
  explicitly.

## Milestone 1374: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added a bounded alloptions/autoload equality
  `wp_options` read slice over the deterministic MySQLi placeholder state
  island. Direct row-set reads now accept exact
  `SELECT option_name, option_value FROM wp_options WHERE autoload = 'yes'`
  shapes, and prepared `mysqli_stmt_execute()` plus `mysqli_execute_query()`
  accept exact
  `SELECT option_name, option_value FROM wp_options WHERE autoload = ?`
  shapes for one string autoload parameter, including backticked
  table/column spelling. The committed `milestone1374` fixture proves a
  `wpdb`-shaped legacy alloptions load and prepared autoload probes. Real
  MySQL, arbitrary SQL, persistent object cache, full option APIs, and native
  DB lowering remain unsupported.

## Milestone 1375: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1371-1374 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1475` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1475`
  fixture tests passed with `0` failures, `855` system PHP comparisons, and
  `620` skipped `phpc-only` fixtures.

## Milestone 1376: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded `spl_autoload_extensions()` autoload
  lifecycle slice. The interpreter now tracks a request-local SPL autoload
  extension string initialized to `.inc,.php`; calls with no argument or
  `null` read it, string arguments replace it and return the new value, and
  dynamic string calls plus function-table/native rejection paths recognize the
  builtin. The new fixture proves default/read/set/null/dynamic-call behavior
  against system PHP. Default `spl_autoload()` file probing through
  the registry, scalar-to-string coercions for extension arguments, closure
  autoload invocation, exact callable diagnostics, enum autoload lookup,
  broader namespace/import canonicalization, references/COW, and native
  lowering remain unsupported.

## Milestone 1381: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded default `spl_autoload()` local file
  probing through the current SPL extension registry. Direct
  `spl_autoload($class)` calls and registered `"spl_autoload"` callbacks now
  lowercase the requested class/interface/trait name, map namespace separators
  to local path separators, probe comma-separated extensions through the
  current include resolver, include the first existing local file once, and
  return `null`. The new fixture proves direct, dynamic-string, namespaced,
  and registered-default callback paths against system PHP. Remaining gaps
  include stream/URL/default-extension file probing, scalar-to-string
  extension coercions, exact warnings/throw behavior, enum autoload lookup,
  broader namespace/import canonicalization, recursive loader edge cases
  beyond the current same-name guard, references/COW, and native lowering.

## Milestone 1377: Reference/COW WordPress Blocker

- [x] Runtime lane: closed a bounded direct-variable parent-container
  reference/COW blocker. Statement-form reference assignment from direct
  free-function, visible object-method, and named static reference-return
  calls can now bind `return $param[$key];` back to the caller when `$param`
  was supplied by a direct variable parent array such as `$items`. Remaining
  reference/COW gaps still include real reference containers, ArrayAccess
  reference roots, alias lifetime cleanup across function/include boundaries,
  broader array/object copy-on-write, non-public/dynamic property reference
  roots, callback/stored argument-array parents for child-slot returns,
  request/session alias behavior, exact alias destruction ordering, and native
  lowering.

## Milestone 1378: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: added bounded upload provenance for the
  explicit `PHPC_FILES` CLI seed. `tmp_name` entries with sibling `error=0`
  now make `is_uploaded_file($path)` true while that local regular file still
  exists, and `move_uploaded_file($from, $to)` moves a registered local path
  once before clearing source provenance. The fixture and focused Rust test
  cover the WordPress-shaped upload check/move path, and direct native calls
  reject under the stream-resource boundary. Remaining gaps include
  multipart/form parsing, runtime temporary upload creation, host-upload
  validation beyond the explicit seed, failed-upload provenance, broader
  nested multi-file metadata, exact PHP warnings, permission/TOCTOU fidelity,
  references/COW, and native upload-state lowering.

## Milestone 1379: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: advanced executable WordPress database
  evidence with a bounded prepared transient-shaped option-value read. The
  deterministic MySQLi `wp_options` state island now accepts exact prepared
  ...
  `mysqli_execute_query($handle, $query, array($name))`. The committed
  `milestone1379` fixture proves a `wpdb`-shaped transient `get_var()` value,
  timeout, and missing-option probe. Real MySQL, arbitrary SQL, persistent
  object cache/transient persistence beyond this state slice, full option
  APIs, and native DB lowering remain unsupported.

## Milestone 1380: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1376-1379 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1479` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1479`
  fixture tests passed with `0` failures, `857` system PHP comparisons, and
  `622` skipped `phpc-only` fixtures.

## Milestone 1381: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded default `spl_autoload()` local file
  probing through the current SPL autoload extension registry. `phpc run` now
  accepts direct and dynamic-string `spl_autoload()` calls plus registered
  `"spl_autoload"` callbacks, lowercases class-like names, maps namespace
  separators to path separators, probes comma-separated extensions through the
  existing include resolver, and includes the first existing local file once.
  Stream/URL/default-extension probing, scalar-to-string extension coercions,
  exact warning/throw behavior, enum autoload lookup, broader
  namespace/import canonicalization, recursive loader edge cases beyond the
  current guard, references/COW, and native lowering remain unsupported.

## Milestone 1382: Reference/COW WordPress Blocker

- [x] Runtime lane: extended the direct-parent child-slot reference-return
  slice to explicit nested child suffixes. Statement-form reference assignment
  from direct free-function, visible object-method, and named static
  reference-return calls now has committed evidence for binding
  `return $param[$key][$subkey];` back to the caller when `$param` was
  supplied by a direct variable parent array such as `$items`, including direct
  parent alias groups. The implementation already used the general explicit
  index-path collector; this milestone adds the missing regression coverage,
  CLI fixture, and support documentation. Remaining reference/COW gaps still
  include real reference containers, ArrayAccess reference roots, alias
  lifetime cleanup across function/include boundaries, broader array/object
  copy-on-write, non-public/dynamic property reference roots,
  callback/stored argument-array parents for child-slot returns,
  request/session alias behavior, exact alias destruction ordering, and native
  lowering.

## Milestone 1383: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests. Milestone 1383 adds bounded
  interpreter support for no-argument `ob_get_length()` over the existing
  output-buffer stack: it returns the innermost active buffer byte length, or
  `false` when no buffer is active. Dynamic-call visibility and native
  rejection now recognize the builtin. Output callbacks, chunk sizes, flags,
  handler status/list APIs, exact multibyte handler semantics,
  output-started/header interaction beyond existing flush tracking,
  fatal-error cleanup, exact warnings, and native lowering remain unsupported.

## Milestone 1384: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added exact no-`LIMIT` full-row
  `SELECT * FROM wp_options WHERE option_name = ...` and prepared
  `SELECT * FROM wp_options WHERE option_name = ?` reads over the deterministic
  MySQLi `wp_options` state island, with `mysqli_fetch_object()` consuming the
  resulting option rows through a WordPress-shaped `wpdb::get_row()` probe.
  This remains placeholder state only, not real MySQL connectivity, arbitrary
  SQL, persistent WordPress option APIs, object-cache persistence, or native DB
  lowering.

## Milestone 1385: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1381-1384 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1483` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1483`
  fixture tests passed with `0` failures, `860` system PHP comparisons, and
  `623` skipped `phpc-only` fixtures.

## Milestone 1386: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: add bounded `class_alias($class, $alias,
  $autoload = true)` support for declared class metadata, including source
  class autoload through the current SPL callback/default-file-probe path,
  alias lookup for `class_exists($alias, false)`, `new AliasName(...)`, and
  `is_a($object, "AliasName")`, duplicate-alias `false` results, focused Rust
  coverage, a PHP-comparison CLI fixture, documentation, and named unsupported
  gaps for interface/trait aliases, exact warning behavior, alias entries in
  `get_declared_classes()`, broader namespace/import canonicalization, and
  native lowering.

## Milestone 1387: Reference/COW WordPress Blocker

- [x] Runtime lane: close the alias-backed direct-call argument parent slice for
  reference-return child-slot binding. Direct by-reference function, visible
  instance-method, and named static method calls can now accept a direct
  argument variable that is already routed through covered array-offset alias
  metadata, such as `$payload =& $_REQUEST["payload"];`, and bind
  `return $param[$key];` back to the underlying request/array/property slot
  instead of requiring a normal caller cell.

## Milestone 1388: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, such as session/cookie
  persistence, output buffering beyond `ob_get_length()`, shutdown fidelity,
  output-started/header interaction, include-path/stat-cache behavior, stream
  context params/effects, filters, additional wrappers, request-body lifetime,
  upload edge cases beyond the explicit `PHPC_FILES` seed, or host filesystem
  semantics. Keep native lowering boundaries explicit when runtime support
  remains interpreter-only. Milestone 1388 adds bounded interpreter support
  for no-argument `ob_list_handlers()` over the existing output-buffer stack,
  returning PHP-compatible default handler names for active no-callback
  buffers, with dynamic-call visibility, explicit native rejection, a CLI
  fixture, and system PHP comparison. Custom callbacks, chunk sizes, flags,
  exact status metadata, `ob_get_status()`, `ob_get_flush()`, output-started
  header interaction, fatal cleanup, exact warning text, and native
  output-buffer lowering remain unsupported.

## Milestone 1389: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added exact direct and prepared
  `SELECT option_id, option_name, option_value, autoload FROM wp_options WHERE
  option_name = ...` reads without `LIMIT 1` over the deterministic MySQLi
  `wp_options` state island, with a `wpdb`-shaped fixture proving `get_row`,
  `get_results`, `get_col`, and missing-row behavior. This remains placeholder
  option-state evidence, not real MySQL, arbitrary SQL, object-cache
  persistence, or native database support.

## Milestone 1390: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1386-1389 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1487` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1487`
  fixture tests passed with `0` failures, `863` system PHP comparisons, and
  `624` skipped `phpc-only` fixtures.

## Milestone 1391: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded `class_alias()` support for declared
  interface metadata, including source-interface autoload through the current
  SPL callback/default-file-probe path, alias lookup for
  `interface_exists($alias, false)`, `instanceof AliasInterface`,
  `is_a($object, "AliasInterface")`, and
  `is_subclass_of("ClassName", "AliasInterface")`, plus classes declared
  after the alias that `implements AliasInterface`. Trait aliases, exact
  warning/error behavior, alias entries in declared interface lists,
  same-file runtime alias ordering, broader namespace/import canonicalization,
  and native lowering remain unsupported.

## Milestone 1392: Reference/COW WordPress Blocker

- [x] Runtime lane: added alias-backed literal
  `call_user_func_array()` reference arguments. Literal callback argument
  arrays such as `array(&$payload, ...)`, where `$payload` is already routed
  through covered direct array-offset alias metadata, now copy in and write
  back through the underlying request/global/array/property slot group for
  ordinary by-reference callbacks and covered reference-return callbacks.
  Real PHP reference containers, broader COW, alias-backed stored argument
  arrays, ArrayAccess roots, non-public/dynamic property callback roots,
  closure/builtin reference-return callbacks, exact alias destruction
  ordering, and native lowering remain unsupported.

## Milestone 1393: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: added bounded interpreter support for
  `ob_get_status($full_status = false)` over the existing output-buffer stack.
  The default path returns an empty array without an active buffer or the
  innermost default-handler status array with bounded PHP-compatible fields;
  `true` returns status arrays for the full active stack from outermost to
  innermost. Dynamic-call visibility, native rejection, a CLI fixture, and
  system PHP comparison are covered. Custom callbacks, custom chunk sizes,
  non-default flags, exact handler metadata beyond the bounded fields,
  `ob_get_flush()`, output-started/header interaction, fatal cleanup, exact
  warning text, and native output-buffer lowering remain unsupported.

  Larger request/SAPI, filesystem, and stream work remains in the compatibility
  ledger for later lanes: session/cookie persistence, shutdown fidelity,
  output-started/header interaction, include-path/stat-cache behavior, stream
  context params/effects, filters, additional wrappers, request-body lifetime,
  upload edge cases beyond the explicit `PHPC_FILES` seed, and host filesystem
  semantics.

## Milestone 1394: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added prepared autoload-only option-name
  equality reads over the deterministic MySQLi `wp_options` state island.
  `mysqli_stmt_execute()`/`mysqli_stmt_get_result()` and
  `mysqli_execute_query($handle, $query, array($name))` now accept exact
  `SELECT autoload FROM wp_options WHERE option_name = ?` shapes, with or
  without `LIMIT 1` and with the current backticked WordPress spelling, for
  `update_option()`-shaped autoload reevaluation probes. Real MySQL,
  arbitrary SQL/projection, persistent object-cache state, full option APIs,
  prepared parameter coercion, ordering/collation fidelity, and native DB
  lowering remain unsupported.

## Milestone 1395: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1391-1394 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch. Manual full gate passed before checkpoint: `cargo
  test` completed successfully, `phpc test` reported `1491` fixture tests
  passed with `0` failures, and `phpc test --compare-php` reported `1491`
  fixture tests passed with `0` failures, `866` system PHP comparisons, and
  `625` skipped `phpc-only` fixtures.
...
- [x] Parser/runtime lane: added bounded `class_parents()` metadata for
  current object values and declared string class names, enabling covered
  userland recursive trait-helper patterns that combine `class_parents()` with
  non-recursive direct `class_uses()`. The slice supports the current
  bool-like scalar autoload flag, dynamic string calls, immediate-parent to
  root ordering for declared user classes, focused Rust tests, and a
  system-PHP-comparable CLI fixture. Built-in/internal parent metadata, exact
  missing-class warning behavior, namespace/import alias expansion beyond
  parsed class-like names, Reflection integration, exact engine ordering for
  every metadata edge case, and native lowering remain unsupported.

## Milestone 1407: Reference/COW WordPress Blocker

- [x] Runtime lane: added the bounded stored-array non-public property bridge
  for `call_user_func_array()`. Direct visible named object-property stored
  argument arrays such as `$this->privateArgs` and
  `$peer->protectedArgs` can now carry reached integer-keyed by-reference
  slots into normal and reference-returning callbacks when those slots were
  assigned by reference through the covered alias-variable path. Kept real PHP
  reference containers, ArrayAccess reference roots, append-offset callback
  roots, direct reference assignment between object-property array offsets
  without an intermediate alias variable, alias lifetime cleanup across
  function/include boundaries, broader array/object copy-on-write,
  request/session alias behavior, and native lowering unsupported.

## Milestone 1408: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: add bounded `register_shutdown_function()` execution for
  request/SAPI shutdown fidelity. `phpc run` now stores supported string
  user/builtin callbacks and public object/static array callables with
  already-evaluated by-value extra arguments, drains them in registration
  order during normal shutdown and after the bounded `exit()` path, runs
  callbacks registered during shutdown later in the same pass, and performs
  this callback drain before public object destructors and final output-buffer
  flushing. Closure callbacks remain registration-only because closure values
  are still inert; by-reference shutdown callback arguments,
  invokable-object callbacks, private/protected method callbacks, exact
  fatal/finally/destructor edge ordering, and native lowering remain
  unsupported.

## Milestone 1409: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: continued attacking executable WordPress
  database/bootstrap evidence from `GOAL.MD`. Closed with bounded direct,
  one-shot prepared, and statement prepared `wp_options` transient-prefix
  result scans that accept exact trailing `ORDER BY option_name` or
  backticked `ORDER BY` suffixes with optional `ASC`, including a committed
  `wpdb`-shaped Milestone 1409 probe. Remaining gaps include real MySQL,
  arbitrary SQL ordering, `DESC`, collation fidelity, full WordPress option
  APIs, persistent object cache, and native DB lowering.

## Milestone 1410: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1406-1409 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. Focused integrated checks passed for `class_parents()`, stored
  non-public object-property `call_user_func_array()` argument arrays,
  shutdown callback execution, ordered transient-prefix MySQLi option scans,
  and milestone fixtures 1406-1409. The serialized full gate passed with
...
## Milestone 1411: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded `ReflectionClass` metadata objects
  for declared user classes, interfaces, and traits. The executable slice
  covers object/string construction with existing autoload probing,
  `getName()`, `getShortName()`, `isInterface()`, `isTrait()`,
  `isInstantiable()`, `getParentClass()`, `getInterfaceNames()`, and
  `hasMethod($name)` over the current metadata tables, plus Rust and CLI
  fixture coverage. `ReflectionMethod`, `ReflectionProperty`,
...
## Milestone 1412: Reference/COW WordPress Blocker

- [x] Runtime lane: closed a bounded string-keyed callback reference-argument
  slice for `call_user_func_array()`. Current user-function callbacks and
  public object/static array-callable methods accept literal and covered stored
  argument arrays whose string keys match declared parameter names, including
  direct variable, request-bag alias, and reference-return alias-binding paths.
  Remaining gaps include full reference containers, ArrayAccess and
  append-offset roots, unknown or duplicate named callback arguments,
  positional arguments after named callback arguments, variadic named callback
  arguments, dynamic literal named keys, broader copy-on-write, request/session
  alias lifetime fidelity, and native lowering.

## Milestone 1413: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded `file_get_contents()` offset/length reads for
  local UTF-8 files, include-path local reads, and deterministic `php://input`
  request-body seeds. The slice supports integer offsets, including negative
  offsets from the end, and optional non-negative integer max lengths while
  keeping native lowering rejected through the existing filesystem-read
  boundary. It does not add binary byte fidelity, wrapper-specific context
  effects, warning-plus-`false` recovery for negative offsets before the
  start, non-UTF-8 paths, stat-cache or `open_basedir` behavior, real SAPI
  body streams, or native filesystem lowering.

## Milestone 1414: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: add bounded executable WordPress transient
  timeout scan evidence over the deterministic MySQLi `wp_options` state
  island. Milestone 1414 supports exact direct and prepared option-name
  projections with
  `option_name LIKE '<prefix>%' AND option_value < <decimal timestamp>`, plus
  the existing exact `ORDER BY option_name` suffix, through
  `mysqli_query()`, `mysqli_execute_query()`, and
  `mysqli_prepare()`/`mysqli_stmt_execute()`/`mysqli_stmt_get_result()`. This
  proves a `wpdb::get_col()`-shaped expired-transient-timeout probe while
  leaving general SQL comparisons, `LIMIT`, transient cleanup joins,
  delete-by-join behavior, persistent object cache, full WordPress option
  APIs, and native database lowering unsupported.

## Milestone 1415: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1411-1414 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. Focused integrated checks passed for `ReflectionClass`, string-keyed
  `call_user_func_array()` reference arguments, `file_get_contents()`
  offset/length reads, expired transient-timeout MySQLi option scans, and
  milestone fixtures 1411-1414. The serialized full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1411-1415 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully after refreshing `ReflectionClass` class-list snapshots,
  `phpc test` reported `1507` fixture tests passed with `0` failures, and
  `phpc test --compare-php` reported `1507` fixture tests passed with `0`
  failures, `878` system PHP comparisons, and `629` `phpc-only` skipped
  fixtures.

## Milestone 1416: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded `ReflectionMethod` metadata objects
  for declared user classes, interfaces, and traits. The executable slice
  covers object/string construction with existing class-like autoload probing,
  inherited class method resolution, declaring-class reflection objects,
  modifier masks, `ReflectionMethod::IS_*` constants, and public/protected/
  private/static/final/abstract/constructor predicates, with Rust and CLI
  fixture coverage against system PHP. `ReflectionProperty`,
  ...
  reflection invocation, exact `ReflectionException` behavior,
  namespace/import alias expansion beyond parsed class-like names, and native
  lowering remain unsupported.

## Milestone 1417: Reference/COW WordPress Blocker

- [x] Runtime lane: add bounded `call_user_func_array()` string-keyed
  by-value argument mapping for current user functions and public object/static
  array callables, plus stable duplicate and unknown named callback argument
  diagnostics. This keeps native lowering rejected and leaves full reference
  containers, ArrayAccess/append-offset callback roots, alias lifetime cleanup,
  broader copy-on-write, request/session alias behavior, variadic named
  arguments, positional-after-named execution, closure/builtin named callback
  dispatch, and full PHP named-argument semantics for later milestones.

## Milestone 1418: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded warning-plus-`false` recovery for
  `file_get_contents()` missing local file reads and negative offsets before
  the start of the current local/`php://input` UTF-8 payloads. The interpreter
  now carries recoverable warning text through `Execution.stderr`, returns
  `false`, and continues execution for those two reached filesystem/stream
  cases. The `milestone1418` fixture proves the CLI path. This does not add
  exact PHP warning text, general warning/error-handler integration,
  binary-safe byte offsets, unsupported wrapper recovery, stream-resource
  warning recovery, `open_basedir`, stat-cache behavior, or native lowering.

## Milestone 1419: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added a bounded transient-timeout cleanup
  delete shape over the deterministic MySQLi `wp_options` state island.
  Direct `mysqli_query()`, one-shot `mysqli_execute_query()`, and prepared
  statement execution now accept exact
  `DELETE FROM wp_options WHERE option_name LIKE ... AND option_value < ...`
  forms, including current backticked spellings, escaped transient prefixes,
  and integer or decimal-integer-string thresholds. This removes only matching
  recorded timeout rows and keeps real MySQL, general SQL comparison,
  transient payload joins/pair deletion, persistent object cache, full
  WordPress option APIs, and native DB lowering unsupported.

## Milestone 1420: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1416-1419 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. Focused integrated checks passed for `ReflectionMethod`, string-keyed
  by-value `call_user_func_array()` mapping, `file_get_contents()`
  warning-plus-`false` recovery, expired transient-timeout MySQLi cleanup
  deletes, and milestone fixtures 1416-1419. The serialized full gate passed
  with `CARGO_TARGET_DIR=/tmp/phpc-target-full-1416-1420
  CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test`
  completed successfully after refreshing `ReflectionMethod` class-list
  snapshots, `phpc test` reported `1511` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1511` fixture tests
  passed with `0` failures, `880` system PHP comparisons, and `631`
  `phpc-only` skipped fixtures.

## Milestone 1421: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded `ReflectionParameter` method
  parameter metadata slice on top of the existing `ReflectionMethod` model.
  `ReflectionMethod::getParameters()`, `getNumberOfParameters()`, and
  `getNumberOfRequiredParameters()` now expose parsed method parameters, and
  `new ReflectionParameter([$object_or_class, $method], $parameter)` accepts
  the current array-callable method shape with integer positions or string
  parameter names. Parameter objects support name, position, declaring
  class/function, optional/default availability and values, by-reference,
  variadic, and type-presence predicates. Remaining gaps include
  `ReflectionProperty`, attributes, file/line/doc-comment metadata,
  `ReflectionType`/`ReflectionNamedType`, `allowsNull()`, parameter
  attributes, default constant-name introspection, function/closure parameter
  targets, extension/internal reflection metadata, exact `ReflectionException`
  behavior, namespace/import alias expansion beyond parsed class-like names,
  and native lowering.

## Milestone 1422: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded append-offset stored callback argument
  root for `call_user_func_array()`. Stored argument-array slots can now be
  assigned by reference from covered direct array append and visible
  object-property array append sources, such as `$args[] =& $items[]` and
  `$args["value"] =& $object->items[]`; callback writeback and supported
  reference-return binding sync the appended source slot and stored argument
  slot through the existing alias metadata. This remains a bounded alias
  group, not real reference containers, ArrayAccess reference roots, dynamic
  append/property roots, full alias lifetime cleanup, broader COW, or native
  lowering.

## Milestone 1423: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded `file_get_contents()` recoverable-warning
  routing through request-local `set_error_handler()` registrations. Missing
  local file reads and negative offsets before the current local or
  `php://input` payload now deliver an `E_WARNING` to supported string
  user-function handlers and public object/static array callable handlers when
  the handler mask includes `E_WARNING`; non-`false` handler returns suppress
  the stderr fallback, while `false` falls through to stderr only when
  `error_reporting()` includes `E_WARNING`. The fixture proves string-handler,
  public object array callable handler, passthrough, restored, and suppressed
  warning paths. Closure handler
  invocation, notices/deprecations/fatal routing, exact PHP warning and handler
  `errstr` text, the full PHP handler stack, broader stream/resource warning
  recovery, binary string fidelity, and native lowering remain unsupported.

## Milestone 1424: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added bounded executable transient payload
  pair deletion evidence over the deterministic MySQLi `wp_options` state
  island. Direct `mysqli_query()`, one-shot `mysqli_execute_query()`, and
  prepared statement execution now accept one exact WordPress-shaped
  `DELETE a, b FROM wp_options a, wp_options b ...` cleanup query where alias
  `a` is a payload row, alias `b` is the matching timeout row formed by the
  supported `CONCAT`/`SUBSTRING` expression, and the timeout value is below a
  direct or prepared decimal threshold. This deletes both rows, updates
  affected-row metadata, and proves a committed `wpdb`-shaped Milestone 1424
  probe. Remaining gaps include real MySQL, arbitrary multi-table deletes,
  subqueries, tables outside the `wp_options` state island, collation/index/
  lock behavior, persistent object cache, full WordPress option APIs, exact
  MySQL affected-row edge cases, and native DB lowering.

## Milestone 1425: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1421-1424 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. Focused integrated checks passed for `ReflectionParameter`,
  append-offset stored `call_user_func_array()` reference roots,
  `file_get_contents()` error-handler warning routing, transient
  payload/timeout pair deletes, and milestone fixtures 1421-1424. The
  serialized full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1421-1425 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully after refreshing `ReflectionParameter` class-list snapshots,
  `phpc test` reported `1515` fixture tests passed with `0` failures, and
  `phpc test --compare-php` reported `1515` fixture tests passed with `0`
  failures, `882` system PHP comparisons, and `633` `phpc-only` skipped
  fixtures.

## Milestone 1426: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded `ReflectionParameter` type metadata
  slice for method parameters. `ReflectionParameter::getType()` now returns
  `null` for untyped method parameters or a request-local
  `ReflectionNamedType` for simple named parameter types; `ReflectionParameter`
  and `ReflectionNamedType` report `allowsNull()`, and `ReflectionNamedType`
  reports `getName()` plus `isBuiltin()` over the current parsed type string.
  Remaining gaps include `ReflectionProperty`, attributes, file/line/doc-comment
  metadata, extension/internal metadata, function/closure parameter targets,
  parameter attributes, `ReflectionUnionType`/`ReflectionIntersectionType`,
  default constant-name introspection, exact `ReflectionException` behavior,
  namespace/import alias expansion beyond parsed class-like names, and native
  lowering.

## Milestone 1427: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded direct `ArrayAccess` reference-root bridge.
  Direct sources such as `$alias =& $bag[$key]` and literal
  `call_user_func_array()` reference elements such as `array(&$bag[$key])`
  now bind to the backing property array slot when the direct object variable
  implements `ArrayAccess`, public `offsetGet($offset)` returns by reference,
  and the body is exactly the supported `return $this->property[$offset];`
  shape. Remaining gaps include real reference containers, by-value
  `offsetGet()` indirect-modification fidelity, broader `offsetGet()` bodies,
  property-held or dynamic `ArrayAccess` roots, append/nested/stored-array
  `ArrayAccess` roots, broader COW, and native lowering.

## Milestone 1428: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded LIFO `set_error_handler()` stack behavior
  for the existing `file_get_contents()` recoverable `E_WARNING` path.
  Registering a new handler now returns the previous handler value while
  keeping it on a request-local stack, and `restore_error_handler()` pops the
  current registration so the prior handler receives later missing-file
  warnings. The `milestone1428` fixture proves top-handler dispatch, previous
  handler return value, restore-to-previous behavior, warning suppression, and
  CLI coverage. This remains limited to recoverable `file_get_contents()`
  warnings and current string/public array-callable handlers; closure handler
  invocation, warnings/notices/deprecations outside this path, active-handler
  stack mutation edge cases, exact PHP warning text, broad binary stream
  fidelity, and native lowering remain unsupported.

## Milestone 1429: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added bounded prepared-statement insert-ID
  metadata over the deterministic MySQLi `wp_options` state island.
  Placeholder `mysqli_stmt` state now tracks a statement-local `insert_id`,
  resets it for non-insert executions and statement reconfiguration, and
  exposes deterministic option IDs through `mysqli_stmt_insert_id()` after
  exact prepared insert, insert-on-duplicate, and replace branches. Remaining
  gaps include real MySQL connectivity, arbitrary prepared SQL,
  schema/index/collation behavior, persistent object cache, full WordPress
  option/transient APIs, exact mysqlnd duplicate-key insert-ID semantics, host
  database state, and native DB lowering.

## Milestone 1430: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1426-1429 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. Focused integrated checks passed for `ReflectionNamedType` parameter
  metadata, direct `ArrayAccess` reference roots, LIFO error-handler stack
  warning routing, prepared statement insert-ID metadata, and milestone
  fixtures 1426-1429. The serialized full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1426-1430 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully after refreshing `ReflectionType`/`ReflectionNamedType`
  class-list snapshots and the older Milestone 1423 stacked-handler snapshot,
  `phpc test` reported `1519` fixture tests passed with `0` failures, and
  `phpc test --compare-php` reported `1519` fixture tests passed with `0`
  failures, `885` system PHP comparisons, and `634` `phpc-only` skipped
  fixtures.

## Milestone 1431: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added a bounded `ReflectionProperty` metadata slice
  for declared user classes. `ReflectionClass` now supports
  `hasProperty($name)`, `getProperty($name)`, and zero-argument
  `getProperties()` over current class metadata, and `ReflectionProperty`
  exposes name, declaring class, visibility/static modifier masks,
  default-value metadata, and untyped `hasType()`/`getType()` results. This
  keeps attributes, typed properties, file/line/doc-comment metadata,
  extension/internal metadata, method/property invocation through reflection,
  exact `ReflectionException` objects, filter masks for `getProperties()`, and
  native lowering outside the supported slice.

## Milestone 1432: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded nested direct `ArrayAccess` reference-root
  bridge on top of the existing direct-object `offsetGet()` bridge. Direct
  sources such as `$alias =& $bag["outer"]["slot"]` and literal
  `call_user_func_array()` reference elements such as
  `array(&$bag["outer"]["slot"])` now bind to the backing property array child
  slot when the direct object variable implements `ArrayAccess`, public
  `offsetGet($offset)` returns by reference, and the body is exactly the
  supported `return $this->property[$offset];` shape. Remaining gaps include
  real reference containers, by-value `offsetGet()` indirect-modification
  fidelity, broader `offsetGet()` bodies, property-held or dynamic
  `ArrayAccess` roots, append/stored-array `ArrayAccess` roots, mixed nested
  `ArrayAccess` chains, broader COW, and native lowering.

## Milestone 1433: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded post-output response-header warnings outside
  the `file_get_contents()` recovery path. After unbuffered output starts,
  `header()`, `header_remove()`, and `setcookie()` now leave request-local
  header state unchanged while emitting recoverable `E_WARNING` events through
  the current `set_error_handler()` stack or stderr fallback; `setcookie()`
  still returns `false` in that state. Remaining gaps include exact PHP
  warning text, broader warning/notice/deprecation surfaces, active
  error-handler stack mutation edge cases, binary string reads,
  shutdown/fatal/destructor ordering, session persistence warnings,
  cookie-attribute formatting, upload state, include-path/stat-cache behavior,
  `open_basedir`, stream filters/wrappers, wrapper-specific stream contexts,
  request-body lifetime, host filesystem edge cases, and native lowering.

## Milestone 1434: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added a bounded executable WordPress
  schema/bootstrap evidence slice for deterministic `wp_options` table probes.
  `mysqli_query()` and the shared pending-result path now return fixed rows for
  exact `SHOW TABLES LIKE 'wp_options'`, `DESCRIBE`/`DESC wp_options`, and
  `SHOW [FULL] COLUMNS FROM wp_options`, including option table field names,
  types, primary/unique key markers, `autoload` default, and placeholder
  utf8mb4 collation metadata. The milestone1434 fixture exercises a small
  `wpdb`-shaped schema probe through `phpc run`. This remains a deterministic
  state-island probe only: no real MySQL connectivity, mutable schema,
  CREATE/ALTER TABLE execution, dbDelta diffing, host index/collation
  inspection, arbitrary SQL, or native DB lowering.

## Milestone 1435: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1431-1434 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. The first full-gate run exposed one older `milestone832` expectation
  that still treated `SHOW FULL COLUMNS FROM wp_options` as an empty
  placeholder; the fixture and `phpc-only` reason were refreshed to the new
  bounded schema rows. The serialized full gate then passed with
...
## Milestone 1436: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded simple named typed property metadata
  for declared class properties with explicit constant-expression defaults.
  The parser stores property `TypeDecl` text, runtime class/property metadata
  preserves it, inherited non-private property redeclarations must keep the
  same type metadata, and `ReflectionProperty::hasType()`/`getType()` now
  expose a request-local `ReflectionNamedType` for those properties. The
  `milestone1436` fixture proves direct/object-target/property-list reflection
  against system PHP. This remains metadata-only: typed property read/write
  enforcement, uninitialized typed property slots, compound
  union/intersection/DNF property type objects, attributes, file/line/doc
  comments, extension/internal metadata, exact type/default validation, and
  native lowering remain unsupported.

## Milestone 1437: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded property-held `ArrayAccess`
  reference-root bridge on top of the direct and nested Milestone 1427/1432
  bridge. Direct visible named object-property roots such as
  `$alias =& $holder->bag["outer"]["slot"]` and literal
  `call_user_func_array()` elements such as
  `array(&$holder->bag["outer"]["slot"])` now bind to the held object's
  backing property array slot when the property value implements
  `ArrayAccess`, public `offsetGet($offset)` returns by reference, and its
  body is exactly `return $this->property[$offset];`. The slice keeps real
  reference containers, dynamic property-held roots, append/stored-array
  `ArrayAccess` roots, mixed nested `ArrayAccess` chains, alias cleanup after
  replacing the containing property, broader COW, and native lowering outside
  the supported surface.

## Milestone 1438: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing broader
  warnings/notices/deprecations, active error-handler stack mutation edge
  cases, binary string reads, shutdown/fatal/destructor edge ordering,
  upload/session/cookie persistence, include-path/stat-cache behavior,
  `open_basedir`, stream filters/wrappers, wrapper-specific stream contexts,
  request-body lifetime, host filesystem edge cases, or native lowering.
  Milestone 1438 adds late-output `session_start()` warning fidelity over the
  bounded CLI request/session model: after unbuffered output starts,
  `session_start()` now returns `false`, leaves session state unchanged, and
  routes a bounded `E_WARNING` through the current `set_error_handler()` stack
  or stderr fallback. Session persistence, locking, save handlers,
  cookie/cache-header emission, exact warning text, active-session restart
  notices, broader warning classes, and native session lowering remain
  unsupported.

## Milestone 1439: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress
  database/bootstrap evidence, prioritizing broader `wpdb`/MySQLi result or
  mutation behavior, arbitrary prepared option/transient query shapes,
  object-cache/transient persistence, option API state, hooks under realistic
  callback shapes, deterministic plugin/theme loading probes,
  schema/install/update flows, or a bootstrap/request probe that moves past
  its next real blocker. Closed with deterministic `SHOW INDEX`/`SHOW KEYS`
  rows for `wp_options` over the MySQLi state island, covering the primary
  `option_id` index and unique `option_name` index for bounded dbDelta-style
  inspection probes without claiming real MySQL or mutable schema support.

## Milestone 1440: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1436-1439 landed, refreshed the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; recorded focused verification; and ran the serialized full
  gate. The serialized full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1436-1440 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1527` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1527` fixture tests
  passed with `0` failures, `891` system PHP comparisons, and `636`
  `phpc-only` skipped fixtures.

## Milestone 1441: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next large object/reflection/class
  metadata blocker, prioritizing typed property read/write enforcement,
  uninitialized typed property slots, property attributes, file/line/doc
  comment metadata, extension/internal metadata, function/closure reflection
  targets, `ReflectionUnionType` or `ReflectionIntersectionType`, magic hooks
  reached by WordPress, autoload/class-alias lifecycle parity, interface
  diagnostics, or broader class metadata behavior.
  Milestone 1441 adds no-default simple named typed property slots for
  instance and static properties, stable uninitialized read errors,
  `isset()`/`empty()` behavior, bounded direct write enforcement, reflection
  default metadata, and object-var filtering. Weak scalar coercions,
  inherited class-name assignment checks, compound types, readonly/property
  hooks, `unset($object->prop)` uninitialization, complex reference/COW write
  paths, and native lowering remain unsupported.

## Milestone 1442: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next large reference/COW blocker, prioritizing
  real reference containers, dynamic property-held `ArrayAccess` roots,
  append/stored-array `ArrayAccess` roots, mixed nested `ArrayAccess` chains,
  alias cleanup after replacing containing objects/properties, alias lifetime
  cleanup across function/include boundaries, broader array/object
  copy-on-write, request/session alias behavior, or variadic and named
  callback-reference semantics that block WordPress.
  Milestone 1442 lets direct stored callback argument arrays preserve
  references sourced from direct and property-held `ArrayAccess` roots through
  the existing bounded by-reference `offsetGet()` bridge. Real reference
  containers, reference array literals stored by value, dynamic roots,
  append-offset `ArrayAccess` source roots, arbitrary `offsetGet()` bodies,
  broader COW, and native lowering remain unsupported.

## Milestone 1443: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing session
  persistence, locking or save-handler behavior, upload persistence,
  cookie/cache-header emission, broader warnings/notices/deprecations, active
  error-handler stack mutation edge cases, binary string reads,
  shutdown/fatal/destructor edge ordering, include-path/stat-cache behavior,
  `open_basedir`, stream filters/wrappers, wrapper-specific stream contexts,
  request-body lifetime, host filesystem edge cases, or native lowering.
  Milestone 1443 adds bounded `session_start(["read_and_close" => true])`
  behavior over the existing in-memory CLI session state: the option uses PHP
  truthiness, materializes `$_SESSION`, returns `true`, and immediately closes
  status back to `PHP_SESSION_NONE` while keeping session data visible. Other
  session options remain accepted-but-ignored array entries; persistence,
  locking, save handlers, cookie/cache-header emission, exact warnings,
  active-session restart notices, and native session lowering remain
  unsupported.

## Milestone 1444: WordPress DB/Bootstrap Evidence Blocker
...
- [x] Compatibility/runtime lane: target executable WordPress
  ...
  object-cache/transient persistence, option API state, hooks under realistic
  callback shapes, deterministic plugin/theme loading probes,
  schema/install/update flows, dbDelta diff probes beyond `wp_options`, or a
  bootstrap/request probe that moves past its next real blocker.
  Milestone 1444 adds a deterministic MySQLi schema-state island for bounded
  `CREATE TABLE` and `ALTER TABLE ... ADD ...` statements plus later table,
  column, and index probes. Real SQL parsing, dropped/renamed/changed
  columns, multi-column indexes/sub-parts, dbDelta diff generation, real DDL,
  schema rollback, host DB inspection, exact warning/error fidelity, and
  native DB lowering remain unsupported.

## Milestone 1445: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1441-1444 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1441-1445 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1529` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1529` fixture tests
  passed with `0` failures, `894` system PHP comparisons, and `635`
  `phpc-only` skipped fixtures.

## Milestone 1446: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next large object/reflection/class
  metadata blocker, prioritizing weak scalar coercions for typed properties,
  inherited class-name assignment checks, property attributes, file/line/doc
  comment metadata, extension/internal metadata, function/closure reflection
  targets, compound property type objects, readonly/property-hook behavior,
  `unset($object->prop)` uninitialization, magic hooks reached by WordPress,
  autoload/class-alias lifecycle parity, interface diagnostics, or broader
  class metadata behavior.
  Milestone 1446 adds visible declared instance typed-property
  uninitialization through `unset($object->prop)`, including read errors,
  `isset()`/`empty()` behavior, `get_object_vars()` filtering,
  reinitialization, and the bounded missing-property `__unset` path. Weak
  scalar coercions, inherited class-name assignment checks, readonly
  properties, property hooks, static typed-property unset, exact untyped
  declared-property unset warnings, complex reference/COW property paths, and
  native lowering remain unsupported.

## Milestone 1447: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next large reference/COW blocker, prioritizing
  real reference containers, reference array literals stored by value,
  dynamic `ArrayAccess` roots, append-offset `ArrayAccess` source roots,
  arbitrary `offsetGet()` bodies, mixed nested `ArrayAccess` chains, alias
  cleanup after replacing containing objects/properties, alias lifetime
  cleanup across function/include boundaries, broader array/object
  copy-on-write, request/session alias behavior, or variadic and named
  callback-reference semantics that block WordPress.
  Milestone 1447 adds bounded append-offset `ArrayAccess` reference sources
  for direct and property-held roots such as `$alias =& $bag[]` and stored
  `call_user_func_array()` argument arrays, using the existing exact
  by-reference `offsetGet()` bridge. Arbitrary `offsetGet()` append logic,
  dynamic roots, mixed nested chains, alias cleanup after replacement, real
  reference containers, broader COW, and native lowering remain unsupported.

## Milestone 1448: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing session
  persistence, locking or save-handler behavior, cookie/cache-header
  emission, broader warnings/notices/deprecations, active-session restart
  notices, garbage collection, strict session-id validation, binary string
  reads, shutdown/fatal/destructor edge ordering, include-path/stat-cache
  behavior, `open_basedir`, stream filters/wrappers, wrapper-specific stream
  contexts, request-body lifetime, host filesystem edge cases, or native
  lowering.
  Milestone 1448 adds active-session `session_start()` restart notices:
  restart attempts emit bounded `E_NOTICE`, return `true`, preserve
  `$_SESSION`, and keep status active even with `read_and_close`. Session
  persistence, locking, save handlers, cookie/cache-header emission, garbage
  collection, strict ids, broader option effects, exact notice text, and
  native lowering remain unsupported.

## Milestone 1449: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress
  database/bootstrap evidence, prioritizing real SQL parsing slices,
  dropped/renamed/changed columns, multi-column indexes or sub-parts, dbDelta
  diff generation, schema rollback, host database inspection, broader
  ...
  realistic callback shapes, deterministic plugin/theme loading probes, or a
  bootstrap/request probe that moves past its next real blocker.
  Milestone 1449 records ordered multi-column index parts and numeric prefix
  sub-parts in the deterministic MySQLi schema-state island, and emits one
  `SHOW INDEX` row per part with `Seq_in_index` and `Sub_part`. Real SQL
  parsing, dropped/renamed/changed columns, expression indexes, ordering/
  opclass metadata, dbDelta diff generation, schema rollback, host DB
  inspection, real MySQL execution, and native DB lowering remain
  unsupported.

## Milestone 1450: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1446-1449 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1446-1450 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1533` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1533` fixture tests
  passed with `0` failures, `897` system PHP comparisons, and `636`
  `phpc-only` skipped fixtures.

## Milestone 1451: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next large object/reflection/class
  metadata blocker. Milestone 1451 adds bounded weak scalar coercions for
  declared typed-property writes to `int`, `float`, `bool`, and `string`
  properties, including instance and static property writes through the
  `phpc run` path with system PHP comparison coverage. Exact deprecation text,
  `strict_types`, inherited class-name assignment checks, union/intersection/
  DNF property writes, readonly properties, property hooks, reference/COW
  property paths, and native lowering remain unsupported.

## Milestone 1452: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next large reference/COW blocker. Milestone
  1452 adds bounded reference array literals stored by value for direct
  variable assignment targets. Covered forms such as
  `$args = array(&$value)`, `$args = array(&$items["slot"])`, and
  `$args = array("value" => &$object->items[$key])` preserve the selected
  alias metadata so later stored-array `call_user_func_array()` invocation,
  reference-return alias binding, and copied-array writes observe the same
  slot. Reference literals assigned into non-variable or alias-backed variable
  targets, arbitrary reference expressions, dynamic `ArrayAccess` roots, mixed nested
  `ArrayAccess` chains, real reference containers, broader array/object COW,
  exact alias destruction ordering, and native lowering remain unsupported.

## Milestone 1453: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests. Milestone 1453 adds
  request-local session snapshots: `session_write_close()` persists the active
  bounded `$_SESSION` array into an in-memory session-id map, later
  `session_start()` reloads the last closed snapshot, and
  `session_start(["read_and_close" => true])` reloads then closes while
  leaving visible `$_SESSION` available. Cross-process session files, locking,
  save handlers, cookie/cache headers, garbage collection, strict id
  validation, broad option effects, exact warning text, restart-safe root
  reference aliases, and native lowering remain unsupported.

## Milestone 1454: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress
  database/bootstrap evidence, prioritizing real SQL parsing slices,
  dropped/renamed/changed columns, expression indexes, index ordering/opclass
  metadata, dbDelta diff generation, schema rollback, host database
  inspection, broader `wpdb`/MySQLi result or mutation behavior, arbitrary
  ...
  Milestone 1454 adds bounded dynamic-schema `ALTER TABLE` column/index
  mutation behavior for `CHANGE COLUMN`, `MODIFY COLUMN`, `DROP COLUMN`,
  `DROP KEY`/`DROP INDEX`, and `DROP PRIMARY KEY`, with `DESCRIBE` and
  `SHOW INDEX` reflecting the recorded changed/dropped schema state.

## Milestone 1455: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1451-1454 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
...
## Milestone 1456: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker.
  Milestone 1456 adds bounded inherited class-name assignment checks for
  declared typed-property writes: objects whose runtime class extends the
  declared property type now satisfy instance and static typed properties,
  while unrelated objects still fail through the existing stable typed-property
  diagnostic. Interface-name compatibility, class aliases in typed-property
  compatibility checks, compound property type objects, readonly/property-hook
  behavior, property attributes, file/line/doc-comment metadata,
  extension/internal metadata, function/closure reflection targets, magic hooks
  reached by WordPress, autoload/class-alias lifecycle parity, interface
  diagnostics, broader reference/COW property writes, and native lowering
  remain unsupported.

## Milestone 1457: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next reference/COW blocker. Milestone 1457
  extends bounded reference array literals stored by value to direct variable
  assignment targets that are already backed by covered array-offset alias
  metadata, such as `$args =& $registry["args"]; $args = array(&$value);`.
  Later stored-array `call_user_func_array()` invocation, reference-return
  alias binding through the alias variable, copied stored arrays, and the
  underlying registry/request-style slot observe the same covered direct
  variable or direct array-offset source. Non-variable assignment targets,
  arbitrary reference expressions, dynamic `ArrayAccess` roots, nested mixed
  ...
  by-reference foreach gaps, and native lowering remain unsupported.

## Milestone 1458: Request/SAPI/Filesystem/Stream Blocker
...
- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  ...
  `open_basedir`, stream filters/wrappers, wrapper-specific stream contexts,
  request-body lifetime, host filesystem edge cases, or native lowering.
  Milestone 1458 adds bounded session cookie-header emission for fresh
  successful `session_start()` calls in the deterministic CLI header log and
  recognizes `session_start(["use_cookies" => false])` as a falsey option
  effect that suppresses that header. Full cookie attributes and encoding,
  cache limiter headers, trans-sid behavior, cookie replacement semantics,
  cross-process session files, locking, save handlers, strict id validation,
  garbage collection, exact warning text, and native session lowering remain
  unsupported.

## Milestone 1459: WordPress DB/Bootstrap Evidence Blocker
...
- [x] Compatibility/runtime lane: target executable WordPress database or
  ...
  metadata, host database inspection, broader `wpdb`/MySQLi result or
  mutation behavior, arbitrary prepared option/transient query shapes,
  object-cache/transient persistence, hooks under realistic callback shapes,
  deterministic plugin/theme loading probes, or a bootstrap/request probe that
  moves past its next real blocker. Milestone 1459 adds deterministic
  `SHOW CREATE TABLE` support over the per-handle MySQLi dynamic schema island
  with recorded columns, primary keys, non-unique keys, multi-column indexes,
  prefix sub-parts, and table collation.

## Milestone 1460: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1456-1459 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
...
## Milestone 1461: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker. Milestone
  1461 adds bounded interface-name typed-property compatibility for declared
  user interfaces: newly instantiated objects now carry implemented interface
  metadata from their class and parent classes, so direct writes to declared
  instance and static typed properties accept objects implementing the declared
  user-interface type while unrelated objects keep the stable typed-property
  diagnostic. Interface aliases, broader built-in/internal interface catalog
  behavior, union/intersection/DNF property types, class aliases in typed-property
  checks, readonly/property-hook behavior, property attributes,
  file/line/doc-comment metadata, extension/internal metadata,
  function/closure reflection targets, magic hooks reached by WordPress,
  autoload/class-alias lifecycle parity beyond existing metadata lookup,
  broader reference/COW property writes, and native lowering remain
  unsupported.

## Milestone 1462: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next reference/COW blocker, prioritizing real
  reference containers, non-variable reference-literal assignment targets,
  arbitrary reference expressions, dynamic `ArrayAccess` roots, nested mixed
  `ArrayAccess` chains, alias cleanup after replacing containing properties,
  broader array/object copy-on-write, exact alias destruction ordering,
  by-reference foreach, or native lowering. Completed as a bounded reference
  array literal assignment slice for direct visible object-property targets:
  `$store->args = array(&$value, ...)` now records covered literal reference
  slots below the property array so later stored-array
  `call_user_func_array()` invocation and reference-return alias binding
  observe the same covered source. Real reference containers, arbitrary
  reference expressions, array-offset/dynamic-property assignment targets,
  dynamic `ArrayAccess` roots, nested mixed `ArrayAccess` chains, broader COW,
  exact alias destruction ordering, broader by-reference foreach, and native
  lowering remain unsupported.

## Milestone 1463: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing session file
  persistence, locking or save-handler behavior, full cookie attributes,
  cache-header emission, garbage collection, strict session-id validation,
  broader session option effects, exact warning/notice/deprecation text,
  binary string reads, shutdown/fatal/destructor edge ordering,
  include-path/stat-cache behavior, `open_basedir`, stream filters/wrappers,
  wrapper-specific stream contexts, request-body lifetime, host filesystem
  edge cases, or native lowering. Milestone 1463 adds bounded deterministic
  session cookie attributes for fresh successful `session_start()` calls from
  `cookie_lifetime`, `cookie_path`, `cookie_domain`, `cookie_secure`,
  `cookie_httponly`, and `cookie_samesite` options. Cookie encoding,
  expiration-date formatting, cookie replacement semantics, cache-header
  emission, cross-process session files, locking, save handlers, strict id
  validation, garbage collection, exact warning text, and native session
  lowering remain unsupported.

## Milestone 1464: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing real SQL parsing slices, dbDelta diff
  generation, schema rollback, expression indexes, index ordering/opclass
  metadata, host database inspection, broader `wpdb`/MySQLi result or mutation
  behavior, arbitrary prepared option/transient query shapes, object-cache/
  ...
  its next real blocker. Milestone 1464 adds exact `SHOW TABLE STATUS LIKE
  '<table>'` and `SHOW TABLE STATUS WHERE Name = '<table>'` probes over the
  per-handle MySQLi dynamic schema island, returning deterministic
  MySQL-shaped table metadata with recorded collation and placeholder
  engine/storage fields.

## Milestone 1465: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1461-1464 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  ...
  passed with `0` failures, `904` system PHP comparisons, and `641`
  `phpc-only` skipped fixtures.

## Milestone 1466: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded class/interface alias compatibility
  for declared typed-property checks. Runtime object relationship metadata now
  records active `class_alias()` lookup names for instantiated classes,
  ancestor classes, and declared implemented interfaces, so direct instance
  and static typed-property writes accept current alias names in the simple
  class/interface property-type subset. This does not add aliases registered
  after an object was instantiated, compound property type objects,
  ...
  broader autoload/class-alias lifecycle parity, or native lowering.

## Milestone 1467: Reference/COW WordPress Blocker

- [x] Runtime lane: target the next reference/COW blocker, prioritizing real
  reference containers, array-offset or dynamic-property reference-literal
  ...
  destruction ordering, by-reference foreach, or native lowering. Completed as
  a bounded direct array-offset reference-literal assignment target slice:
  `$registry["args"] = array(&$value, ...)` now records covered literal
  reference slots below the stored array-offset container, so later
  `call_user_func_array($callback, $registry["args"])` invocation and
  reference-return alias binding observe the same covered source. Real
  reference containers, dynamic-property assignment targets, arbitrary
  reference expressions, dynamic `ArrayAccess` roots, nested mixed
  `ArrayAccess` chains, broader COW, exact alias destruction ordering,
  broader by-reference foreach, and native lowering remain unsupported.

## Milestone 1468: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing cookie encoding,
  expiration-date formatting, cookie deletion/replacement semantics,
  cache-header emission, session file persistence, locking or save-handler
  behavior, garbage collection, strict session-id validation, broader session
  option effects, exact warning/notice/deprecation text, binary string reads,
  shutdown/fatal/destructor edge ordering, include-path/stat-cache behavior,
  `open_basedir`, stream filters/wrappers, wrapper-specific stream contexts,
  request-body lifetime, host filesystem edge cases, or native lowering.
  Milestone 1468 adds bounded deterministic session file persistence when
  `session.save_path` is set through `ini_set()` and a bounded explicit
  `session_id()` is provided before start. `session_start()` loads an existing
  PHP-compatible `sess_<id>` file for string-keyed scalar and array session
  values, and `session_write_close()` writes the active `$_SESSION` array back
  for later `phpc run` invocations. Locking, save handlers, garbage
  collection, strict id validation, integer top-level session keys,
  object/resource session
  serialization, malformed session-file recovery diagnostics, broader option
  effects, exact warning text, and native lowering remain unsupported.

## Milestone 1469: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing real SQL parsing slices, dbDelta diff
  generation, schema rollback, expression indexes, index ordering/opclass
  metadata, host database inspection, table status/detail metadata, broader
  `wpdb`/MySQLi result or mutation behavior, arbitrary prepared
  ...
  Milestone 1469 adds bounded filtered column metadata over the MySQLi
  dynamic schema island: exact `SHOW FULL COLUMNS FROM <table> LIKE
  '<column>'`, `SHOW COLUMNS FROM <table> WHERE Field = '<column>'`, and
  `DESCRIBE`/`DESC <table> <column>` probes return the matching recorded
  column or an empty result set for missing exact columns. Real MySQL
  connectivity, wildcard `LIKE` filters, arbitrary predicates, dbDelta,
  full `wpdb`, persistent object cache, and native database lowering remain
  unsupported.

## Milestone 1470: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1466-1469 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  ...
  passed with `0` failures, `907` system PHP comparisons, and `642`
  `phpc-only` skipped fixtures.

## Milestone 1471: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: typed-property compatibility now accepts
  class/interface aliases registered after an object was instantiated for
  direct declared instance and static property writes. This remains bounded to
  simple class/interface typed-property checks; compound union/intersection/DNF
  property type objects, readonly/property-hook behavior, property attributes,
  file/line/doc-comment metadata, extension/internal metadata, function/closure
  reflection targets, magic hooks reached by WordPress, broader
  autoload/class-alias lifecycle parity, interface diagnostics, and native
  object lowering remain unsupported.

## Milestone 1472: Reference/COW WordPress Blocker

- [x] Runtime lane: added a bounded dynamic-property reference array literal
  assignment target. A direct public dynamic property such as
  `$store->{$name} = array(&$value, ...)` now records covered literal
  reference slots below the evaluated property name, so later stored-array
  `call_user_func_array()` invocation and reference-return alias binding reuse
  the same bounded alias metadata. This remains alias metadata, not real
  reference containers, and does not add append-offset reference-literal
  assignment targets, non-public dynamic-property alias roots, magic-property
  reference containers, arbitrary reference expressions, dynamic `ArrayAccess`
  roots, nested mixed `ArrayAccess` chains, broader array/object COW, exact
  alias destruction ordering, by-reference foreach expansion, or native
  lowering. Focused verification passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-refcow-1472 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0`: `cargo test -p phpc --test
  array_reference_literals
  array_reference_literals_assigned_to_dynamic_property_feed_call_user_func_array
  -- --test-threads=1`; full affected `cargo test -p phpc --test
  array_reference_literals -- --test-threads=1`; direct `cargo run -q -p
  phpc -- run
  tests/fixtures/milestone1472/reference_array_literals_dynamic_property_target.php`;
  direct `php
  tests/fixtures/milestone1472/reference_array_literals_dynamic_property_target.php`;
  `cargo run -q -p phpc -- test tests/fixtures/milestone1472`; `cargo run
  -q -p phpc -- test --compare-php tests/fixtures/milestone1472`; `cargo
  run -q -p phpc -- test --compare-php-json tests/fixtures/milestone1472`;
  `cargo run -q -p phpc -- test --list-fixtures
  tests/fixtures/milestone1472`; `cargo run -q -p phpc -- compile
  tests/fixtures/milestone1472/reference_array_literals_dynamic_property_target.php
  --emit-ir` rejected native lowering at the existing user-function lowering
  boundary; `cargo fmt --check`; `cargo check -p phpc`; and `git diff
  --check` passed. Full expensive `tools/run-tests.sh`, checkpoint, commit,
  and push were deferred per lane instructions.

## Milestone 1473: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: added bounded explicit session-id
  validation at `session_start()` time. Non-empty ids outside ASCII letters,
  digits, `_`, and `-` now emit a bounded recoverable warning, return `false`,
  leave session status as `PHP_SESSION_NONE`, avoid session cookie headers, and
  do not materialize `$_SESSION`. Broader PHP session-id policy, comma-id
  compatibility, exact PHP warning text, session file locking, save handlers,
  garbage collection, malformed session-file recovery diagnostics, cookie
  expiration/deletion/replacement semantics, cache headers, and native session
  lowering remain unsupported.

## Milestone 1474: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: added bounded `%` wildcard metadata filters
  over the MySQLi dynamic schema island. `SHOW TABLES LIKE`, `SHOW TABLE
  STATUS LIKE`, `SHOW [FULL] COLUMNS FROM <table> LIKE`, and `SHOW COLUMNS
  FROM <table> WHERE Field LIKE` now match deterministic schema metadata with
  sorted table/status rows. Real MySQL connectivity, `_` wildcard and SQL
  escape semantics, arbitrary predicates, broader identifier syntax, dbDelta,
  full `wpdb`, persistent object cache, and native database lowering remain
  unsupported.

## Milestone 1475: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1471-1474 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1471-1475 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1553` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1553` fixture tests
  passed with `0` failures, `909` system PHP comparisons, and `644`
  `phpc-only` skipped fixtures.

## Milestone 1476: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: added bounded compound property type metadata and
  write checks. Property declarations now accept union types such as
  `HookContract|OtherHook|null` and pure intersection types such as
  `HookContract&TaggedContract`; direct instance/static writes enforce those
  shapes through the current scalar coercion and class/interface relationship
  checks; and `ReflectionProperty::getType()` now materializes bounded
  `ReflectionUnionType` and `ReflectionIntersectionType` objects with
  `allowsNull()` and `getTypes()`. Parenthesized DNF property types, exact PHP
  union scalar coercion preference, compound parameter/return reflection type
  objects, readonly properties, property hooks, property attributes,
  reference/COW interactions, and native object lowering remain unsupported.

## Milestone 1477: Reference/COW WordPress Blocker

- [x] Runtime lane: added bounded append-offset reference array literal
  assignment targets. Direct append, nested append, and direct visible
  object-property append targets such as `$args[] = array(&$value)`,
  `$registry["groups"][] = array(&$value)`, and
  `$store->groups[] = array(&$items["slot"])` now record covered literal
  reference slots below the actual appended key, so later stored-array
  `call_user_func_array()` invocation, reference-return alias binding, and
  copied-array writes reuse the same alias metadata. This remains bounded alias
  metadata, not real reference containers, and does not add dynamic-property
  append targets, `ArrayAccess` append targets, non-public dynamic-property
  alias roots, magic-property reference containers, arbitrary reference
  expressions, dynamic `ArrayAccess` roots, nested mixed `ArrayAccess` chains,
  broader array/object COW, exact alias destruction ordering,
  by-reference foreach expansion, or native lowering.

## Milestone 1478: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: added bounded malformed session-file
  recovery for the current `session.save_path`/explicit `session_id()` file
  slice. If an existing `sess_<id>` file cannot be decoded by the bounded
  scalar/array session decoder, `session_start()` emits one recoverable
  warning, returns `true`, marks the session active, and materializes empty
  `$_SESSION`. Session file locking, save handlers, garbage collection, exact
  PHP warning/recovery parity, broader session-id policy, integer top-level
  session keys, object/resource session serialization, cookie deletion/
  replacement semantics, cache headers, and native session lowering remain
  unsupported.

## Milestone 1479: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing real SQL parsing slices, dbDelta diff
  generation, schema rollback, expression indexes, index ordering/opclass
  metadata, host database inspection, `_` wildcard or escaped metadata
  `LIKE`, broader `wpdb`/MySQLi result or mutation behavior, arbitrary
  prepared option/transient query shapes, object-cache/transient persistence,
  hooks under realistic callback shapes, deterministic plugin/theme loading
  probes, or a bootstrap/request probe that moves past its next real blocker.
  Closed with bounded `_` wildcard and backslash-escaped `%`, `_`, and `\`
  literal matching for the existing MySQLi dynamic schema metadata island,
  plus focused Rust and `phpc-only` fixture coverage. This remains deterministic
  in-memory schema-state evidence, not real MySQL support.

## Milestone 1480: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1476-1479 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1476-1480 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1557` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1557` fixture tests
  passed with `0` failures, `911` system PHP comparisons, and `646`
  `phpc-only` skipped fixtures.
...
- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  ...
  Closed with bounded `ReflectionFunction` metadata for declared user
  functions named by string plus `ReflectionParameter` support for function
  string targets. The slice covers function names, parameter counts,
  `getParameters()`, return-type metadata, by-reference-return flags,
  `getDeclaringFunction()`, and `getDeclaringClass() === null` for user
  function parameters. Internal function reflection, closure targets,
  attributes, file/line/doc-comment metadata, exact `ReflectionException`
  behavior, invocation/value mutation, and native reflection lowering remain
  unsupported.
...
- [x] Runtime lane: target real reference containers, magic-property reference
  ...
  Closed with bounded dynamic property-held `ArrayAccess` reference roots for
  direct holder variables. Statement-form reference assignment, literal
  `call_user_func_array()` argument arrays, and stored callback argument arrays
  now cover shapes such as `$alias =& $holder->{$name}["slot"]` when the
  selected visible property holds an `ArrayAccess` object whose public
  by-reference `offsetGet($offset)` returns `$this->property[$offset]`.
  Real reference containers, invisible selected properties, magic-property
  reference containers, arbitrary reference expressions, mixed nested
  `ArrayAccess` chains, alias cleanup after replacing containing properties,
  broader array/object COW, exact alias destruction ordering, by-reference
  foreach expansion, and native lowering remain unsupported.
...
- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  ...
  Closed with bounded `setrawcookie()` support in the deterministic CLI header
  log. It shares the current `setcookie()` signature, attributes, same-name
  replacement, output-started warning behavior, function-table/callability
  visibility, and native rejection, while preserving the raw string value.
  Cookie name validation/encoding, `Max-Age`, path/domain-aware replacement,
  full option validation, exact warning text, real SAPI emission, session
  cookie replacement parity, cache headers, session locking/save handlers/GC,
  and native header-state lowering remain unsupported.
...
- [x] Compatibility/runtime lane: target executable WordPress database or
  ...
  plugin/theme loading probes, or a bootstrap/request probe that moves past
  its next real blocker.
  Closed with bounded `ASC`/`DESC` index-part ordering metadata for the
  existing per-handle MySQLi dynamic schema-state island. Recorded
  `CREATE TABLE`/`ALTER TABLE` index definitions now expose descending parts
  through `SHOW INDEX`/`SHOW KEYS` `Collation` values and preserve descending
  parts in deterministic `SHOW CREATE TABLE` text. This remains in-memory
  schema evidence, not real MySQL index inspection, expression indexes, opclass
  metadata, dbDelta diff generation, or host database support.

## Milestone 1490: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1486-1489 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for `ReflectionFunction` user-function metadata, dynamic property-held
  `ArrayAccess` reference roots, bounded `setrawcookie()`, and MySQLi schema
  index ordering metadata; existing `get_declared_classes()` and enum
  metadata snapshots were updated for the new core `ReflectionFunction`
  placeholder. Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1486-1490 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1565` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1565` fixture tests
  passed with `0` failures, `915` system PHP comparisons, and `650`
  `phpc-only` skipped fixtures.

## Milestone 1491: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  closure reflection targets, function file/line/doc-comment metadata,
  internal/builtin reflection metadata, readonly/property-hook behavior,
  attributes, exact reflection exceptions, invocation/value mutation, magic
  hooks reached by WordPress, broader autoload/class-alias lifecycle parity,
  interface diagnostics, or broader class metadata behavior.
  Closed with bounded declared user-function `ReflectionFunction` source
  metadata. `getFileName()`, `getStartLine()`, `getEndLine()`, and
  `getDocComment()` now report the current parsed user-function source path,
  declaration/closing-brace lines, and directly preceding `/** ... */`
  docblock text, matching system PHP for the covered WordPress-style callback
  fixture. Internal function reflection, closure targets, attributes and exact
  docblock association across attributes/unusual trivia, invocation/value
  mutation, exact `ReflectionException` parity, broader method/class/property
  source metadata, and native reflection lowering remain unsupported.

## Milestone 1492: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference containers, magic-property reference
  containers, arbitrary reference expressions, non-direct dynamic holder
  expressions, invisible dynamic property reference sources, mixed nested
  `ArrayAccess` chains, alias cleanup after replacing containing properties,
  broader array/object copy-on-write, exact alias destruction ordering,
  by-reference foreach expansion, or native lowering.
  Closed with a bounded method-context dynamic property-held `ArrayAccess`
  reference-root slice for visible non-public holder properties. Statement-form
  reference assignment, literal `call_user_func_array()` argument arrays, and
  stored callback argument arrays now have fixture coverage for shapes such as
  `$alias =& $this->{$name}["slot"]`,
  `array(&$this->{$name}["outer"]["slot"], ...)`, and
  `$args["value"] =& $this->{$name}["created"]["leaf"]` when the selected
  private/protected property is visible and holds the bounded by-reference
  `offsetGet()` `ArrayAccess` object. Real reference containers,
  magic-property references, arbitrary expressions, non-direct holder
  expressions outside the slice, mixed nested `ArrayAccess` chains, alias
  cleanup, broad COW, alias destruction ordering, by-reference foreach, and
  native lowering remain unsupported.

## Milestone 1493: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing cookie
  ...
  handlers, garbage collection, binary string reads, shutdown/fatal/destructor
  ordering, include-path/stat-cache behavior, `open_basedir`, stream
  filters/wrappers, wrapper-specific stream contexts, request-body lifetime,
  host filesystem edge cases, or native lowering.
  Closed with a bounded `headers_sent($file, $line)` output-parameter evidence
  slice. The existing no-argument behavior is preserved, direct named and
  string-valued calls write direct-variable filename and line outputs for the
  first unbuffered output location, and direct variables backed by the current
  array-offset reference-alias metadata write through to the aliased slot.
  Non-variable, array/object-property, and callback-mediated output parameters,
  exact warning text, broader SAPI emission, shutdown-time buffer visibility,
  and native header-state lowering remain unsupported.

## Milestone 1494: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: add bounded dbDelta-relevant column metadata
  evidence over the deterministic MySQLi schema-state island. Milestone 1494
  preserves direct column `DEFAULT`/`DEFAULT NULL`/`NOT NULL`/
  `auto_increment` metadata plus inline column primary/unique/non-unique key
  metadata in exact `CREATE TABLE` and `ALTER TABLE ... ADD`/`MODIFY`/`CHANGE`
  forms, and exposes the recorded shape through `DESCRIBE`,
  `SHOW FULL COLUMNS`, `SHOW INDEX`, and deterministic `SHOW CREATE TABLE`.
  This remains a bounded schema island, not real SQL parsing, full dbDelta,
  host database inspection, expression indexes, persistent object cache, full
  `wpdb`, or native DB lowering.

## Milestone 1495: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1491-1494 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for declared-user-function `ReflectionFunction` source metadata,
  context-aware dynamic property-held `ArrayAccess` reference roots,
  `headers_sent($file, $line)` output parameters, and dbDelta-relevant MySQLi
  schema column metadata; milestone fixtures 1491-1494 all passed, with system
  PHP comparisons for 1491, 1492, and 1493 and a documented `phpc-only` skip
  for 1494. Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1491-1495 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1569` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1569` fixture tests
  passed with `0` failures, `918` system PHP comparisons, and `651`
  `phpc-only` skipped fixtures.
...
- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  ...
  replacement, session cookie replacement parity, cache headers, session
...
- [x] Parser/runtime lane: closed a bounded declared class-like
  `ReflectionClass` source metadata slice. Declared user classes, interfaces,
  and traits loaded from a known CLI/fixture or include path now report
  `getFileName()`, `getStartLine()`, `getEndLine()`, and `getDocComment()`
  from parsed declaration metadata, including parent `ReflectionClass` objects
  returned by `getParentClass()`. The `milestone1501` fixture matches system
  PHP for the covered WordPress-style class/interface/trait docblock and
  source-line shape. This does not add property/parameter source metadata,
  attributes or exact docblock association across attributes/unusual trivia,
  internal/builtin reflection source metadata, closure reflection targets,
  exact `ReflectionException` behavior, invocation/value mutation,
  readonly/property hooks, broader magic hooks, autoload/class-alias lifecycle
  parity, or native reflection lowering.
...
- [x] Runtime lane: target real reference containers, magic-property reference
  ...
  array/object copy-on-write, exact alias destruction ordering, by-reference
  foreach expansion, or native lowering.
  Closed with bounded alias cleanup for direct root-variable `unset($name)`.
  Removing a covered direct array root or direct object root now detaches
  aliases below that root, preserves each alias variable's last observed
  value, and prevents later alias writes from recreating the removed root.
  Remaining gaps include real reference containers, plain object-property
  `unset(...)` alias cleanup, magic-property reference containers, non-direct
  holder expressions, mixed nested `ArrayAccess` chains, broad COW, exact alias
  destruction ordering, by-reference foreach expansion, and native lowering.
...
- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  ...
  Closed with bounded path/domain-aware replacement for deterministic
  `setcookie()`/`setrawcookie()` headers. Same-name cookies now replace only
  when the normalized non-empty `path` and `domain` identity also match, so
  WordPress-shaped root/admin/network cookies can coexist while repeated
  writes to the same identity update the current CLI header log. Remaining
  gaps include cookie name/option validation, `Max-Age`, case-insensitive
  domain identity normalization, session cookie replacement parity, cache
  headers, session locking/save handlers/GC, binary reads, shutdown/fatal/
  destructor ordering, include-path/stat-cache behavior, stream wrappers/
  contexts, request-body lifetime, host filesystem edge cases, exact warning
  text, real SAPI emission, and native lowering.
...
- [x] Compatibility/runtime lane: target executable WordPress database or
  ...
  Closed with bounded `SHOW INDEX` filtering over the MySQLi schema-state
  island. `SHOW INDEX`/`SHOW INDEXES`/`SHOW KEYS FROM <table>` now supports
  `WHERE Key_name = '<key>'` and `WHERE Key_name LIKE '<pattern>'`, including
  backticked `` `Key_name` `` spellings, for deterministic WordPress
  schema-inspection probes. Remaining gaps include real SQL parsing, arbitrary
  `SHOW INDEX WHERE` predicates, custom `ESCAPE` clauses, SQL-mode-aware
  wildcard parsing, host database inspection, real index behavior, dbDelta
  diff generation, and native database lowering.

## Milestone 1505: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1501-1504 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for declared class-like `ReflectionClass` source metadata, root-variable
  `unset($name)` alias cleanup, path/domain-aware cookie replacement, and
  bounded `SHOW INDEX` `Key_name` filtering; milestone fixtures 1501-1504 all
  passed, with system PHP comparisons for 1501 and 1502 and documented
  `phpc-only` skips for 1503 and 1504. Manual full gate passed with
...
## Milestone 1506: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  property/parameter source metadata, closure reflection targets,
  internal/builtin reflection source metadata, attributes and exact docblock
  association, exact reflection exceptions, invocation/value mutation,
  readonly/property-hook behavior, WordPress-reached magic hooks, broader
  autoload/class-alias lifecycle parity, interface/trait source metadata, or
  broader class metadata behavior.
  Closed with bounded declared user-class `ReflectionProperty::getDocComment()`
  metadata. The parser now retains directly preceding `/** ... */` docblocks
  on class property declarations, and runtime reflection returns that docblock
  or `false` for properties resolved directly, through inherited public or
  protected metadata, and through `ReflectionClass::getProperties()`.
  Remaining gaps include property file/line metadata, parameter source
  metadata, attributes and exact docblock association across unusual trivia,
  internal/builtin reflection source metadata, closure reflection targets,
  exact `ReflectionException` objects, reflection invocation/value mutation,
  readonly/property-hook behavior, broader magic hooks, autoload/class-alias
  lifecycle parity, and native reflection lowering.

## Milestone 1507: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference containers, plain object-property
  `unset(...)` alias cleanup, magic-property reference containers, arbitrary
  reference expressions, non-direct holder expressions, invisible dynamic
  property reference sources, mixed nested `ArrayAccess` chains, alias cleanup
  beyond covered root/slot paths, broader array/object copy-on-write, exact
  alias destruction ordering, by-reference foreach expansion, or native
  lowering.
  Closed with a bounded plain object-property `unset(...)` alias cleanup slice:
  visible public properties and method-context private/protected properties now
  detach covered array-slot aliases below the removed property and preserve the
  remaining alias variables' last observed values. This does not add real
  reference containers, magic-property reference containers, arbitrary
  reference expressions, non-direct holder expressions, invisible dynamic
  property reference sources, mixed nested `ArrayAccess` chains, broader
  array/object copy-on-write, exact alias destruction ordering, by-reference
  `foreach` expansion, or native lowering.

## Milestone 1508: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing
  ...
  SAPI emission, or native lowering. Closed with bounded case-insensitive
  domain identity normalization for deterministic `setcookie()`/
  `setrawcookie()` replacement. Replacement now lowercases non-empty ASCII
  domain text for identity comparison while preserving the caller-provided
  domain in emitted `Set-Cookie` lines. Remaining gaps include cookie
  name/option validation, `Max-Age`, session cookie replacement parity, cache
  headers, session locking/save handlers/GC, binary reads, shutdown/fatal/
  destructor ordering, include-path/stat-cache behavior, stream wrappers/
  contexts, request-body lifetime, host filesystem edge cases, exact warning
  text, IDNA/trailing-dot/domain-policy canonicalization, real SAPI emission,
  and native lowering.

## Milestone 1509: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing broader dbDelta-relevant schema diffing,
  real SQL parsing slices, host database inspection, arbitrary query/result
  behavior, prepared statements, charset/collation/error fidelity,
  SQL-mode-aware wildcard behavior, persistent object-cache/transient
  ...
  blocker.
  Closed with bounded custom `LIKE ... ESCAPE '<char>'` parsing for the
  deterministic MySQLi schema metadata island, proved through a dbDelta-shaped
  `SHOW INDEX` `Key_name LIKE 'meta!_%' ESCAPE '!'` probe. Remaining gaps
  include real SQL parsing, arbitrary `SHOW COLUMNS WHERE` and `SHOW INDEX
  WHERE` predicates beyond the documented equality/LIKE forms,
  SQL-mode-aware wildcard parsing such as `NO_BACKSLASH_ESCAPES`, host
  database inspection, real index behavior, dbDelta diff generation, and
  native database lowering.

## Milestone 1510: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1506-1509 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for declared user-class `ReflectionProperty::getDocComment()` metadata,
  plain object-property `unset(...)` alias cleanup, case-insensitive cookie
  domain identity replacement, and bounded `LIKE ... ESCAPE` schema metadata
  filtering; milestone fixtures 1506-1509 all passed, with system PHP
  comparisons for 1506 and 1507 and documented `phpc-only` skips for 1508
  and 1509. Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1506-1510 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1581` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1581` fixture tests
  passed with `0` failures, `925` system PHP comparisons, and `656`
  `phpc-only` skipped fixtures.

## Milestone 1511: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  property file/line metadata, parameter source metadata, closure reflection
  targets, internal/builtin reflection source metadata, attributes and exact
  docblock association, exact reflection exceptions, invocation/value
  mutation, readonly/property-hook behavior, WordPress-reached magic hooks,
  broader autoload/class-alias lifecycle parity, interface/trait metadata, or
  broader class/object semantics.
  Closed with bounded `ReflectionClass::getTraitNames()` and `getTraits()`
  metadata for declared user classes that directly use supported traits.
  `getTraitNames()` returns the direct trait names in declaration order and
  `getTraits()` returns bounded `ReflectionClass` objects keyed by trait name,
  matching system PHP for the covered direct class/interface/trait fixture.
  Remaining gaps from that slice included trait-body `use` declarations, recursive helper
  aggregation across parent classes beyond userland `class_parents()` plus
  `class_uses()` patterns, built-in/internal trait catalogs, namespace import
  alias expansion beyond parsed class-like names, exact reflection exceptions,
  reflection invocation/value mutation, and native lowering.

## Milestone 1512: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference containers, magic-property
  reference containers, arbitrary reference expressions, non-direct holder
  expressions, invisible dynamic property reference sources, mixed nested
  `ArrayAccess` chains, alias cleanup beyond covered root/property/slot
  paths, broader array/object copy-on-write, exact alias destruction ordering,
  by-reference `foreach` expansion, or native lowering.
  Closed with a bounded by-reference `foreach` expansion for direct dynamic
  object-property array roots. Public roots such as
  `$object->{$name}["child"]` and method-context private/protected roots such
  as `$this->{$name}` now use the evaluated property name with the existing
  context-aware object-property alias metadata, so loop writes, appended tail
  entries, and post-loop lingering references match the covered PHP behavior.
  Remaining gaps include real reference containers, magic-property reference
  containers, arbitrary reference expressions, non-direct dynamic-property
  holder expressions, invisible selected dynamic properties, mixed nested
  `ArrayAccess` chains, alias cleanup beyond covered root/property/slot paths,
  broader array/object copy-on-write, exact alias destruction ordering,
  object/Traversable by-reference iteration, and native lowering.

## Milestone 1513: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing cookie
  name/option validation, `Max-Age`, session cookie replacement parity, cache
  headers, session locking, save handlers, garbage collection, binary reads,
  shutdown/fatal/destructor ordering, include-path/stat-cache behavior, stream
  wrappers/contexts, request-body lifetime, host filesystem edge cases, exact
  warning text, IDNA/trailing-dot/domain-policy canonicalization, real SAPI
  emission, or native lowering.
  Closed with bounded `Max-Age` emission for nonzero deterministic
  `setcookie()`/`setrawcookie()` expirations in the CLI header log. Past
  expirations now emit `Max-Age=0`, covering WordPress-style cookie deletion
  headers while preserving existing value encoding, raw-cookie behavior,
  path/domain attributes, and replacement identity. Remaining gaps include
  cookie name validation/encoding, exact request-time/Date-header parity for
  future `Max-Age` values, array option validation beyond the documented
  keys, session cookie replacement parity, cache headers, session locking,
  save handlers, garbage collection, binary reads, shutdown/fatal/destructor
  ordering, include-path/stat-cache behavior, broader stream wrappers and
  context effects, request-body lifetime, host filesystem edge cases, exact
  warning text, IDNA/trailing-dot/domain-policy canonicalization, real SAPI
  emission, and native lowering.

## Milestone 1514: WordPress DB/Bootstrap Evidence Blocker
...
- [x] Compatibility/runtime lane: target executable WordPress database or
  ...
  loading probes, or a bootstrap/request probe that moves past its next real
  blocker.
  Closed with bounded per-handle `NO_BACKSLASH_ESCAPES` handling for dynamic
  schema metadata `LIKE` filters in the MySQLi schema-state island. Accepted
  `SET SESSION sql_mode='NO_BACKSLASH_ESCAPES'` assignments now affect later
  `SHOW TABLES`, `SHOW TABLE STATUS`, `SHOW COLUMNS`, and `SHOW INDEX` LIKE
  filtering on the same placeholder handle, while explicit `ESCAPE` clauses
  remain supported. Remaining gaps include broad SQL-mode semantics, exact
  MySQL string-literal and wildcard parity, host database inspection,
  arbitrary SQL/dbDelta diffing, real index behavior, charset/collation/error
  fidelity, persistent cache/transients, plugin/theme loading, and native
  database lowering.

## Milestone 1515: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1511-1514 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for direct `ReflectionClass` trait metadata, by-reference foreach over
  direct dynamic object-property array roots, deterministic cookie `Max-Age`
  emission, and bounded SQL-mode-aware schema `LIKE` filtering; milestone
  fixtures 1511-1514 all passed, with system PHP comparisons for 1511 and
  1512 and documented `phpc-only` skips for 1513 and 1514. Regression
  snapshots for milestone1483 and milestone1488 were updated for the new
  `Max-Age=0` cookie output and rechecked. Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1511-1515 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1585` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1585` fixture tests
  passed with `0` failures, `927` system PHP comparisons, and `658`
  `phpc-only` skipped fixtures.

## Milestone 1516: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  trait-body `use` declarations, property file/line metadata, parameter
  source metadata, closure reflection targets, internal/builtin reflection
  source metadata, attributes and exact docblock association, exact reflection
  exceptions, invocation/value mutation, readonly/property-hook behavior,
  WordPress-reached magic hooks, broader autoload/class-alias lifecycle
  parity, interface/trait metadata, or broader class/object semantics.
  Closed with a bounded trait-body `use` composition slice. Simple
  `use TraitName;` and `use TraitA, TraitB;` declarations inside traits now
  parse and compose supported public methods and constants into classes that
  consume the outer trait, while direct class trait reflection remains
  non-recursive. The new `milestone1516` fixture proves `phpc run` CLI
  behavior and matches system PHP for the covered nested trait method,
  constant, and direct trait metadata shape. Trait-body adaptations, recursive
  trait reflection metadata, trait properties, non-public/static/abstract/final
  trait members, exact PHP conflict diagnostics, references/COW, autoloading of
  nested trait dependencies, and native trait lowering remain unsupported.

## Milestone 1517: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference containers, magic-property
  reference containers, arbitrary reference expressions, non-direct dynamic
  property holder expressions, invisible selected dynamic properties, mixed
  nested `ArrayAccess` chains, alias cleanup beyond covered root/property/slot
  paths, broader array/object copy-on-write, exact alias destruction ordering,
  object/Traversable by-reference iteration, or native lowering. Closed with a
  bounded non-direct dynamic-property holder slice for by-reference `foreach`:
  object-result holder expressions such as `$holders["bag"]->{$name}["child"]`
  and method-context `$this->holder()->{$name}` now evaluate the holder once
  into a temporary object root and reuse the existing visible object-property
  alias machinery, so loop writes and lingering references update the selected
  property array slot. Real reference containers, magic-property reference
  containers, arbitrary reference expressions, invisible selected dynamic
  properties, mixed nested `ArrayAccess` chains, alias cleanup beyond covered
  root/property/slot paths, broader array/object copy-on-write, exact alias
  destruction ordering, object/Traversable by-reference iteration, and native
  lowering remain open.

## Milestone 1518: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing cookie
  name validation/encoding, exact request-time/Date-header parity for future
  `Max-Age` values, array option validation beyond documented keys, session
  cookie replacement parity, cache headers, session locking, save handlers,
  garbage collection, binary reads, shutdown/fatal/destructor ordering,
  include-path/stat-cache behavior, broader stream wrappers and context
  effects, request-body lifetime, host filesystem edge cases, exact warning
  text, IDNA/trailing-dot/domain-policy canonicalization, real SAPI emission,
  or native lowering.
  Closed with a bounded `setcookie()`/`setrawcookie()` options-array key
  validation slice. Options-array calls now reject numeric keys and unknown
  string keys before changing the deterministic CLI header log, with focused
  Rust tests and a `milestone1518` CLI fixture. The fixture is `phpc-only`
  because system PHP throws `ValueError` objects for the same invalid option
  shape while phpc still records a bounded runtime diagnostic. Remaining SAPI
  gaps include cookie name validation/encoding, exact request-time/Date-header
  parity for future `Max-Age` values, case-insensitive option-key matching,
  exact `ValueError` text/objects, session/cache/locking/save-handler
  behavior, real SAPI emission, and native lowering.

## Milestone 1519: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing broad SQL-mode semantics, exact MySQL
  string-literal and wildcard parity, broader dbDelta-relevant schema diffing,
  real SQL parsing slices, host database inspection, arbitrary query/result
  behavior, prepared statements, charset/collation/error fidelity,
  ...
  Closed with bounded prepared schema metadata filtering over the MySQLi
  dynamic schema-state island. `mysqli_execute_query()` and
  `mysqli_prepare()`/`mysqli_stmt_execute(..., array(...))` now route one
  string placeholder parameter into the documented `SHOW TABLES`, `SHOW TABLE
  STATUS`, `SHOW COLUMNS`, and `SHOW INDEX`/`SHOW KEYS` equality/`LIKE`
  metadata filters, including `LIKE ? ESCAPE '<char>'` and the existing
  per-handle `NO_BACKSLASH_ESCAPES` branch. Remaining gaps include arbitrary
  prepared SQL, host database inspection, broad SQL parsing, exact MySQL
  string-literal/wildcard parity beyond this metadata slice, dbDelta diffing,
  real charset/collation/error fidelity, persistent cache/transients,
  plugin/theme loading, bootstrap request progress, and native database
  lowering.

## Milestone 1520: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1516-1519 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for trait-body `use` composition, by-reference foreach over non-direct
  dynamic-property holder roots, invalid cookie options-array rejection, and
  prepared schema metadata filters; milestone fixtures 1516-1519 all passed,
  with system PHP comparisons for 1516 and 1517 and documented `phpc-only`
  skips for 1518 and 1519. Manual full gate passed with
  `CARGO_TARGET_DIR=/tmp/phpc-target-full-1516-1520 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1589` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1589` fixture tests
  passed with `0` failures, `929` system PHP comparisons, and `660`
  `phpc-only` skipped fixtures.

## Milestone 1521: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  trait conflict diagnostics and adaptation syntax, trait properties,
  inherited interface compatibility, constructor/destructor fidelity, magic
  hooks reached by WordPress, class alias/autoload lifecycle, parameter/source
  reflection metadata, closure/internal reflection targets, property file/line
  metadata, attributes/docblock association, exact reflection exceptions,
  invocation/value mutation, readonly/property-hook behavior, or broader
  class/object semantics. Keep native trait/object lowering as an explicit
  rejection unless implemented with tests. Closed with a bounded unresolved
  trait method conflict diagnostic. When two different composed traits provide
  the same public instance method and no class method or bounded `insteadof`
  adaptation resolves it, `phpc run` now fails with a trait-specific stable
  diagnostic before class registration. The new `milestone1521` fixture proves
  the CLI path and is marked `phpc-only` because system PHP reports an
  engine-specific fatal error. Exact PHP fatal-error text, executing
  unresolved conflicts, trait-body adaptations, trait properties,
  non-public/static/abstract/final trait methods, broad conflict resolution,
  and native trait lowering remain unsupported.

## Milestone 1522: Reference/COW WordPress Blocker

- [x] Runtime lane: advanced a bounded alias-lifetime slice for
  by-reference `foreach` over non-direct named-property holder expressions
  that evaluate to objects. Named-property forms such as
  `$holders["bag"]->items["outer"]` and method-context
  `$this->holder()->items` now reuse the existing temporary object root and
  visible property alias machinery, matching the already covered non-direct
  dynamic-property slice for loop writes and post-loop lingering references.
  This does not add general reference containers, arbitrary reference
  expressions, invisible selected properties, magic-property reference
  containers, mixed nested `ArrayAccess` chains, broader COW, exact alias
  destruction ordering, object/Traversable by-reference iteration,
  superglobal reference lifetime beyond existing covered roots, or native
  reference/COW lowering.

## Milestone 1523: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing cookie name
  validation/encoding, exact request-time/Date-header parity, case-insensitive
  cookie option keys if PHP-compatible, session cookie replacement parity,
  cache headers, sessions, locking/save handlers, garbage collection, binary
  reads, output buffering, shutdown/fatal/destructor ordering,
  include-path/stat-cache behavior, stream wrappers/context effects,
  request-body lifetime, host filesystem edge cases, upload metadata, exact
  warning text, IDNA/trailing-dot/domain-policy canonicalization, real SAPI
  emission, or native lowering.
  Closed with bounded case-insensitive options-array key matching for the
  documented `setcookie()`/`setrawcookie()` option set. The runtime now accepts
  mixed-case forms such as `Expires`, `Path`, `Secure`, and `SameSite`, while
  preserving the existing pre-mutation rejection for numeric keys and unknown
  string keys. Remaining SAPI gaps include cookie name validation/encoding,
  exact request-time/Date-header parity, exact `ValueError` text/objects,
  session/cache/locking/save-handler behavior, upload/body edge cases, real
  SAPI emission, and native lowering.

## Milestone 1524: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing broad SQL-mode semantics, exact MySQL
  string-literal and wildcard parity, broader dbDelta-relevant schema diffing,
  real SQL parsing slices, host database inspection, arbitrary query/result
  behavior, prepared statement result binding, transactions and errors,
  charset/collation fidelity, persistent object-cache/transient behavior,
  hook callback shapes, deterministic plugin/theme loading probes, REST/admin
  bootstrap probes, or a front-controller probe that moves past its next real
  blocker. Closed with a bounded prepared-statement result-binding slice:
  `mysqli_stmt_bind_result()` now accepts direct variable array-offset targets
  for deterministic buffered placeholder results, and the milestone1524
  fixture proves `phpc run` writes seed-post columns into a `wpdb`-style row
  array while keeping native DB lowering unsupported.

## Milestone 1525: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1521-1524 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for unresolved trait method conflict diagnostics, by-reference foreach over
  non-direct named-property holder roots, case-insensitive cookie option keys,
  and MySQLi statement result binding into direct row-array slots; milestone
  fixtures 1521-1524 all passed, with a system PHP comparison for 1522 and
  documented `phpc-only` skips for 1521, 1523, and 1524. Manual full gate
  passed with `CARGO_TARGET_DIR=/tmp/phpc-target-full-1521-1525
  CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test`
  completed successfully, `phpc test` reported `1593` fixture tests passed
  with `0` failures, and `phpc test --compare-php` reported `1593` fixture
  tests passed with `0` failures, `930` system PHP comparisons, and `663`
  `phpc-only` skipped fixtures.

## Milestone 1526: Object/Interface WordPress Blocker

- [x] Parser/runtime lane: target the next object/class blocker, prioritizing
  exact trait conflict/adaptation behavior, trait properties, inherited
  interface compatibility, constructor/destructor fidelity, magic hooks reached
  by WordPress, class alias/autoload lifecycle, parameter/source reflection
  metadata, closure/internal reflection targets, property file/line metadata,
  attributes/docblock association, exact reflection exceptions,
  invocation/value mutation, readonly/property-hook behavior, or broader
  class/object semantics. Keep native trait/object lowering as an explicit
  rejection unless implemented with tests. Completed as a bounded trait
  property slice: class-body and simple nested trait composition now carries
  supported trait properties into consuming class metadata/defaults, dedupes
  identical duplicate trait/class property definitions, and rejects
  incompatible composed trait properties with a stable trait-use diagnostic.

## Milestone 1527: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference/COW behavior, prioritizing general
  ...
  Completed as a bounded alias-lifetime slice for by-reference `foreach` over
  multi-slot reference-return child-array aliases. A reference-returning
  function that returns `return $param[$key];` can now feed by-reference
  iteration when `$param` was supplied by a direct caller variable sharing its
  cell with another direct name; loop writes and the lingering post-loop
  reference bind through the covered alias group. This does not add general
  reference containers, arbitrary reference expressions, magic-property
  reference containers, mixed nested `ArrayAccess` chains, broad COW, exact
  alias destruction ordering, object/Traversable by-reference iteration,
  superglobal reference lifetime beyond the covered alias metadata, or native
  reference/COW lowering.

## Milestone 1528: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: implement bounded duplicate differently cased
  `setcookie()`/`setrawcookie()` options-array key parity by using the last
  inserted documented option value after ASCII-case-insensitive matching, with
  focused header tests, a CLI fixture, docs, and native header-state rejection
  kept explicit. Remaining gaps include cookie name validation/encoding, exact
  ...
  output buffering, shutdown/fatal/destructor ordering, include-path/stat-cache
  behavior, stream wrappers/context effects, request-body lifetime, host
  filesystem edge cases, upload metadata, exact warning text,
  IDNA/trailing-dot/domain-policy canonicalization, real SAPI emission, and
  native lowering.

## Milestone 1529: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing true by-reference result aliases,
  object-property result targets, unbuffered statement fetching, arbitrary
  prepared SQL, broad SQL-mode semantics, exact MySQL string-literal and
  wildcard parity, dbDelta-relevant schema diffing, host database inspection,
  arbitrary query/result behavior, transactions and errors,
  charset/collation fidelity, persistent object-cache/transient behavior,
  hook callback shapes, deterministic plugin/theme loading probes, REST/admin
  bootstrap probes, or a front-controller probe that moves past its next real
  blocker. Completed as a bounded prepared-statement result-binding slice:
  `mysqli_stmt_bind_result()` now accepts direct object-property result targets
  such as `$row->ID` and direct object-property array offsets such as
  `$row->fields[$key]`, with keys evaluated at bind time and deterministic
  placeholder rows copied during `mysqli_stmt_fetch()`. This does not add true
  by-reference result aliases, dynamic object-property target expressions,
  unbuffered statement fetching, arbitrary prepared SQL, real database state,
  exact warning/error fidelity, persistent cache/transients, broader bootstrap
  progress, or native DB lowering.

## Milestone 1530: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1526-1529 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for trait-property composition, multi-slot reference-return by-reference
  foreach aliases, duplicate differently cased cookie option keys, and MySQLi
  statement result binding into direct object-property targets; milestone
  fixtures 1526-1529 all passed, with system PHP comparisons for 1526 and
  1527 and documented `phpc-only` skips for 1528 and 1529. The stale
  milestone707 trait-property unsupported fixture was retargeted to the still
  unsupported trait-body adaptation-in-trait boundary. Manual full gate passed
  with `CARGO_TARGET_DIR=/tmp/phpc-target-full-1526-1530 CARGO_BUILD_JOBS=1
  CARGO_INCREMENTAL=0 tools/run-tests.sh`: `cargo test` completed
  successfully, `phpc test` reported `1597` fixture tests passed with `0`
  failures, and `phpc test --compare-php` reported `1597` fixture tests
  passed with `0` failures, `932` system PHP comparisons, and `665`
  `phpc-only` skipped fixtures.

## Milestone 1531: Object/Interface WordPress Blocker

- [x] Object/interface lane: target the next large class/object blocker,
  prioritizing trait-body adaptations, inherited interface compatibility,
  ...
  reflection exceptions, invocation/value mutation, readonly/property-hook
  ...
  Completed as a bounded trait-body method-adaptation slice: trait-body
  `use` declarations inside traits may now use the same current public
  instance method `as` aliases, visibility-only adaptations, protected/private
  aliases, qualified `insteadof` conflict resolution, and same-block winner
  alias interaction already supported for class-body trait use declarations.
  The new `milestone1531` fixture proves a WordPress-style helper trait that
  adapts nested traits before a class consumes it, satisfies an interface
  through the nested public alias, calls a protected nested alias from class
  context, and matches system PHP. This does not add non-public/static/
  abstract/final trait methods, unqualified `insteadof`, unqualified
  visibility-only adaptations across multiple used traits, trait property or
  constant adaptations, exact PHP fatal wording, recursive trait-body
  adaptation breadth beyond the existing stable recursive-use diagnostic,
  `__TRAIT__` context, references/copy-on-write, or native trait/object
  lowering.

## Milestone 1532: Reference/COW WordPress Blocker

- [x] Runtime lane: target real reference/COW behavior. Milestone 1532 adds a
  bounded by-reference `foreach` bridge for direct and visible property-held
  `ArrayAccess` offset-array roots such as `$bag["outer"]` and
  `$holder->{$name}["outer"]`, reusing the existing exact public by-reference
  `offsetGet($offset) { return $this->property[$offset]; }` mapping so loop
  writes, appended tail visitation, and post-loop lingering references route
  through the backing property array slot. General reference containers,
  magic-property reference containers, arbitrary reference expressions,
  invisible selected dynamic properties, non-direct holder ArrayAccess roots,
  mixed nested `ArrayAccess` chains, broader array/object copy-on-write, exact
  alias destruction ordering, object/Traversable by-reference iteration,
  superglobal reference lifetime, and native lowering remain unsupported.

## Milestone 1533: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: added bounded `setcookie()`/`setrawcookie()` cookie-name
  validation before deterministic header-log mutation. Names must be non-empty
  strings and cannot contain `"="`, `","`, `";"`, space, tab, carriage return,
  newline, vertical tab, or form feed, matching PHP's rejected byte set while
  preserving phpc's current stable runtime diagnostic instead of exact
  `ValueError` objects/text. The new `milestone1533` fixture pins the CLI
  runtime error path. Remaining SAPI gaps include cookie-name encoding, exact
  request-time/Date-header parity, session cookie replacement parity, cache
  headers, sessions, locking/save handlers, garbage collection, binary reads,
  output buffering, shutdown/fatal/destructor ordering, include-path/stat-cache
  behavior, stream wrappers/context effects, request-body lifetime, host
...
## Milestone 1534: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing true by-reference result aliases,
  unbuffered statement fetching, arbitrary prepared SQL, broad SQL-mode
  ...
  Completed as a bounded unbuffered prepared-statement fetch slice:
  `mysqli_stmt_fetch()` can now consume the deterministic executed placeholder
  statement result without requiring `mysqli_stmt_store_result()` first, while
  `mysqli_stmt_num_rows()` remains buffered-only before an explicit store. The
  new WordPress-shaped `wpdb` probe proves this path through `phpc run`. This
  does not add true by-reference result aliases, real mysqlnd unbuffered
  transfer, arbitrary prepared SQL, host database state, PHP warning/error
  fidelity, persistent cache/transients, broader bootstrap progress, or native
  DB lowering.

## Milestone 1535: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1531-1534 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
  Closed after integrating all four lanes. Focused integrated checks passed
  for nested trait-body adaptations, by-reference foreach over covered
  `ArrayAccess` offset roots, cookie-name validation, and unbuffered prepared
  statement fetches; milestone fixtures 1531-1534 all passed, with system PHP
  comparisons for 1531 and 1532 and documented `phpc-only` skips for 1533 and
  1534. The retargeted milestone707 trait-member boundary also passed. The
...
- [x] Object/interface lane: target the next large class/object blocker,
  ...
  attributes/docblock association, exact reflection exceptions,
  invocation/value mutation, readonly/property-hook behavior, broader
...
- [x] Object/interface lane: closed with a bounded `ReflectionProperty` value
  mutation slice. Public declared instance and static properties now support
  `getValue()`/`setValue()` through `phpc run`, including inherited public
  instance properties and current typed-property coercions. Non-public and
  dynamic property value reflection, exact reflection exception text,
  property alias/reference/COW side effects, readonly/property hooks, and
  native lowering remain unsupported.

## Milestone 1542: Reference/COW WordPress Blocker

- [x] Runtime lane: closed with a bounded by-reference parameter bridge for
  non-direct holder property-held `ArrayAccess` offset arguments. Direct
  user-function calls such as
  `handler($holders["bag"]->store["outer"]["slot"])` and dynamic selected
  property forms such as `handler($holders["bag"]->{$name}["outer"]["slot"])`
  now evaluate the holder once, reuse the exact supported public
  by-reference `offsetGet($offset) { return $this->property[$offset]; }`
  bridge, and write callee mutations back through the backing property array
  slot. General reference containers, broader COW, arbitrary reference
  expressions, mixed nested `ArrayAccess` chains, superglobal lifetime, and
  native reference/COW lowering remain unsupported.

## Milestone 1543: Request/SAPI/Filesystem/Stream Blocker

- [x] Runtime lane: closed with a bounded request-local PHP stat-cache slice
  for successful local `filesize()` and `filemtime()` metadata reads.
  `clearstatcache()` clears all cached entries, and
  `clearstatcache(false, $path)` invalidates one resolved local path. Realpath
  cache entries, stat-cache coverage for other metadata functions and
  wrappers, include_path/open_basedir policy, stream-wrapper cache
  interaction, exact warnings/errors, broader SAPI/session/upload/output
  behavior, and native stat-cache lowering remain unsupported.

## Milestone 1544: WordPress DB/Bootstrap Evidence Blocker

- [x] Compatibility/runtime lane: closed with a bounded prepared `wp_options`
  `option_name LIKE ?` wildcard scan slice for exact option-row projections.
  Prepared scans now distinguish `%`, `_`, and backslash-escaped wildcard
  literals. Prepared LIKE deletes, expired-timeout predicates, prepared
  `ESCAPE` clauses, SQL-mode-aware option reads, arbitrary prepared SQL, host
  DB execution, collation fidelity, object cache/transients, and native DB
  lowering remain unsupported.

## Milestone 1545: WordPress-Focused Queue Refresh

- [x] Tests/docs lane: after Milestones 1541-1544 landed, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.

## Milestone 1546: Object/Interface WordPress Blocker

- [ ] Object/interface lane: target the next large class/object blocker,
  prioritizing inherited interface compatibility, constructor/destructor
  fidelity, magic hooks reached by WordPress, class alias/autoload lifecycle,
  closure/internal reflection targets, static or non-public reflection
  invocation, exact reflection exceptions, readonly/property-hook behavior,
  broader recursive trait adaptation behavior, or broader class/object
  semantics. Keep native object/trait lowering as an explicit rejection unless
  implemented with tests.

## Milestone 1547: Reference/COW WordPress Blocker
...
## Milestone 1548: Request/SAPI/Filesystem/Stream Blocker

- [ ] Runtime or IR/lowering lane: target request/SAPI, filesystem, and stream
  behavior that blocks real WordPress requests, prioritizing realpath-cache
...
## Milestone 1549: WordPress DB/Bootstrap Evidence Blocker

- [ ] Compatibility/runtime lane: target executable WordPress database or
  bootstrap evidence, prioritizing prepared LIKE deletes, expired-transient
  predicates with wildcard parity, prepared `ESCAPE` clauses, real mysqlnd
...
## Milestone 1550: WordPress-Focused Queue Refresh

- [ ] Tests/docs lane: after Milestones 1546-1549 land, refresh the
  compatibility ledger, support docs, progress log, lane-worker queue, and
  loop memory; record focused verification; run the serialized full gate; and
  checkpoint the batch.
