...
- `str_replace()` over current scalar and stringable-object search,
  replacement, and scalar subject operands after scalar string conversion,
  returning the replaced byte string. Scalar search/replacement over array
  subjects maps each top-level ordered value through the same byte replacement
  path while preserving keys and writing the aggregate optional `$count`
  argument by reference. Invalid top-level resource, closure, exception, and
  non-stringable object operands throw catchable PHP-style `array|string`
  `TypeError` diagnostics. Array search/replacement forms and full nested
  subject parity remain outside the current bounded path.
...
- Exact `soundex()` locale/non-ASCII behavior plus resource/reference operand
  ...
  `__toString()` support.
- Complete `str_replace()` array search/replacement semantics, full nested
  array-subject parity, array-to-string warnings, and nested replacement-count
  parity beyond the current scalar search/replacement bounded path.
