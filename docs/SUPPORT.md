...
- `str_replace()` over current scalar/stringable-object operands and array
  search/replacement/subject forms, returning replaced byte strings or mapped
  arrays with subject keys preserved, pairing search and replacement arrays by
  insertion order, treating missing replacement entries as empty strings,
  writing the optional `$count` argument by reference, and throwing catchable
  PHP-style `array|string` `TypeError` diagnostics for invalid top-level
  resource, closure, exception, and non-stringable object operands.
...
- Exact `soundex()` locale/non-ASCII behavior plus resource/reference operand
  ...
  `__toString()` support.
- Complete `str_replace()` nested element conversion parity, array-to-string
  warnings, and remaining object/resource edge diagnostics beyond the current
  bounded array path.
