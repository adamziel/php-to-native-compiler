...
- Assembly emission: uses LLVM tools when available, with a temporary `cc -S`
  ...
  discovery semantics for every tool, backend-specific stdout/stderr
  guarantees, backend-specific assembly text, PHP zvals, native symbol-table
  storage, references/copy-on-write, exact native error objects, and broader
