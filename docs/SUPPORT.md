...
- builtins for the documented subset: `strlen`, `strtolower`, `trim`, `ltrim`,
  ...
  unsupported.
  `urlencode($string)` and `rawurlencode($string)` accept scalar/null
  string-convertible inputs over the current byte string model. `urlencode()`
  uses application/x-www-form-urlencoded spacing and encodes `~`, while
  `rawurlencode()` uses RFC 3986 spacing and preserves `~`.
  `urldecode($string)` and `rawurldecode($string)` decode percent escapes,
  leave malformed or incomplete escapes literal, and return a binary-string
  value when decoded bytes are not valid UTF-8. `urldecode()` additionally
  maps `+` to a space; `rawurldecode()` preserves `+`. Arrays, exact
  TypeError objects/text, disabled-function policy, and native lowering remain
  unsupported for these URL codec builtins.
...
- Native codegen: LLVM IR/assembly supports only straight-line echo/assignment
  ...
  `error_reporting`, `min`, `rand`, `uniqid`, `hash_hmac`, `md5`, `basename`, `dirname`, `file_exists`, `file_get_contents`, `is_uploaded_file`, `move_uploaded_file`, `str_getcsv`, `parse_str`, `urlencode`, `rawurlencode`, `urldecode`, `rawurldecode`,
  `file_put_contents`, `readfile`, `unlink`, `mkdir`, `rmdir`, `copy`, `rename`, `chdir`, `scandir`, `stat`, `lstat`, `fileperms`, `chmod`,
  `fopen`, `stream_context_create`, `stream_context_get_options`, `stream_context_get_params`, `stream_context_get_default`, `stream_context_set_default`, `stream_context_set_option`, `stream_context_set_params`, `fwrite`, `fscanf`, `fread`, `rewind`, `stream_get_contents`, `feof`, `ftell`, `fseek`, `fflush`, `ftruncate`, `fstat`, `stream_get_meta_data`, `fclose`, `opendir`, `readdir`, `rewinddir`, `closedir`, `filesize`, `filemtime`,
...
- Function calls: user-defined positional calls are supported in `phpc run`,
  ...
  `str_contains`, `str_starts_with`, `str_ends_with`, `strspn`, `strcspn`, `strpbrk`, `strpos`, `stripos`, `strrpos`, `strripos`, `strstr`, `strchr`, `stristr`, `strtok`, `substr`, `substr_replace`, `substr_count`, `preg_match`, `preg_replace`, `preg_split`, `preg_replace_callback`, `str_replace`, `str_getcsv`, `urlencode`, `rawurlencode`, `urldecode`, `rawurldecode`, `error_reporting`,
...
- Builtins: `strlen`, `strtolower`, `trim`, `ltrim`, `rtrim`, `strcasecmp`, `strncmp`, `strncasecmp`, `str_contains`,
  `str_starts_with`, `str_ends_with`, `strspn`, `strcspn`, `strpbrk`, `strpos`, `stripos`, `strrpos`, `strripos`, `strstr`, `strchr`, `stristr`, `strtok`, `substr`, `substr_replace`, `substr_count`, `str_replace`, `str_getcsv`, `parse_str`, `urlencode`, `rawurlencode`, `urldecode`, `rawurldecode`, `printf`, `fprintf`, `sprintf`, `vsprintf`, `vprintf`, `vfprintf`,
  ...
  `strpos`, `stripos`, `strrpos`, `strripos`, `substr`, `urlencode`,
  `rawurlencode`, `urldecode`, `rawurldecode`, `printf`, `fprintf`,
  `sprintf`, `vprintf`, `vfprintf`, `implode`, `basename`, `dirname`, `defined`,
...
- `urlencode()`/`rawurlencode()`/`urldecode()`/`rawurldecode()` outside the
  current scalar/null byte-string argument subset: array/object/resource
