...
and subject values. Both share the same one-level search-array and direct
count-output path; `str_ireplace()` applies ASCII case-insensitive matching
before replacing the original subject slice. Native function-table
introspection recognizes both names, while direct native calls still reject
until string allocation, array forms, count-output references, and diagnostics
have a lowered runtime model.
