...
- `PtnValue` is a tagged union over scalar payloads plus `const char *string`
  ...
  (`src/backend/runtime.rs:185`, `src/backend/runtime.rs:192`), so ownership is
  not represented in the value itself.
- `ptn_duplicate_string()` allocates C-string copies
  (`src/backend/runtime.rs:225`). `ptn_value_to_string()` always returns an
  owned duplicate for scalar string conversion (`src/backend/runtime.rs:2193`),
  ...
  `src/backend/runtime.rs:2220`, `src/backend/runtime.rs:2240`).
- `ptn_echo()` writes scalar values directly and does not allocate for current
  scalar output (`src/backend/runtime.rs:2592`). Echo is not the major
  allocation hotspot after the direct scalar path.
- `ptn_concat()` borrows or owns converted operands, but always allocates a new
  joined string result (`src/backend/runtime.rs:2267`). Generated chained concat
  therefore allocates one owned string per binary concat node.
- Array string keys are duplicated and can be freed
  (`src/backend/runtime.rs:242`, `src/backend/runtime.rs:311`). Literal arrays
  allocate a `PtnArray`, entry vector, and optional index slots
  (`src/backend/runtime.rs:457`), but there is no `PtnArray` or `PtnValue`
  destructor.
- `ptn_array_set_entry()` replaces an existing entry value without freeing the
  overwritten payload (`src/backend/runtime.rs:439`). Array reads and foreach
  return shallow `PtnValue` aliases (`src/backend/runtime.rs:1180`,
  `src/backend/runtime.rs:1447`).
- Symbol tables duplicate names and free names/index storage on teardown
  (`src/backend/runtime.rs:493`), but they do not free stored values.
  `ptn_symbols_set()` overwrites values in place without releasing the old
  payload (`src/backend/runtime.rs:597`). `ptn_runtime_free()` only frees symbol
  tables (`src/backend/runtime.rs:770`).
- User-function frames initialize a fresh runtime symbol table and store
  by-value parameter `PtnValue`s from caller arguments (`src/backend.rs:123`,
  `src/backend.rs:140`). Frame teardown has the same symbol-table-only cleanup
  boundary (`src/backend.rs:172`).
- Generated expression statements and internal-call statements materialize a
  ...
  (`src/backend.rs:258`, `src/backend.rs:298`). Runtime binary expressions and
  calls similarly create temporaries without ownership cleanup
  (`src/backend.rs:1276`, `src/backend.rs:1753`).
...
| --- | ---: | ---: |
| alloc-heavy run 1 | 0.26 | 39308 |
| alloc-heavy run 2 | 0.29 | 39308 |
| alloc-heavy run 3 | 0.25 | 39372 |
...
| control run 2 | 0.07 | 1708 |
| control run 3 | 0.07 | 1708 |
