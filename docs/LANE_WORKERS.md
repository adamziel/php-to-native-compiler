...
Current next queue after the object-iteration Milestone 1667 slice:
...
- Milestone 1666: COW tests/docs integration, full gate, checkpoint, push, and
  cleanup completed.
- Milestone 1667: bounded by-value public-object and userland `Iterator`
  foreach completed.
- Milestone 1668: next reference/COW container blocker; keep all available
...
- Milestone 1652: prior COW tests/docs integration, full gate, checkpoint,
  push, and cleanup completed.
...
- Object/interface lane: Milestone 1593 targets inherited interface
  compatibility, interface/trait method reflection invocation, exact
  reflection exceptions, constructor/destructor fidelity, magic hooks reached
  by WordPress, class alias/autoload lifecycle, readonly/property hooks,
  adapted recursive trait ordering/conflicts, built-in/internal trait catalogs,
  broader closure/internal reflection targets, or native object/reflection
  lowering.
- Reference/COW lane: Milestone 1594 targets real reference containers,
  general magic-property reference containers, arbitrary reference expressions,
  ...
  reference-returning `__get()` as a by-value normal property read, or native
  lowering diagnostics.
- Request/SAPI/filesystem/stream lane: Milestone 1595 targets exact PHP
  realpath-cache byte accounting, realpath-cache entries from more filesystem
  operations, broader stat-cache coverage, include_path/open_basedir policy,
  stream-wrapper cache interaction, cookie-name encoding, exact `ValueError`
  parity, output buffering, shutdown/fatal/destructor ordering, request-body
  lifetime, uploads, host filesystem edge cases, or native lowering.
- WordPress DB/bootstrap lane: Milestone 1596 targets arbitrary prepared SQL,
  broader SQL-mode/string-literal/wildcard parity, direct literal pattern
  ...
  object-cache/transients, hook callback breadth, plugin/theme loading breadth,
  or REST/admin/front-controller bootstrap progress.
- Native runtime ABI lane: Milestone 1597 targets linked native execution,
  dynamic string expression lowering, diagnostics handles, binary PHP string
  value handles, array/object/resource/reference ABI shapes, request-state
  handles, WordPress host-state ABI, C fallback assembly helper calls, or
  explicit native rejections where runtime ABI coverage is still missing.
- Tests/docs lane: Milestone 1598 is the next tests/docs slot after the next
  WordPress-focused implementation batch.

Milestones 1657 through 1660 closed the latest COW-focused implementation
lanes; Milestone 1661 refreshed the queue and recorded the full-gate result.
Milestone 1648 plus COW slices 1652-1656 closed the previous COW-focused
...
Milestones 1581 through 1585 closed the latest WordPress-focused
implementation and native lanes; Milestone 1586 refreshed the queue and
recorded the full-gate result.
Milestones 1575 through 1579 closed the previous WordPress-focused
implementation and native lanes; Milestone 1580 refreshed the queue and
recorded the full-gate result.
Milestones 1569 through 1573 closed the latest WordPress-focused
implementation and native lanes; Milestone 1574 is the integration/full-gate
refresh slot for that batch.
...
full-gate result.
Milestones 1531 through 1534 closed the latest four WordPress-focused
implementation lanes, and Milestone 1535 refreshed the queue and recorded the
full-gate result.
Milestones 1526 through 1529 closed the latest four WordPress-focused
implementation lanes, and Milestone 1530 refreshed the queue and recorded the
full-gate result.
...
implementation lanes, and Milestone 1515 refreshed the queue and recorded the
full-gate result.
Milestones 1506 through 1509 closed the previous four WordPress-focused
implementation lanes, and Milestone 1510 refreshed the queue and recorded the
full-gate result.
Milestones 1501 through 1504 closed the previous four WordPress-focused
...
implementation lanes, and Milestone 1485 refreshed the queue and recorded the
full-gate result.
...
593, 598, 603, 608, 613, 618, 1202, 1207, 1212, 1217, 1222, 1223, 1227, 1228, 1232, 1233, 1237, 1238, 1242, 1243, 1247, 1248, 1252, 1253, 1257, 1258, 1262, 1263, 1267, 1268, 1272, 1273, 1277, 1278, 1282, 1283, 1287, 1288, 1292, 1293, 1302, 1303, 1307, 1308, 1312, 1313, 1317, 1318, 1322, 1323, 1327, 1328, 1332, 1333, 1337, 1338, 1342, 1343, 1347, 1348, 1352, 1353, 1357, 1358, 1362, 1363, 1367, 1368, 1372, 1373, 1377, 1378, 1382, 1383, 1387, 1388, 1392, 1393, 1397, 1398, 1402, 1403, 1407, 1408, 1412, 1413, 1417, 1418, 1422, 1423, 1427, 1428, 1432, 1433, 1437, 1438, 1442, 1443, 1447, and 1448 closed recent runtime slots, and Milestones 569,
573, 588, 591, 596, 601, 606, 611, 616, 1203, 1208, 1213, and 1218 closed recent IR/lowering slots.
Milestones 1224, 1229, 1234, 1239, 1244, 1249, 1254, 1259, 1264, 1269, 1274, 1279, 1284, 1289, 1294, 1304, 1309, 1314, 1319, 1324, 1329, 1334, 1339, 1344, 1349, 1354, 1359, 1364, 1369, 1374, 1379, 1384, 1389, 1394, 1399, 1404, 1409, 1414, 1419, 1424, 1429, 1434, 1439, 1444, and 1449 closed recent WordPress compatibility evidence work. Milestones
604, 609, 614, 619, 1205, 1210, 1215, 1220, 1225, 1230, 1235, 1240, 1245, 1250, 1255, 1260, 1265, 1270, 1275, 1280, 1285, 1290, 1295, 1300, 1305, 1310, 1315, 1320, 1325, 1330, 1335, 1340, 1345, 1350, 1355, 1360, 1365, 1370, 1375, 1380, 1385, 1390, 1395, 1400, 1405, 1410, 1415, 1420, 1425, 1430, 1435, 1440, 1445, and 1450 closed recent tests/docs
queue refreshes.
